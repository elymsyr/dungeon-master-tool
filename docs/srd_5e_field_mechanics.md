...
| **`spellcasting_focus`** | text | n/a | UI: focus seçimi öneri | none | n/a | s.96–97. Inventory'de Focus item'ı varsa V/S/M satisfy. |
| **`feature_table`** | levelTable (Map<int,int>) | level-up:N | `PlayerCharacter.class_resources`, derived: features-active-at-level | per-row | per-class: kendi seviyesine göre | s.28+. **Açık mekanik #1:** levelTable Map<int,int> tipi feature'ları tip-güvenli temsil edemez (Rage uses, Sneak Attack dice, Bardic Inspiration die, Extra Attack count). |
...
| **`feature_table`** | levelTable | level-up:N (level_in(parent_class) ≥ N) | `PlayerCharacter.class_resources` | per-row | per-class: parent_class seviyesine göre | s.28+. **Açık mekanik #1:** Class.feature_table ile aynı yetersizlik. |
| **`flavor_description`** | markdown | n/a | — | none | n/a | UI/lore. |

**Açık konular:** `feature_table` typed feature listesi olmalı (§5 #1).
...
| **`description`** \* | markdown | cast | derived: effect resolution | manual-import | n/a | s.107+. **Açık mekanik:** Damage rolls, save effects, condition grants free-form metin; SpellAttack/SaveOrDamage/ConditionOnAttack DSL primitives yok. |
| **`at_higher_levels`** | levelTextTable | cast (slot > base level) | derived: scaling effect | manual-import | n/a | s.104. Free-form per-level. |
...
| **`save_dc`** | text | save-roll | derived: target save DC | parse | n/a | "DC 13 DEX" gibi free-form; parser gerek. |
...
| **`sense_grants`** | textarea | always | derived: senses | manual-import | n/a | s.255. e.g. "darkvision 60 ft." |
...
| **`traits`** | markdown | always | derived: passive abilities | manual-import | n/a | s.255. |
| **`actions`** \* | markdown | use | action economy | manual-import | n/a | s.255. Creature-action listesi alternatif. |
| **`bonus_actions`** | markdown | use | BA economy | manual-import | n/a | s.255. |
| **`reactions`** | markdown | use | reaction economy | manual-import | n/a | s.255. |
| **`legendary_action_uses`** | integer (0–5) | turn-start | LA pool reset | set | n/a | s.255. |
| **`legendary_actions`** | markdown | use (LA) | LA economy | manual-import | n/a | s.255. |
| **`lair_actions`** | markdown | initiative 20 (in lair) | lair action trigger | manual-import | n/a | s.255. |
| **`spellcasting_block`** | markdown | cast | derived: spell list + slots | manual-import | n/a | s.255. **Açık konu:** Free-form; spell relations kullanılmıyor (gear_refs Spell tutmaz). |
...
| **`damage_dice`** | text | damage-roll | damage | parse-and-roll | n/a | **Açık konu:** dice tipi kullanılmamış (text); parser gerek. |
...
Şema-vs-SRD uyuşmazlıkları. Hepsi sadece dokümante edilir; bu iterasyonda şema değiştirilmez. Numaralar §2/§3 satırlarındaki çapraz referanslara karşılık gelir.

1. **`Class.feature_table` / `Subclass.feature_table` `levelTable` (Map<int,int>)** — feature'ları typed temsil edemez (Rage uses, Sneak Attack dice, Bardic Inspiration die, Extra Attack count). Numeric only. Deferred: typed `level→feature[]` shape.
...
**Ek notlar (yapısal eksikler — yeni numara almıyor):**
- `Trap.damage_dice` `text` tipinde (`dice` değil) — tutarsızlık.
- `creature-action.save_dc` `text` ("DC 13 DEX") — parser gerek.
- `creature-action.recharge` `text` ("5-6", "Short Rest") — parser gerek.
- `Monster.spellcasting_block` markdown — spell relations kullanılmıyor; cast zincirine bağlanmıyor.
- `Pack.contents` markdown — quantity-on-relation desteği yok (design §9 #2).
