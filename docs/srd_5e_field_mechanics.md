...
### 0.6 `grantedModifiers` DSL

Species traits, Feat benefits, MagicItem effects, Trait description gibi yapısal bonuslar için tek typed liste tipi. Her satır:

```
{
  kind: <ModifierKind>,
  target_kind: <ability|skill|save|damage-type|condition|sense|speed-type|weapon-category|class|spell|feat|tool|language|armor-category|null>,
  target_ref: <UUID of lookup/entity row OR null>,
  value: <int OR null>,
  scaling: <"flat"|"per-level"|"per-proficiency-bonus"|null>,
  condition_ref: <condition lookup UUID OR null — bu modifier sadece şart sağlanırsa aktif>,
  notes: <string OR null — narrative override>,
}
```

**Controlled vocabulary — `kind` (24 değer):**

| Kind | target_kind | value | Açıklama / Örnek |
|---|---|---|---|
| `ability_score_bonus` | ability | int | +1 STR (Tough, Dwarf) |
| `ability_score_max_increase` | ability | int | Max cap ↑ (Epic Boon: 20→30) |
| `ac_bonus` | null | int | Cloak of Protection +1 AC |
| `save_bonus` | ability or null | int | Cloak of Protection +1 all saves; Resilient +1 specific |
| `skill_bonus` | skill | int | Helm of Telepathy unused |
| `attack_bonus` | weapon-category or null | int | Bracers of Archery +2 ranged; +1 Sword |
| `damage_bonus` | damage-type or weapon-category or null | int | +1 weapon, +2 fire damage |
| `speed_bonus` | speed-type | int | Mobile +10 walking; Boots of Speed |
| `hp_bonus_flat` | null | int | Aasimar L1 +1 HP |
| `hp_bonus_per_level` | null | int | Tough +2/level |
| `initiative_bonus` | null | int | Alert +PB; Boots of Speed +5 |
| `passive_score_bonus` | skill | int | Observant +5 passive Perception/Investigation |
| `resistance_grant` | damage-type | null | Dwarf poison resistance |
| `immunity_grant` | damage-type or condition | null | Periapt of Proof — Poison immune |
| `vulnerability_grant` | damage-type | null | Lycanthrope silver weakness |
| `condition_save_advantage` | condition | null | Brave (Halfling) — adv vs Frightened |
| `condition_save_immunity` | condition | null | Periapt — Disease immune |
| `attack_advantage_vs` | condition | null | (e.g. paralyzed targets) |
| `proficiency_grant` | skill\|tool\|weapon\|armor-category\|save | null (target_ref required) | Skilled feat granted skill |
| `expertise_grant` | skill | null | Prodigy expertise |
| `language_grant` | language | null (target_ref) | Linguist |
| `sense_grant` | sense | int (range_ft) | Darkvision 60ft (Elf) |
| `spell_known_grant` | spell | null | Magic Initiate granted cantrip/L1 |
| `spell_at_will_grant` | spell | null | Drow Magic — Faerie Fire 1/day |
| `feat_grant` | feat | null | Background.origin_feat_ref akışı |
| `feature_text` | null | null | Free-form fallback (notes alanında) |

**Multiclass davranışı:**
- Modifier'lar yayılımsızdır: kaynak entity uygulandığı sürece aktif (Species sürekli, Feat aldığında, MagicItem equipped+attuned, Trait grant edildiğinde).
- Aynı `kind+target_ref` çoklu kaynaktan gelirse: `bonus` türleri **toplanır** (stacking allowed by default — istisna magic bonus non-stack rule SRD §1.5'te değil; magic AC vs Shield spell kuralları homebrew).
- `resistance/immunity/advantage` flag'leri **non-stack** — varsa var.
- `condition_ref` set ise modifier sadece koşul aktifken uygulanır (Rage'de damage resistance gibi class feature uygulamaları).

Tam zincir validation §4'te.

### 0.7 3-State Checkbox Sistemi

Her kategori subsection'ının altındaki **Mekanik Kapsamı** listesi 3 checkbox kullanır:

```
- [ ] [ ] [ ] **`field_key` → hedef** — açıklama (s.NNN).
   ↑   ↑   ↑
   p   s   i
```

| Pozisyon | Etiket | Anlamı |
|---|---|---|
| 1 | **[p] Planned** | Mekanik gelecek roadmap'e eklendi (issue / task açıldı) |
| 2 | **[s] Supported** | Şemada bu mekaniği taşıyan field tanımlı (`content.dart` / `dm.dart`) |
| 3 | **[i] Implemented** | Dart runtime'da resolver bu mekaniği uyguluyor (test'le doğrulanmış) |

İlk yazımda **hiçbiri işaretlenmez** — kullanıcı zaman içinde işaretler. `[p][s]` = data tutuluyor ama runtime hiçbir şey yapmıyor (form doldur, etki yok). `[p][s][i]` = tam çalışıyor. Yan-dosya `srd_5e_mechanics.md` ile aynı semantik.

**Tablolardan farkı:** mevcut 7-sütunlu tablo (Alan / Tip / Tetik / Hedef / Op / Multiclass / SRD) **şema spec**'tir; checkbox listesi **progress tracker**'dır. Tablo dokunulmaz — checkbox blok ona ek bir görsel kontrol katmanıdır.
...
**Açıklama:** Lookup field'ların çoğu sadece identifier (name/icon/description), ama bazıları mekaniği doğrudan etkiler — yukarıdaki tablo bu mekanik field'ları toplar. PB ve formül ile çarpılan multiplier'lar (size/coin/lifestyle), tag-style flag'ler (condition.stacks, damage-type.is_physical), ve typed enum dispatcher'lar (weapon-property.mechanic_kind, weapon-mastery.effect_kind).

**Mekanik kapsamı:**

- [x] [x] [ ] **`size.space_ft` → creature space** — Tiny=2.5, S/M=5, Large=10, Huge=15, Gargantuan=20.
- [x] [x] [ ] **`size.hit_die_size` → monster default HD** — Class HD'si yoksa fallback (s.22).
- [x] [x] [ ] **`size.carrying_multiplier` → carry capacity** — `STR × 15 × multiplier` (s.16).
- [x] [x] [ ] **`sense.default_range_ft` → rangedSense fallback** — Darkvision 60, Truesight 120 vb. consumer override edebilir.
- [x] [x] [ ] **`damage-type.bypassable_by_magical` → resistance bypass** — Magical/silvered/adamantine nonmagical resistance'ı bypass eder (sadece B/P/S için true).
- [x] [x] [ ] **`rarity.value_gp` → fiyat formülü** — Common 100/Uncommon 400/Rare 4000/VR 40000/Legendary 200000; consumable ½.
- [x] [x] [ ] **`rarity.crafting_time_days` → downtime tracker** — Crafting süreci günleri.
- [x] [x] [ ] **`rarity.crafting_cost_gp` → gp drain** — Crafting maliyeti.
- [x] [x] [ ] **`coin.value_in_gp` → currency conversion** — CP=0.01, SP=0.1, EP=0.5, GP=1, PP=10; 50 coins = 1 lb.
- [x] [x] [ ] **`lifestyle.cost_per_day_gp` → günlük gp drain** — Wretched 0, Squalid 0.1, ..., Aristocratic ≥10.
- [x] [x] [ ] **`condition.stacks` → stack rule** — Sadece Exhaustion true; diğerleri non-stack.
- [x] [x] [ ] **`condition.grants_incapacitated` → incapacitated chain** — Stunned/Paralyzed/Petrified/Unconscious true → no actions + concentration drop.
- [x] [x] [ ] **`damage-type.is_physical` → B/P/S classification** — Magic weapon resistance bypass için kullanılır.
- [x] [x] [ ] **`duration-unit.is_concentration_compatible` → concentration check** — Rounds/Minutes/Hours true; Instantaneous false.
- [x] [x] [ ] **`weapon-property.mechanic_kind` → property dispatch** — 11 kind enum (Finesse/Heavy/Light vs.); resolver typed switch.
- [x] [x] [ ] **`weapon-mastery.effect_kind` → mastery effect dispatch** — 8 kind enum (Cleave/Graze/Nick/Push/Sap/Slow/Topple/Vex).
- [x] [x] [ ] **`weapon-mastery.effect_value` → mesafe/değer** — Push=10ft, Slow=10ft, Cleave/Graze=0.
- [x] [x] [ ] **`weapon-mastery.save_ability` → mastery save** — Topple=CON; diğerleri save'siz.

**§1.1 Identifier-only lookups (mekanik etkisi yok, sadece name/icon/description):**

- [x] [x] [ ] **36 identifier-only lookup şeması** — ability/skill/damage-type/condition/size/creature-type/alignment/language/weapon-category/property/mastery/armor-category/tool-category/spell-school/magic-item-category/rarity/speed-type/sense/action/area-shape/attitude/cover/illumination/hazard/feat-category/lifestyle/coin/tier-of-play/travel-pace/arcane-focus/druidic-focus/holy-symbol/plane/casting-component/casting-time-unit/duration-unit. Mekanik consumer field'larında uygulanır.
...
**Açıklama:** Bir karakter sınıfa atandığında HD/HP, save profs, weapon/armor profs, skill seçim havuzları akar. Multiclass'ta save profs ve starting equipment yalnız ilk sınıftan; weapon/armor subset SRD s.25 tablosuna göre. Caster sınıflar `casting_ability_ref` üzerinden `spell_save_dc` formülünü besler; spell slot havuzu çoklu caster'da combined-table ile birleşir (s.26).
...
- [x] [x] [ ] **`primary_ability_ref` → multiclass eligibility** — Yeni sınıf eklenirken hem mevcut hem yeni sınıfın primary'si ≥13 olmalı; PC'a yazılmaz, gate kontrolünde okunur.
- [x] [x] [ ] **`hit_die` → `combat_stats.max_hp` & `hit_dice_remaining`** — İlk sınıfta L1 max+CON, sonraki seviyelerde avg+CON eklenir; her seviye HD havuzuna 1 die katar.
- [x] [x] [ ] **`saving_throw_refs` → `saving_throw_proficiencies[]`** — Sadece ilk sınıftan akar; multiclass'ta sonraki sınıfın save'leri gelmez.
- [x] [x] [ ] **`skill_proficiency_choice_count` + `skill_proficiency_options[]` → skill choice pool** — UI'de N adet kutu açar, kullanıcı seçer; sonuç `skill_proficiencies`'e union.
- [x] [x] [ ] **`weapon_proficiency_categories[]` → `weapon_proficiencies[]`** — Multiclass'ta SRD s.25 subset (Barb/Fight/Pal/Ranger martial alır, diğerleri kısıtlı).
- [x] [x] [ ] **`weapon_proficiency_specifics[]` → `weapon_proficiencies[]`** — Spesifik silah grant'leri; multiclass'ta tipik gelmez (sınıfa özel).
- [x] [x] [ ] **`tool_proficiency_count` + `tool_proficiency_options[]` → `tool_proficiencies[]`** — Skill ile aynı choice mantığı, first-class-only.
- [x] [x] [ ] **`armor_training_refs[]` → `armor_trainings[]`** — Multiclass'ta Light+Medium+Shield genelde gelir; Heavy & Martial sınıfa özel. Heavy worn without training → disadv Str/Dex tests, no spells.
- [x] [x] [ ] **`default_inventory_refs[]` → `PlayerCharacter.inventory[]`** — İlk sınıf seçildiğinde otomatik kopya, sonraki sınıflarda eklenmez.
- [x] [x] [ ] **`starting_gold_dice` → `PlayerCharacter.gp`** — İlk sınıfta zar atılır; background gold ile karşılıklı (biri seçilir).
- [x] [x] [ ] **`casting_ability_ref` → `spell_save_dc` / `spell_attack_bonus`** — Formül: DC = `8 + PB + ability_mod`, attack = `PB + ability_mod`. Caster olmayan sınıfta null. Multiclass'ta her caster kendi DC'sini hesaplar (şu an tek skaler — açık mekanik §5 #3).
- [x] [x] [ ] **`caster_kind` → spell slot havuzu girişi** — Full=lvl, Half=½lvl, Third=⅓lvl combined-table'a feed eder; Pact ayrı havuz; Ritual slot vermez.
- [x] [x] [ ] **`spellcasting_focus_ref` → V/S/M satisfaction flag** — Inventory'de focus item varsa Material component shortcut; UI rehberi.
- [x] [x] [ ] **`features` → `class_resources` + level-gated feature aktivasyonu** — Typed feature satırları (level/name/dice/uses/recharge); level-up'ta yeni satırlar aktive olur, Rage/Sneak Attack vb. bu havuzdan beslenir.
- [x] [x] [ ] **`cantrips_known_by_level` → `spells_known[type=cantrip]` cap** — Level başına bilinen cantrip sayısı sınırı; her caster sınıf kendi cap'i.
- [x] [x] [ ] **`prepared_spells_by_level` → `prepared_spells[]` cap** — Long-rest'te yeniden seçilebilen prepared spell sayısı; multiclass'ta her sınıf ayrı.
- [x] [x] [ ] **`spell_slots_by_level` → `spell_slots`** — Level-up + long-rest'te combined-table'a girer; multiclass'ta tüm caster'lar tek slot havuzunda toplanır (Pact ayrı).
- [x] [x] [ ] **`multiclass_requirements` → eligibility gate** — Karakter sınıfa girmeden önce kontrol; başarısızsa class atama bloklanır.
- [x] [x] [ ] **`primary_ability_ref` advisory + `secondary_ability_ref` advisory** — UI'de "stat dağıtımı için önerilen" rehber; mekanik etki yok.
- [x] [x] [ ] **`complexity` advisory** — UI'de zorluk rozeti; mekanik etki yok.
- [x] [x] [ ] **`starting_equipment_options` markdown choice prompt** — Narrative "A vs B" karar metni; typed import `default_inventory_refs` üstünden.
- [x] [x] [ ] **Class-aware multiclass starting profs subset (s.25)** — Şemada kodlu değil, narrative `multiclass_requirements`'tan parse edilmesi gerek (açık mekanik §5 #7).
...
**Açıklama:** Subclass parent class'a bağlı feature seti. Çoğu sınıf L3'te, Cleric L1'de açılır. Subclass kendi `features` listesi parent class'ın seviyesine bakarak aktive olur (`level_in(parent_class) ≥ N`).

**Mekanik kapsamı:**

- [x] [x] [ ] **`parent_class_ref` → eligibility gate** — Subclass yalnız parent class içinde geçerli; başka sınıfa atanamaz.
- [x] [x] [ ] **`granted_at_level` → aktivasyon seviyesi** — Parent class'ın level_in değeri bu eşiğe ulaştığında subclass features tetiklenir.
- [x] [x] [ ] **`features` → `class_resources`** — Typed feature satırları parent class feature'ları gibi level-up'ta sıralı aktive olur.
- [x] [x] [ ] **`flavor_description` → UI lore** — Mekanik etki yok, sadece subclass tanıtım metni.
...
| **`granted_modifiers`** [] | grantedModifiers (DSL §0.6) | always | PC derived stats per modifier kind | union (per modifier) | n/a | **K6 kapandı:** Typed bonus DSL. Lucky reroll, Brave adv, Goliath Powerful Build encode edilir. |
| **`traits`** \* | markdown | always | UI: narrative | manual | n/a | s.84-86. Narrative; typed `granted_modifiers`. |
...
**Açıklama:** Species karakter yaratımında bir kez seçilir; size, hız, creature type, dil/sense/resistance/skill bonusları + species-spesifik traits (Lucky, Brave, Powerful Build) hep kalıcı uygulanır. Sayısal bonuslar `granted_modifiers` DSL'inde, narrative kalan kısım `traits` markdown'da.

**Açık konular kapandı:** `granted_modifiers` typed DSL ile Lucky reroll-on-1 (`condition_save_advantage` veya `feature_text` notes), Halfling Nimbleness (`feature_text`), Goliath Powerful Build (`feature_text`) — non-bonus özel kuralllar `feature_text` fallback'ine düşer; numeric ise typed kind'lar (örn. ability_score_bonus, sense_grant) kullanılır.

**Mekanik kapsamı:**

- [x] [x] [ ] **`size_ref` → space + carry capacity** — Tiny/Small/Medium/Large; carrying = `STR × multiplier(size)`. Çoğu species Medium.
- [x] [x] [ ] **`speed_ft` → `combat_stats.speed`** — Walk speed override base; class feature/feat (Fast Movement, Mobile) üstüne ekler.
- [x] [x] [ ] **`creature_type_ref` → `PlayerCharacter.creature_type`** — Spell targeting (Hold Person sadece Humanoid), monster ability ref'leri için kullanılır.
- [x] [x] [ ] **`granted_modifiers[]` → PC derived stats** — Typed ability_score_bonus, sense_grant, resistance_grant vs. her modifier kind'ı kendi target'ına uygulanır.
- [x] [x] [ ] **`traits` → narrative UI** — Markdown; özel kurallar `granted_modifiers` ile kapsanmadıysa sadece açıklayıcı metin kalır.
- [x] [x] [ ] **`granted_languages[]` → `languages[]`** — Common otomatik default; bu liste ek diller union eder.
- [x] [x] [ ] **`granted_senses[]` → `senses[]`** — Darkvision 60 (most), 120 (Dwarf/Orc), Truesight (rare).
- [x] [x] [ ] **`granted_damage_resistances[]` → resistance set** — Damage roll'da tüketilir; Dwarf=Poison, Tiefling=Fire vb.
- [x] [x] [ ] **`granted_skill_proficiencies[]` → `skill_proficiencies[]`** — Elf Keen Senses gibi sabit skill grant'leri union edilir.
- [x] [x] [ ] **`age` → lore only** — Mekanik etki yok.
...
**Açıklama:** Background karakter yaratımında origin paketi sağlar: 2 skill prof, opsiyonel tool prof, ASI dağılımı (+2/+1 veya +1/+1/+1), origin feat, equipment paketi. Multiclass'tan etkilenmez — tek seferlik atama.

**Mekanik kapsamı:**

- [x] [x] [ ] **`granted_skill_refs[]` → `skill_proficiencies[]`** — 2 sabit skill, union edilir.
- [x] [x] [ ] **`granted_tool_refs[]` → `tool_proficiencies[]`** — Sabit veya choice tool prof grant'ı.
- [x] [x] [ ] **`granted_language_count` → language seçim sayısı** — UI choice; SRD 2024'te genelde 0.
- [x] [x] [ ] **`ability_score_options[]` + ASI dağılımı → `stat_block`** — +2/+1 veya +1/+1/+1 manuel dağıtım; max 20 cap. Açık mekanik §5 #12.
- [x] [x] [ ] **`origin_feat_ref` → `feats[]`** — Background L1 feat (Acolyte=Magic Initiate Cleric, Criminal=Alert vb.) otomatik append.
- [x] [x] [ ] **`default_inventory_refs[]` → `inventory[]`** — Equipment paketi auto-import.
- [x] [x] [ ] **`starting_equipment` markdown choice** — Narrative "A vs B" seçim metni; typed import default_inventory_refs.
- [x] [x] [ ] **`starting_gold_gp` → `gp`** — Default başlangıç altını; class.starting_gold_dice ile karşılıklı.
- [x] [x] [ ] **`gold_alternative_gp` → `gp` (replace inventory)** — Equipment paketi yerine altın seçeneği (~50 GP).
...
| **`granted_modifiers`** [] | grantedModifiers (DSL §0.6) | always (after take) | PC derived stats per modifier kind | union | n/a | **K6 kapandı:** Alert init+PB, Lucky reroll, Tough +HP/level, Observant +5 passive, Mobile +10 speed encode edilebilir. |
| **`benefits`** \* | markdown | always | UI: narrative | manual | n/a | s.87. Narrative; typed `granted_modifiers`. |

**Açıklama:** Feat karakter yaratımında veya level-up'ta alınabilen yetenek paketi. 4 kategori: Origin (L1 background), General (L4+ ASI slot), Fighting Style (class feature), Epic Boon (L19+). Prereq sistemi typed alanlardan eligibility'yi türetir; benefits hem ASI (`asi_*`) hem ongoing modifier (`granted_modifiers`) içerebilir.

**Mekanik kapsamı:**

- [x] [x] [ ] **`category_ref` → eligibility gating** — Origin=L1, General=L4+ ASI, Fighting Style=class feature, Epic Boon=L19.
- [x] [x] [ ] **`prereq_ability_ref` + `prereq_min_score` → ability gate** — Heavy Armor Master STR≥13 gibi typed kontrol.
- [x] [x] [ ] **`prereq_class_refs[]` → class gate** — Sadece bu sınıflara açık (örn. War Caster caster sınıflarda).
- [x] [x] [ ] **`prereq_species_refs[]` → species gate** — Species-locked feat'lar (rare).
- [x] [x] [ ] **`prereq_min_character_level` → total level gate** — Epic Boons L19+ kontrolü.
- [x] [x] [ ] **`prereq_requires_spellcasting` → caster gate** — PC herhangi bir caster_kind ≠ None mı kontrolü (Magic Initiate, Ritual Caster).
- [x] [x] [ ] **`prerequisite` markdown** — Narrative açıklama; gating typed alanlardan.
- [x] [x] [ ] **`repeatable` + `repeatable_limit` → take counter** — Aynı feat'ı birden fazla alma izni; ASI/Skilled/Magic Initiate variant'lar.
- [x] [x] [ ] **`asi_ability_options[]` + `asi_amount` + `asi_max_score` → `stat_block` artışı** — Typed ASI uygulaması; cap'i aşmaz.
- [x] [x] [ ] **`ability_score_increase` markdown** — Narrative ASI metni; typed alanlar `asi_*`.
- [x] [x] [ ] **`granted_modifiers[]` → PC derived stats** — Alert init+PB, Tough +HP/level, Lucky reroll, Mobile +10 speed, Observant +5 passive vs.
- [x] [x] [ ] **`benefits` markdown** — Narrative; typed kapsama dışı kuralları açıklar.
...
**Açıklama:** Spell entity bir tarif: level/school/casting time/range/area/components/duration + `effects` typed DSL. Karakterin `spells_known` veya `prepared_spells` listesine girince `cast` action çağrılabilir; uygun slot harcanır, `effects` satırı çalışır (damage roll, save, condition apply). Concentration tek-instance; yenisi öncekini düşürür.

**Mekanik kapsamı:**

- [x] [x] [ ] **`level` → required slot** — Cantrip (0) slot harcamaz; L1+ slot gerektirir, upcast slot ≥ level kontrolü.
- [x] [x] [ ] **`school_ref` → enum tag** — Counterspell/Detect Magic gibi school-bazlı interaction'lar için.
- [x] [x] [ ] **`casting_time_amount` + `casting_time_unit_ref` → action economy** — Action/BA/Reaction/dakika/saat.
- [x] [x] [ ] **`reaction_trigger` → reaction trigger string** — Reaction casting şart metni (Shield "when hit" vb.).
- [x] [x] [ ] **`is_ritual` → ritual cast option** — Slot harcamadan +10 dakika ile cast; Ritual Adept prepare etmeden mümkün kılar.
- [x] [x] [ ] **`range_type` + `range_ft` → targeting** — Self/Touch/Ranged/Sight/Unlimited; Ranged ise feet sayısı.
- [x] [x] [ ] **`area_shape_ref` + `area_size_ft` → AoE coverage** — Cone/Cube/Cylinder/Emanation/Line/Sphere; size feet.
- [x] [x] [ ] **`components[]` → V/S/M satisfy** — V=verbal, S=somatic, M=material; focus item M'i karşılar.
- [x] [x] [ ] **`material_description` + `material_cost_gp` + `material_consumed` → M cost** — Consumed=true ise gp düşer; örn Find Familiar 10gp consumed, Identify 100gp not.
- [x] [x] [ ] **`duration_unit_ref` + `duration_amount` → ends-at** — Instantaneous/Round/Minute/Hour/Day/Until Dispelled.
- [x] [x] [ ] **`requires_concentration` → `concentration` slot** — Tek concentration; yenisi eski'yi düşürür (s.179).
- [x] [x] [ ] **`description` markdown** — Narrative; mekanik etki `effects`'ten.
- [x] [x] [ ] **`effects[]` typed DSL → resolution** — Her satır kind (damage/heal/condition/buff/debuff) + dice + save_ability + save_effect (none/half/negate/partial); cast sırasında satır satır resolve.
- [x] [x] [ ] **`at_higher_levels_text` → upcast scaling** — Slot > base level'da ek dice; numeric scaling effects[].scaling_dice'da.
- [x] [x] [ ] **`class_refs[]` → spell list belonging** — PC.spells_known'a ekleme gating; multiclass'ta her sınıf kendi listesine bakar.
- [x] [x] [ ] **`damage_type_refs[]` → resistance check** — Damage roll'da hedefin resistance/vulnerability/immunity setiyle karşılaştırılır.
- [x] [x] [ ] **`save_ability_ref` → target save** — Save DC = caster'ın spell_save_dc'si; başarılı save → save_effect uygulanır.
- [x] [x] [ ] **`attack_type` → attack roll variant** — None/Melee/Ranged spell attack; vokabüler farkı creature-action ile (§5 #9).
- [x] [x] [ ] **`applied_condition_refs[]` → target conditions[]** — Spell-level condition apply (effects[] içindeki ile birlikte).
...
**Açıklama:** Weapon equip edildiğinde attack action'da kullanılır. Damage roll = weapon.damage_dice + ability_mod (STR melee, DEX ranged, Finesse seçim). `property_refs` ve `mastery_ref` typed enum dispatch ile davranış modifierları (Heavy disadv Small, Light off-hand, Reach 10ft, Mastery Cleave/Push/Sap vb.) sağlar.

**Mekanik kapsamı:**

- [x] [x] [ ] **`category_ref` → prof check** — Simple/Martial; PC.weapon_proficiencies içeriyorsa attack roll'a PB eklenir.
- [x] [x] [ ] **`is_melee` → ability default** — STR (melee) / DEX (ranged); Finesse property override eder.
- [x] [x] [ ] **`damage_dice` → damage roll** — Roll + ability_mod; crit'te dice 2x.
- [x] [x] [ ] **`damage_type_ref` → resistance check** — Bludg/Pierc/Slash + magical type'lar.
- [x] [x] [ ] **`property_refs[]` → behavior modifiers** — Her property mechanic_kind enum'dan dispatch (Finesse/Heavy/Light/Two-Handed/Versatile/Reach/Thrown/Loading/Ammunition).
- [x] [x] [ ] **`mastery_ref` → mastery effect** — Cleave/Graze/Nick/Push/Sap/Slow/Topple/Vex; aktif mastery slotuna bağlı non-stack.
- [x] [x] [ ] **`normal_range_ft` + `long_range_ft` → range gating** — Beyond normal disadv, beyond long auto miss.
- [x] [x] [ ] **`versatile_damage_dice` → 2H grip damage** — Versatile property gerekli; iki elle tutulduğunda alternate dice.
- [x] [x] [ ] **`ammunition_type_ref` → ammo consumption** — Loading: 1 ammo per Action/BA/Reaction; recover ½ after fight.
- [x] [x] [ ] **`cost_gp` → buy/sell** — Sell ½ default.
- [x] [x] [ ] **`weight_lb` → carry total** — Encumbrance toplamına ekler.
...
**Açıklama:** Armor equip edildiğinde AC formülünü besler. Light=base+DEX, Medium=base+min(DEX,2), Heavy=base only. Training yoksa Str/Dex tests disadv + spell cast yasak. STR<requirement → speed −10. Stealth disadvantage flag'ı stealth check'lere uygulanır.

**Mekanik kapsamı:**

- [x] [x] [ ] **`category_ref` → training check** — armor_trainings içermiyorsa disadv all Str/Dex tests + no spell.
- [x] [x] [ ] **`base_ac` + `adds_dex` + `dex_cap` → `combat_stats.ac`** — Formula: `base + (DEX cap'lı) + shield + misc`.
- [x] [x] [ ] **`strength_requirement` → speed penalty** — STR<req → speed −10.
- [x] [x] [ ] **`stealth_disadvantage` → stealth check** — DEX(Stealth) check'lerine disadv.
- [x] [x] [ ] **`don_time_minutes` / `doff_time_minutes` → time cost** — Light 1m, Medium 5m, Heavy 10m, Shield 1 action.
- [x] [x] [ ] **`cost_gp` → buy/sell** — Sell ½ default.
- [x] [x] [ ] **`weight_lb` → carry total** — Encumbrance ekleme.
...
**Açıklama:** Tool prof'una sahipse PB eklenir; ilgili skill prof varsa Advantage. `craftable_items` listesi crafting (s.103) için input. Variant_of ile sub-tool grupları (Carpenter's = Artisan's variant) bağlı.

**Mekanik kapsamı:**

- [x] [x] [ ] **`category_ref` → grouping** — Artisan/Gaming/Musical/Other.
- [x] [x] [ ] **`variant_of_ref` → variant link** — Sub-tool grupları için bağlantı.
- [x] [x] [ ] **`ability_ref` → check ability** — Tool kullanımında default ability rolü (örn Thieves' Tools = DEX).
- [x] [x] [ ] **`utilize_check_dc` → default DC** — Utilize action default zorluk.
- [x] [x] [ ] **`utilize_description` → usage hint** — UI rehber metni.
- [x] [x] [ ] **`craftable_items[]` → crafting input** — Bu tool ile yapılabilen eşya listesi (s.103).
- [x] [x] [ ] **`cost_gp` / `weight_lb` → ekonomi/carry** — Standart eşya alanları.
...
**Açıklama:** Adventuring Gear genel ekipman havuzu. Utilize action ile DC + ability check (Healer's Kit DC10 vb.). Consumable=true ise kullanımda envanterden düşer. is_focus=true olan item'lar V/S/M satisfy eder; class.spellcasting_focus_ref ile uyuşmalı.

**Mekanik kapsamı:**

- [x] [x] [ ] **`cost_cp` → currency conversion** — CP precision; coin lookup üstünden cp/sp/ep/gp/pp arasına dönüştürülür.
- [x] [x] [ ] **`weight_lb` → carry total** — Encumbrance.
- [x] [x] [ ] **`utilize_check_dc` + `utilize_ability_ref` → ability check** — Utilize action'da DC + ability rolü.
- [x] [x] [ ] **`utilize_description` → narrative effect** — Markdown; mekanik gating typed alanlar üstünden.
- [x] [x] [ ] **`consumable` → inventory dec** — Use action'da envanterden 1 düşer (potion, holy water vb.).
- [x] [x] [ ] **`is_focus` + `focus_kind_ref` → V/S/M satisfy** — Spellcasting focus eşleşmesi sağlar.
...
**Açıklama:** Ammunition bundle olarak alınır (20 ok, 30 sapan taşı vb.); attack'ta 1 düşer, savaş sonrası ½ recover. Weapon.ammunition_type_ref ile eşleşir.

**Mekanik kapsamı:**

- [x] [x] [ ] **`storage_container` → UI container hint** — Quiver/pouch/case ipucu.
- [x] [x] [ ] **`cost_gp` / `weight_lb` → ekonomi/carry** — Bundle başına.
- [x] [x] [ ] **`bundle_count` → attack consumption** — Attack başına 1 düşer; savaş sonu ½ recover.
...
**Açıklama:** Pack hazır eşya bundle'ı (Explorer's Pack, Dungeoneer's Pack vb.). Satın alındığında veya açıldığında `content_refs` + `content_quantities` typed map'e göre PC.inventory'ye auto-import edilir.

**Mekanik kapsamı:**

- [x] [x] [ ] **`cost_gp` → buy** — Toplam pack fiyatı.
- [x] [x] [ ] **`weight_lb` → carry total** — Pack ağırlığı.
- [x] [x] [ ] **`content_refs[]` + `content_quantities` → `inventory[]`** — Map<entity_id, qty> ile auto-import (örn {torch_id: 10}).
- [x] [x] [ ] **`contents` markdown** — Narrative; typed alanlar zaten kapsıyor.
...
**Açıklama:** Mount karaktere binildiğinde rider'ın hareket havuzunu mount.speed_ft ile değiştirir. Trained=true ise rider mount'u kontrol eder; aksi takdirde mount kendi initiative'inde hareket eder ve sadece Dash/Disengage/Dodge yapar.

**Mekanik kapsamı:**

- [x] [x] [ ] **`carrying_capacity_lb` → cargo limit** — Mount üzerine yüklenebilen max ağırlık.
- [x] [x] [ ] **`speed_ft` → rider speed** — Mounted iken rider mount.speed_ft kullanır.
- [x] [x] [ ] **`cost_gp` → buy/sell** — Standart ekonomi.
- [x] [x] [ ] **`is_trained` → control mode** — Trained: rider tam kontrol; Untrained: mount independent (s.16).
...
**Açıklama:** Vehicle (Land/Waterborne/Airborne) AC + HP'ye sahiptir; saldırılarda hasar alır, damage_threshold altındaki vuruşları görmezden gelir. Crew gerektirebilir, cargo + passenger sınırı taşır.

**Mekanik kapsamı:**

- [x] [x] [ ] **`vehicle_kind` → grouping** — Land/Waterborne/Airborne tipi.
- [x] [x] [ ] **`speed_mph` → travel speed** — Saatlik kat edilen mesafe.
- [x] [x] [ ] **`crew` / `passengers` → kapasite** — Çalıştıracak ve binebilecek kişi sayısı.
- [x] [x] [ ] **`cargo_tons` → cargo limit** — Max yük.
- [x] [x] [ ] **`ac` + `hp` → combat target** — Saldırılırsa AC/HP üstünden çözülür.
- [x] [x] [ ] **`damage_threshold` → ignore-small-hits** — Bu değerin altında damage 0 sayılır.
- [x] [x] [ ] **`cost_gp` → buy/sell** — Ekonomi.
...
**Açıklama:** Trinket karakter yaratımında opsiyonel d100 tablosundan rastgele alınan küçük gizemli eşya. Mekanik etkisi yok; lore/RP amacı.

**Mekanik kapsamı:**

- [x] [x] [ ] **`roll_d100` → table key** — d100 sonucu trinket'i belirler (s.26-27).
- [x] [x] [ ] **`description` markdown** — Trinket'in açıklaması; flavor only.
...
| **`granted_modifiers`** [] | grantedModifiers (DSL §0.6) | always (when equipped/attuned) | PC derived stats per modifier kind | union | n/a | **K6 kapandı:** Cloak of Protection +1 AC/all saves, Bracers of Archery +2 ranged attack, Bag of Tricks immunity vs X encode edilir. |
| **`effects`** \* | markdown | always (when equipped/attuned) | UI: narrative | manual | n/a | s.207+. Narrative; typed `granted_modifiers`. |
...
**Açıklama:** Magic Item envantere girer; bazıları attunement gerektirir (max 3 attuned slot per PC). Equip + (varsa) attune sonrası `granted_modifiers` aktive olur. Sentient item'lar kendi stat block'una sahiptir; alignment uyumsuzluğu konflikt yaratabilir.

**Mekanik kapsamı:**

- [x] [x] [ ] **`magic_category_ref` → grouping** — Armor/Potion/Ring/Rod/Scroll/Staff/Wand/Weapon/Wondrous.
- [x] [x] [ ] **`rarity_ref` → fiyat + filter** — Rarity.value_gp formülü ekonomi tarafına; UI filter.
- [x] [x] [ ] **`requires_attunement` → attuned_items[] gate** — Max 3 attune slot; untuned magic item çalışır ama sınırlı (sadece passive).
- [x] [x] [ ] **`attunement_class_refs[]` → class gate** — Holy Avenger=Paladin gibi class-locked.
- [x] [x] [ ] **`attunement_species_refs[]` → species gate** — Race-restricted.
- [x] [x] [ ] **`attunement_alignment_refs[]` → alignment gate** — Holy Avenger good-aligned.
- [x] [x] [ ] **`attunement_min_ability_ref` + `attunement_min_ability_score` → ability gate** — Belirli ability score eşiği.
- [x] [x] [ ] **`attunement_spellcaster_only` → caster gate** — Sadece caster sınıflar attune edebilir.
- [x] [x] [ ] **`attunement_prereq` markdown** — Narrative gating; typed gate alanlar baskın.
- [x] [x] [ ] **`is_cursed` → curse flag** — Identification gizler; remove curse spell ile çözülür.
- [x] [x] [ ] **`base_item_ref` → base inheritance** — Longsword +1 gibi item'lar base weapon stats devralır.
- [x] [x] [ ] **`charges_max` + `charge_regain` → charge pool** — Use action ile charge düşer; regain text "1d6+4 at dawn" parser yok (§5 #10).
- [x] [x] [ ] **`activation` → action economy** — None/Magic Action/BA/Reaction/Utilize/Command Word/Consumable.
- [x] [x] [ ] **`command_word` → use string** — Command Word activation için.
- [x] [x] [ ] **`granted_modifiers[]` → PC stats** — Cloak of Protection +1 AC/saves, Bracers of Archery +2 ranged vb.
- [x] [x] [ ] **`effects` markdown** — Narrative; typed kapsama dışı kuralları açıklar.
- [x] [x] [ ] **`cost_gp` / `weight_lb` → ekonomi/carry** — Standart eşya alanları.
- [x] [x] [ ] **`is_sentient` + sentient_int/wis/cha + sentient_alignment_ref → item NPC** — Sentient item kendi stat block'una sahip; alignment çatışması conflict resolution doğurur.
- [x] [x] [ ] **`sentient_communication` / `sentient_senses` / `sentient_special_purpose` → narrative** — Sentient item iletişim/algı/amaç metinleri; DM trigger için.
...
| **`granted_modifiers`** [] | grantedModifiers (DSL §0.6) | always (when assigned) | host entity (NPC/Monster/PC) derived stats per kind | union | n/a | **K6 kapandı:** Trait race-shared shape; Brave/Lucky/Darkvision-grant typed encode. |
| **`description`** | markdown | always (when assigned) | UI: narrative | manual | n/a | Narrative; typed `granted_modifiers`. |

**Açıklama:** Trait yardımcı kategori — NPC/Monster/PC ortak trait listeleri için. Numeric bonuslar `granted_modifiers` typed DSL'inde; description markdown narrative ek.

**Mekanik kapsamı:**

- [x] [x] [ ] **`source` → grouping** — Race/class/monster/feat name etiketi.
- [x] [x] [ ] **`trait_kind` → filter** — Passive/Sense/Defensive/Movement/Spellcasting/Other.
- [x] [x] [ ] **`granted_modifiers[]` → host stats** — Brave/Lucky/Darkvision-grant gibi typed bonusu host entity'e uygular.
- [x] [x] [ ] **`description` markdown** — Narrative; typed kapsama dışı.
...
**Açıklama:** Creature Action NPC/Monster action havuzu. action_type ile economy slotu (Action/BA/Reaction/Legendary/Lair/Mythic/Free), is_attack=true ise attack roll branch'i, recharge_kind ile yenilenme (turn-start roll, rest, dawn). save_dc + save_ability_ref typed pair → target save.

**Mekanik kapsamı:**

- [x] [x] [ ] **`source` → grouping** — Origin (monster/NPC/class) etiketi.
- [x] [x] [ ] **`action_type` → economy slot** — Action/Bonus Action/Reaction/Legendary/Lair/Mythic/Free.
- [x] [x] [ ] **`recharge_kind` + `recharge_min_roll` → recharge** — None/Roll d6 ≥ X/Short Rest/Long Rest/Day/Dawn/Dusk.
- [x] [x] [ ] **`recharge` markdown** — Narrative fallback; typed pair var.
- [x] [x] [ ] **`uses_per_day` → daily charge** — Per-day budget.
- [x] [x] [ ] **`is_attack` → attack branch** — true → attack_kind/bonus/reach/range zorunlu.
- [x] [x] [ ] **`attack_kind` → flow** — Melee/Ranged × Weapon/Spell (4 değer; spell.attack_type ile §5 #9 vokabüler farkı).
- [x] [x] [ ] **`attack_bonus` → roll formül** — d20 + attack_bonus.
- [x] [x] [ ] **`reach_ft` → melee reach** — Default 5.
- [x] [x] [ ] **`range_normal_ft` + `range_long_ft` → ranged ranges** — Beyond normal disadv, beyond long auto miss.
- [x] [x] [ ] **`damage_dice` + `damage_type_ref` → damage roll** — Tipik formula + resistance check.
- [x] [x] [ ] **`save_dc` + `save_ability_ref` → target save** — Typed integer DC + ability rel.
- [x] [x] [ ] **`applied_condition_refs[]` → target conditions[]** — On hit / failed save uygulanan condition union.
- [x] [x] [ ] **`effects[]` typed DSL → resolution** — Spell effects ile aynı: damage/heal/condition/buff/debuff satırları.
- [x] [x] [ ] **`description` markdown** — Narrative; typed alanlar baskın.
...
**Açıklama:** Monster bestiary entry; bir Encounter aracılığıyla combat'a girer. CR + XP encounter difficulty input; HP_average veya HP_dice ile spawn HP belirlenir. Tüm action havuzu (action/BA/reaction/legendary/lair) creature-action ref'leri üstünden; spell havuzu spell ref'leri üstünden typed.

**Mekanik kapsamı:**

- [x] [x] [ ] **`size_ref` → space + carry capacity** — Saldırı reach, hareket alanı.
- [x] [x] [ ] **`creature_type_ref` → targeting** — Hold Person (Humanoid), Turn Undead vb. spell gating.
- [x] [x] [ ] **`tags_line` → UI tags** — "(goblinoid)" gibi parantez tag'ler.
- [x] [x] [ ] **`alignment_ref` → lore** — Mekanik etki yok.
- [x] [x] [ ] **`ac` → hit/miss check** — Stat block AC final değer.
- [x] [x] [ ] **`ac_note` → AC source UI** — "natural armor", "plate" vb. açıklama.
- [x] [x] [ ] **`initiative_modifier` → init roll** — d20 + mod.
- [x] [x] [ ] **`initiative_score` → passive init** — 10 + mod (alternative).
- [x] [x] [ ] **`hp_average` / `hp_dice` → spawn HP** — Default avg; rolled mode dice atar.
- [x] [x] [ ] **`speed_walk_ft` + 4 alternatif speed → speed havuzu** — Walk/Burrow/Climb/Fly/Swim feet.
- [x] [x] [ ] **`can_hover` → fly hover flag** — Fly speed kontrolü.
- [x] [x] [ ] **`stat_block` → ability mods** — STR..CHA scores'tan modifier türevi.
- [x] [x] [ ] **`save_bonuses` → save override** — Stat block save satırı (ability_mod + PB if prof + misc).
- [x] [x] [ ] **`skill_bonuses` → skill override** — Stat block skill satırı (Stealth +6 vb.).
- [x] [x] [ ] **`resistance_refs[]` → ½ damage** — Damage roll'da uygulanır.
- [x] [x] [ ] **`vulnerability_refs[]` → ×2 damage** — Damage roll'da uygulanır.
- [x] [x] [ ] **`damage_immunity_refs[]` → 0 damage** — Damage roll'da uygulanır.
- [x] [x] [ ] **`condition_immunity_refs[]` → condition blocked** — Apply-condition akışında blok.
- [x] [x] [ ] **`senses[]` → typed sense+range** — Darkvision 60ft, Truesight 120ft (T7 kapandı).
- [x] [x] [ ] **`passive_perception` → passive Wis(Perception)** — Stealth karşıtı.
- [x] [x] [ ] **`language_refs[]` → comprehension** — Hangi dilleri anladığı.
- [x] [x] [ ] **`telepathy_ft` → telepathy range** — Mind link mesafesi.
- [x] [x] [ ] **`cr` → XP + difficulty** — Encounter difficulty input (s.202).
- [x] [x] [ ] **`xp` → death award** — Monster öldüğünde party'ye verilen XP.
- [x] [x] [ ] **`proficiency_bonus` → save/skill PB** — CR'den derive (mechanics §17.2).
- [x] [x] [ ] **`trait_refs[]` → passive abilities** — Typed trait listesi (T8 kapandı).
- [x] [x] [ ] **`action_refs[]` → action economy** — Action slot'undaki creature-action listesi.
- [x] [x] [ ] **`bonus_action_refs[]` → BA economy** — BA slot.
- [x] [x] [ ] **`reaction_refs[]` → reaction economy** — Reaction slot.
- [x] [x] [ ] **`legendary_action_uses` + `legendary_action_refs[]` → LA havuzu** — Round başına LA budget reset.
- [x] [x] [ ] **`lair_action_refs[]` → init 20 lair trigger** — Lair'da init 20'de tetiklenir.
- [x] [x] [ ] **`spell_refs[]` → spell list** — Typed spell relation (T4 kapandı); slot havuzu kendi class.spell_slots'ı veya manual.
- [x] [x] [ ] **`gear_refs[]` → loot inventory** — Ölünce drop edilebilen eşyalar.
...
**Açıklama:** Animal Beast filter'ı sadece UI'de ayırmak için Monster'ın delta'sı. Tüm mekanik §2.19'daki gibi çalışır; Druid Wild Shape, Conjure Animals, Ranger Companion gibi feature'larda Beast type'a filtre uygulanır.

**Mekanik kapsamı:**

- [x] [x] [ ] **Tüm field davranışı §2.19 Monster ile özdeş** — Animal-spesifik kural yok; ayrı slug sadece UI/filter.
- [x] [x] [ ] **Beast type filter eligibility** — Wild Shape / Conjure Animals / Ranger Companion gibi feature'lar Animal slug'ından çekim yapar.
...
**Açıklama:** Player Character tüm Tier-1 mekaniklerinin tüketicisi. species_ref + class_refs + class_levels + background_ref kaynak alanlardır; tüm derived stat'lar (PB, ability_mod, AC, save bonusları, skill bonusları, spell DC, spell_slots) bunlardan türer. combat_stats ve durum alanları (death_saves, conditions, temp_hp) live state taşır.

**Açık konular:** §5 #2 (proficiency_bonus derived), #3 (multi-DC), #8 (AC formula), #13 (class_levels typing).

**Mekanik kapsamı:**

- [x] [x] [ ] **`species_ref` → species derived'lar** — size, speed, granted_modifiers, creature_type kaynağı.
- [x] [x] [ ] **`class_refs[]` + `class_levels` → class derived'lar** — total_level, PB, all class field consumption.
- [x] [x] [ ] **`subclass_refs[]` → subclass features** — Parent class level eşeleşmeli; class_resources'a katkı.
- [x] [x] [ ] **`background_ref` → background grants** — Skill+tool, origin feat, gold, equipment kaynağı.
- [x] [x] [ ] **`alignment_ref` → lore + magic item gating** — Holy Avenger gibi alignment-locked attune'lerde gate.
- [x] [x] [ ] **`xp` → level türevi** — xpToLevel(xp) per s.23 → total_level.
- [x] [x] [ ] **`proficiency_bonus` → roll bonusları** — Şu an manuel; derive olmalı (§5 #2).
- [x] [x] [ ] **`feats[]` → feat effects** — ASI L4/8/12/16 + origin feat union.
- [x] [x] [ ] **`languages[]` → comprehension** — Common + species + background + class union.
- [x] [x] [ ] **`tool_proficiencies[]` → tool check bonus** — `ability_mod + (PB if in list)`.
- [x] [x] [ ] **`weapon_proficiencies[]` → attack bonus** — Attack roll'a PB ekleme.
- [x] [x] [ ] **`armor_trainings[]` → AC + spell gate** — Untrained Heavy → disadv Str/Dex tests + no spells.
- [x] [x] [ ] **`saving_throw_proficiencies[]` → save bonus** — `ability_mod + (PB if proficient)`.
- [x] [x] [ ] **`skill_proficiencies[]` + `skill_expertises[]` → skill bonus** — Expertise PB×2 (s.182).
- [x] [x] [ ] **`stat_block` → ability mods** — `floor((score-10)/2)`, cap 20 (30 max).
- [x] [x] [ ] **`combat_stats.hp` → live HP** — 0 → Unconscious + Death Saves.
- [x] [x] [ ] **`combat_stats.max_hp` → HP cap** — Class HD + CON formülü, Tough feat ekler.
- [x] [x] [ ] **`combat_stats.ac` → AC** — `armor.base_ac + DEX(cap'li) + shield + misc` (§5 #8).
- [x] [x] [ ] **`combat_stats.speed` → walk speed** — Species + class bonusları, Heavy STR penalty.
- [x] [x] [ ] **`combat_stats.initiative` → init roll** — `d20 + DEX_mod + PB-if-Alert/Feral`.
- [x] [x] [ ] **`combat_stats.level` → total level** — Σ class_levels.values.
- [x] [x] [ ] **`temp_hp` → buffer** — Damage'da önce düşer; LR ends; replace no-stack.
- [x] [x] [ ] **`death_saves_successes` / `death_saves_failures` → death save tracker** — 3 success → Stable; 3 fail → die; nat 20 → 1 HP; nat 1 → +2 fail.
- [x] [x] [ ] **`heroic_inspiration` → reroll** — Bool; spend → reroll d20 (max 1 active).
- [x] [x] [ ] **`hit_dice_remaining` → SR healing pool** — SR'da 1 spend → `d_size + CON` heal; LR'da +½ refresh.
- [x] [x] [ ] **`saving_throws` table → UI rendering** — Bonus formülü save-roll'da kullanılır.
- [x] [x] [ ] **`skills` table → UI rendering** — Skill bonus formülü check'te.
- [x] [x] [ ] **`senses[]` → special sight** — Darkvision/Truesight vb. union; species + magic item.
- [x] [x] [ ] **`passive_perception` → 10 + Wis(Perception) mod** — Stealth karşıtı; +5 adv, −5 disadv.
- [x] [x] [ ] **`passive_insight` / `passive_investigation` → 10 + ability(skill) mod** — Pasif check'ler.
- [x] [x] [ ] **`inventory[]` → eşya listesi** — weapon/armor/gear/magic-item.
- [x] [x] [ ] **`attuned_items[]` → attuned bonuses** — Max 3 attune; granted_modifiers aktif olur.
- [x] [x] [ ] **`cp/sp/ep/gp/pp` → currency** — Coin.value_in_gp ile convert.
- [x] [x] [ ] **`current_conditions[]` → active conditions** — Exhaustion stack; diğerleri non-stack.
- [x] [x] [ ] **`casting_ability_ref` → spell DC + atk** — Per-class; multiclass'ta birden fazla DC (§5 #3).
- [x] [x] [ ] **`spell_save_dc` → enemy save DC** — `8 + PB + ability_mod(casting_ability_ref)`.
- [x] [x] [ ] **`spell_attack_bonus` → spell attack roll** — `PB + ability_mod`.
- [x] [x] [ ] **`spells_known[]` → spell pool** — Class.cantrips_known + spellbook (Wizard).
- [x] [x] [ ] **`prepared_spells[]` → castable list** — LR'da yeniden seçim; cap class.prepared_spells_by_level.
- [x] [x] [ ] **`spell_slots` → castable resource** — Combined-multiclass-table (s.26).
- [x] [x] [ ] **`pact_magic_slots` → Warlock slot pool** — SR'da yenilenir; ayrı havuz (Pact ≠ regular).
- [x] [x] [ ] **`class_resources` → counter pool** — Rage/Sneak Attack/Bardic Insp/Action Surge vb. (§5 #1).
- [x] [x] [ ] **`trinket_ref` → flavor only** — Mekanik etki yok.
- [x] [x] [ ] **`personality_traits/ideals/bonds/flaws/age/height/weight/eyes/skin/hair/appearance/backstory/allies_organizations` → UI/roleplay** — Tüm narratif alanlar; mekanik etki yok.
...
**Açıklama:** NPC PC ile benzer şekilde combat-capable ama campaign çevresinde yer alır. attitude_ref Friendly/Indifferent/Hostile sosyal etkileşim DC'sini etkiler; goals/mannerisms/secrets DM-side narrative.

**Mekanik kapsamı:**

- [x] [x] [ ] **`species_ref` / `class_refs[]` / `background_ref` → derived'lar** — PC ile aynı kaynak alanları.
- [x] [x] [ ] **`level` → PB (single-class)** — NPC multiclass desteklenmez; tek int.
- [x] [x] [ ] **`alignment_ref` → lore** — Mekanik etki yok.
- [x] [x] [ ] **`attitude_ref` → social DC** — Friendly/Indifferent/Hostile Influence action DC'sini etkiler (s.10, 184).
- [x] [x] [ ] **`location_ref` → world-map source** — UI/lore.
- [x] [x] [ ] **`faction` → free-form** — Lore tag.
- [x] [x] [ ] **`stat_block` + `combat_stats` + `proficiency_bonus` + `initiative_modifier` → combat numerics** — PC ile aynı semantik.
- [x] [x] [ ] **`saving_throws` / `skills` table → bonus rendering** — PC'deki gibi.
- [x] [x] [ ] **`resistance_refs[]` / `vulnerability_refs[]` / `damage_immunity_refs[]` / `condition_immunity_refs[]` → defenses** — Damage roll + apply-condition gating.
- [x] [x] [ ] **`senses[]` + `passive_perception` + `language_refs[]` + `telepathy_ft` → algı** — Vision/comprehension.
- [x] [x] [ ] **`trait_refs[]` → passive abilities** — NPC pariteyle Monster gibi.
- [x] [x] [ ] **`action_refs[]` / `special_action_refs[]` → action havuzu** — Action + LA/Lair/Mythic.
- [x] [x] [ ] **`equipment_refs[]` → inventory** — Stat block loot.
- [x] [x] [ ] **`spell_refs[]` → spellbook** — Typed spell relation.
- [x] [x] [ ] **`goals` / `appearance` / `mannerisms` / `secrets` markdown** — Narrative; mekanik etki yok.
...
**Açıklama:** Applied Condition bir Condition lookup'ının runtime instance'ı. Hedefin `current_conditions[]` listesine girer; süresi turn-end'de tick eder, save_frequency'ye göre yeniden save'lenir.

**Mekanik kapsamı:**

- [x] [x] [ ] **`condition_ref` → host conditions[]** — Union (Exhaustion stack, diğerleri non-stack).
- [x] [x] [ ] **`source_entity_ref` → causality** — Hangi entity uyguladı (DM tracking).
- [x] [x] [ ] **`duration_rounds` → expiry tick** — null = indefinite; "1 minute / dawn" duration round'a sığmaz (§5 #11).
- [x] [x] [ ] **`save_dc` + `save_ability_ref` → save attempt** — End-of-turn veya trigger save.
- [x] [x] [ ] **`save_frequency` → save trigger window** — none / start-of-turn / end-of-turn / when-damaged.
- [x] [x] [ ] **`notes` → DM tracking** — Free-form not.
...
**Açıklama:** Location coğrafi/kavramsal birim. illumination_ref + hazard_refs aktif sahnede mekanik etki doğurur (vision, hazard exposure). description_long player-visible, secrets DM-only.

**Mekanik kapsamı:**

- [x] [x] [ ] **`danger_level` → encounter pacing** — Safe/Low/Medium/High/Deadly UI helper.
- [x] [x] [ ] **`environment` → tag** — "Forest", "Dungeon" gibi free-form.
- [x] [x] [ ] **`parent_location_ref` → hierarchy** — Nested location ağacı.
- [x] [x] [ ] **`plane_ref` → lore** — Material/Astral/Ethereal vb.
- [x] [x] [ ] **`illumination_ref` → vision modifier** — Bright/Dim/Darkness sight check etkisi (s.11).
- [x] [x] [ ] **`hazard_refs[]` → hazard exposure** — Burning/Falling/Suffocation vb. union (s.12).
- [x] [x] [ ] **`description_long` markdown → player-visible** — UI display.
- [x] [x] [ ] **`secrets` markdown → DM-only** — Online modda gizlenir.
...
**Açıklama:** Scene bir Location içinde geçen narrative segment. illumination + travel_pace aktif olduğunda mekanik etkiler (vision, hareket çarpanı). NPC ve Quest ref'leri sadece UI bağlantısı.

**Mekanik kapsamı:**

- [x] [x] [ ] **`location_ref` → context** — Sahnenin geçtiği yer.
- [x] [x] [ ] **`status` → tracking** — Planned/Active/Completed/Skipped.
- [x] [x] [ ] **`illumination_ref` → vision modifier** — Bright/Dim/Darkness.
- [x] [x] [ ] **`travel_pace_ref` → speed multiplier** — Fast/Normal/Slow.
- [x] [x] [ ] **`beats` markdown → outline** — DM plan notları.
- [x] [x] [ ] **`npc_refs[]` → cast** — Sahnedeki NPC bağlantısı.
- [x] [x] [ ] **`quest_refs[]` → tied quest** — Sahne içinde aktif quest'ler.
...
**Açıklama:** Quest tamamlandığında reward_item_refs + reward_xp + reward_gp typed alanlardan otomatik grant; reward markdown narrative kompleksiteyi anlatır. status quest tracker; secrets DM-only.

**Mekanik kapsamı:**

- [x] [x] [ ] **`status` → tracking** — Not Started/Active/Completed/Failed.
- [x] [x] [ ] **`giver_ref` → quest NPC** — UI link.
- [x] [x] [ ] **`reward_item_refs[]` → `inventory[]`** — Typed loot auto-grant on complete.
- [x] [x] [ ] **`reward_xp` → `PlayerCharacter.xp`** — XP sum on complete.
- [x] [x] [ ] **`reward_gp` → `PlayerCharacter.gp`** — Gold sum on complete.
- [x] [x] [ ] **`reward` markdown → narrative** — Typed alanlar baskın.
- [x] [x] [ ] **`objective` markdown → player-visible** — Görev özeti.
- [x] [x] [ ] **`secrets` markdown → DM-only** — Twist/secret motivation.
...
**Açıklama:** Encounter combat / hazard / trap event'i. monsters_refs + npcs_refs spawn list; environmental_effect_refs + trap_refs aktif tetikleyiciler. xp_budget Σ monster.xp ile hesaplanır, difficulty enum'u s.202 XP budget tablosu üstünden doğrulanır.

**Mekanik kapsamı:**

- [x] [x] [ ] **`location_ref` → context** — Encounter geçtiği yer.
- [x] [x] [ ] **`difficulty` → XP budget validate** — Trivial/Low/Moderate/High/Deadly s.202 tablosu.
- [x] [x] [ ] **`monsters_refs[]` + `npcs_refs[]` → combat tracker** — Spawn listesi.
- [x] [x] [ ] **`environmental_effect_refs[]` → start-of-encounter trigger** — Per-encounter aktif environment.
- [x] [x] [ ] **`trap_refs[]` → trigger** — Encounter içindeki trap'lar.
- [x] [x] [ ] **`setup` markdown → DM notes** — Sahne hazırlık.
- [x] [x] [ ] **`tactics` markdown → DM-only** — Düşman taktikleri (gizli).
- [x] [x] [ ] **`xp_budget` → difficulty doğrulaması** — Σ monster.xp hesabı.
...
**Açıklama:** Trap entity bestiary-style trap tarifi (vs SRD §13.11'deki sample trap'lar). trigger_kind typed enum + save_dc/save_ability_ref + damage_dice typed; detection_dc + disable_dc + disable_ability_ref ile detect/disarm gating.

**Mekanik kapsamı:**

- [x] [x] [ ] **`trigger_kind` → trigger machinery** — Pressure/Tripwire/Proximity/Sound/Magical/Touch/Other.
- [x] [x] [ ] **`trigger` markdown** — Narrative; typed kind baskın.
- [x] [x] [ ] **`save_dc` + `save_ability_ref` → save attempt** — Tipik DEX (Reflex) save.
- [x] [x] [ ] **`damage_dice` + `damage_type_ref` → damage** — Typed dice (T1 kapandı).
- [x] [x] [ ] **`applied_condition_refs[]` → save-fail conditions** — Restrained, Poisoned vb.
- [x] [x] [ ] **`detection_dc` → Perception threshold** — Search action ile DC kontrolü.
- [x] [x] [ ] **`disable_dc` + `disable_ability_ref` → disarm threshold** — Genelde DEX (Thieves' Tools) veya INT (Investigation).
- [x] [x] [ ] **`countermeasures` markdown → DM helper** — Iron Spike vs gibi alternatif çözümler.
...
**Açıklama:** Poison delivery vector'a göre uygulanır (Contact/Ingested/Inhaled/Injury). Save_dc + ability_ref hedefin save'ini belirler; başarısız save → damage + condition (genelde Poisoned). Drow Poison 3d6, Wyvern Poison 7d6 gibi typed dice değerleri.

**Mekanik kapsamı:**

- [x] [x] [ ] **`poison_kind` → delivery flow** — Contact/Ingested/Inhaled/Injury (s.197).
- [x] [x] [ ] **`save_dc` + `save_ability_ref` → save attempt** — Tipik CON save.
- [x] [x] [ ] **`damage_dice` + `damage_type_ref` → damage** — Genelde Poison damage type.
- [x] [x] [ ] **`applied_condition_refs[]` → save-fail conditions** — Poisoned, Unconscious vb.
- [x] [x] [ ] **`duration_rounds` → effect süresi** — Save-fail'in geçerli kalma süresi.
- [x] [x] [ ] **`effect` markdown** — Narrative; typed alanlar baskın.
- [x] [x] [ ] **`cost_gp` → buy/sell** — Standart ekonomi.
...
**Açıklama:** Curse hedefe kalıcı condition uygular; removed_by_spell_refs ile typed çare (Remove Curse, Greater Restoration, Wish vb.) listelenir. trigger + effect + removed_by markdown narrative açıklamalar.

**Mekanik kapsamı:**

- [x] [x] [ ] **`trigger` markdown → narrative** — Lanetin nasıl tetiklendiği.
- [x] [x] [ ] **`applied_condition_refs[]` → host conditions[]** — Typed condition grant.
- [x] [x] [ ] **`granted_modifiers[]` → ongoing PC stats** — Numeric ongoing penalties (yeni eklenmiş typed alan).
- [x] [x] [ ] **`removed_by_spell_refs[]` → cure gate** — Bu spell'lerden biri cast edilirse curse kalkar.
- [x] [x] [ ] **`effect` markdown → narrative** — Typed alanlar baskın.
- [x] [x] [ ] **`removed_by` markdown → narrative** — Cure açıklaması; typed list baskın.
...
**Açıklama:** Environmental Effect (Heat, Cold, Falling, Drowning vb.) encounter veya scene boyunca save-fail'lerde hedefe damage + condition uygular. Encounter.environmental_effect_refs'ten bağlanır.

**Mekanik kapsamı:**

- [x] [x] [ ] **`damage_dice` + `damage_type_ref` → damage** — Heat/Cold/Fall vb. typed dice.
- [x] [x] [ ] **`applied_condition_refs[]` → save-fail conditions** — Exhaustion, Frightened gibi.
- [x] [x] [ ] **`granted_modifiers[]` → ongoing PC stats** — Sürekli mekanik etki (typed yeni alan).
- [x] [x] [ ] **`save_dc` + `save_ability_ref` → save attempt** — Uygun ability save (genelde CON/STR/DEX).
- [x] [x] [ ] **`effect` markdown → narrative** — Typed alanlar baskın.
...
**Açıklama:** Hireling günlük cp ücretiyle PC'ye eşlik eder (skill_ref = uzmanlık alanı). Skilled flag SRD s.102'ye göre ≥2 sp/day grubunu işaretler.

**Mekanik kapsamı:**

- [x] [x] [ ] **`skill_ref` → uzmanlık** — UI: hireling specialty.
- [x] [x] [ ] **`daily_cost_cp` → günlük gp drain** — Hire/day → currency düşer.
- [x] [x] [ ] **`skilled` → kategori flag** — Skilled (≥2 sp/day) / Untrained (~2 sp).
...
**Açıklama:** Service hizmet satın alımı (Spellcasting, Transport, Shelter, Other). cost_cp tek-seferlik harcamayı tetikler (örn Cure Wounds spellcasting service ~10gp).

**Mekanik kapsamı:**

- [x] [x] [ ] **`kind` → grouping** — Spellcasting/Transport/Shelter/Other.
- [x] [x] [ ] **`cost_cp` → currency drain** — Use action'da PC.cp..pp düşer.
- [x] [x] [ ] **`availability` → UI hint** — "Cities only" gibi şart metni.
...
**Aşama checkbox'ları (her ok = bir resolver adımı):**

- [x] [x] [ ] **Adım 1: Class assignment kaynak akışı** — class_refs → hit_die/save/skill/weapon/armor/tool/equipment/gold/casting_ability/feature_table[1] tüm propagasyonları.
- [x] [x] [ ] **Adım 2: Origin (background + species) akışı** — Background skills/tools/ASI/feat/gold + Species size/speed/languages/senses/resistances/skills/traits.
- [x] [x] [ ] **Adım 3: Ability scores akışı** — Standard/Random/Point Buy + ASI'den stat_block + ability_mod ile saving_throws/skills/AC/init/HP/spell_DC propagasyonu.
- [x] [x] [ ] **Adım 4: Equipment + inventory akışı** — Class.starting_equipment + Background.default_inventory PC.inventory'ye union; Armor.equip → AC formula tetikleyicisi.
- [x] [x] [ ] **Adım 5: Final detaylar** — total_level=1 → PB=+2; combat_stats.hp = max_hp; passive_perception; spell_save_dc/atk (caster); spell_slots/cantrips_known.
...
- [x] [x] [ ] **xp → level threshold check** — xpToLevel(xp) > current_level olunca level-up tetikle.
- [x] [x] [ ] **class_levels[<chosen>] += 1 + total_level recompute** — Class seçimi, total_level Σ, PB güncelleme.
- [x] [x] [ ] **PB propagation** — saving_throws/skills/attack/spell DC rendering yenilenir.
- [x] [x] [ ] **HP artışı (rolled veya fixed)** — hit_dice_remaining +1; max_hp += avg(d_size)+CON.
- [x] [x] [ ] **Class.feature_table[N] aktivasyonu** — Yeni level satırındaki feature/resource'lar class_resources'a eklenir.
- [x] [x] [ ] **Subclass selection trigger** — Class.feature_table[N]'da subclass row varsa subclass_refs seçimi açılır.
- [x] [x] [ ] **ASI / Feat seçimi** — Level-up satırında ASI varsa stat_block veya feats[] güncellemesi.
- [x] [x] [ ] **Subclass.features aktivasyonu** — Subclass.granted_at_level ≤ N ise subclass features class_resources'a eklenir.
- [x] [x] [ ] **Long-rest spell_slots/pact/HD/exhaustion/prepared/temp_hp refresh** — Tüm post-rest tetikleyicileri.
...
- [x] [x] [ ] **Slot gate** — Spell.level vs PC.spell_slots[≥]; cantrip slot harcamaz; ritual path opsiyonel.
- [x] [x] [ ] **Action economy** — casting_time_amount + casting_time_unit_ref → Action/BA/Reaction/Min/Hour slot tüketimi; Reaction → reaction_trigger gerek.
- [x] [x] [ ] **V/S/M satisfy** — V (silenced değil), S (free hand), M (focus/pouch/specific item); material_consumed → gp düş.
- [x] [x] [ ] **Targeting** — range_type + range_ft + area_shape_ref + area_size_ft AoE coverage hesabı.
- [x] [x] [ ] **Attack roll branch** — attack_type Melee/Ranged → spell attack roll = `PB + spell_mod + bonuses`.
- [x] [x] [ ] **Save roll branch** — save_ability_ref → target.save_bonus vs PC.spell_save_dc; success → save_effect uygula.
- [x] [x] [ ] **Damage resistance check** — damage_type_refs vs target.resistance/vulnerability/immunity; ½ / ×2 / 0.
- [x] [x] [ ] **Concentration management** — requires_concentration=true → mevcut concentration drop; damage'da DC 10 ya da ½ dmg save.
- [x] [x] [ ] **Active effect tracker** — duration_unit_ref + duration_amount → AppliedCondition veya active-effect entry duration_rounds takibi.
...
**Açık mekanik checkbox'ları** (planned/supported/implemented — kapsam takibi):

- [x] [x] [ ] **#1 ✅ `Class.features` / `Subclass.features` `classFeatures` typed list** — Rage uses, Sneak Attack dice, Bardic Inspiration die, Extra Attack count typed encode. (Önceden levelTable Map<int,int>.)
- [x] [x] [ ] **#2 `PlayerCharacter.proficiency_bonus` manuel int** — `total_level`'dan derive edilmeli. Multiclass §1.8 zorunluluğu (PB total character level'a göre). Şu an cache; level-up'ta güncellenmesi convention.
- [x] [x] [ ] **#3 `PlayerCharacter.spell_save_dc` / `spell_attack_bonus` tek skaler** — multiclass'ta iki caster için iki ayrı DC olur (Cleric WIS, Wizard INT). Schema tek tutuyor. Deferred: per-class derived map.
- [x] [x] [ ] **#4 `Class.spell_slots_by_level` per-class ama multiclass slot havuzu combined table gerektirir** (s.26). Cached field yok; resolver runtime'da Σ over caster classes hesaplamalı. Pact slots `pact_magic_slots`'ta ayrı.
- [x] [x] [ ] **#5 `Class.saving_throw_refs` first-class-only kuralı** — SRD-mandated (s.25) ama şemada kodlu değil. "Yalnız PC'nin starting class'ı save prof verir" kuralı convention; resolver bunu zorlamalı.
- [x] [x] [ ] **#6 ✅ `Species.traits`, `Feat.benefits`, `MagicItem.effects`, `Trait.description` markdown blob** — typed `granted_modifiers[]` DSL ile çözüldü (Tier-B). 24 modifier kind tüm bonus tipi taşır; markdown narrative kalır.
- [x] [x] [ ] **#7 Multiclass starting proficiency tablosu (s.25)** — `Class.multiclass_requirements` markdown'da; data değil. "Barb multiclass'a girince sadece Shields + Martial weapon training" subset kuralı kodlanamaz.
- [x] [x] [ ] **#8 `PlayerCharacter.combat_stats.ac` serbest int** — armor formülü kodlu değil (`base_ac + DEX [+cap] + shield + misc`). Unarmored Defense (Barb=10+DEX+CON, Monk=10+DEX+WIS) seçim mekanizması yok; non-stack multiclass kuralı advisory.
- [x] [x] [ ] **#9 `Spell.attack_type` enum {None,Melee,Ranged}** vs **`creature-action.attack_kind` enum {Melee Weapon, Ranged Weapon, Melee Spell, Ranged Spell}** — iki vokabüler. Reconcile gerek veya mapping function.
- [x] [x] [ ] **#10 `MagicItem.charges_max + charge_regain (text)`** — regain free-form ("1d6+4 at dawn"). Long-rest engine auto-restore edemez; parser yazılana kadar manuel.
- [x] [x] [ ] **#11 `AppliedCondition.duration_rounds int (null=indefinite)`** — Exhaustion stack ve "1 minute / until next dawn / until cured" duration'lar lossy. Round-based tek granularity; minute/hour/day → round (1 min = 10 rounds).
- [x] [x] [ ] **#12 `Background.ability_score_options` sadece ability listesi** — "+2/+1 vs +1/+1/+1" choice (s.83) kodlu değil; dağıtım PC.stat_block'ta serbest. UI seçim sunmalı + max-20 cap zorlamalı.
- [x] [x] [ ] **#13 `PlayerCharacter.class_levels` `levelTable` (Map<int,int>)** — implicit key class_id (UUID), value level. "level" değil. Type aliasing convention; FieldType naming yanıltıcı.
- [x] [x] [ ] **#14 `Animal` slug Monster duplicate** — şema seviyesinde `_animalCategory()` Monster field listesini `copyWith` ile rebuild eder. Field semantikleri özdeş; UI/import/codec ayrı slug filter için ayrı.
...
**Schema değişikliği uygulananlar — Tier-B (granted_modifiers DSL):**
- ✅ **Yeni FieldType:** `grantedModifiers` — typed bonus DSL (24 kind, §0.6 spec).
- ✅ **Species.granted_modifiers** — Lucky/Brave/Darkvision/Speed/Resistance grants typed.
- ✅ **Feat.granted_modifiers** — Alert/Tough/Mobile/Observant numeric bonuses typed.
- ✅ **MagicItem.granted_modifiers** — Cloak of Protection AC/save, Bracers of Archery attack bonus typed.
- ✅ **Trait.granted_modifiers** — race-shared trait shape; NPC/Monster trait_refs üstünden tüketilir.
