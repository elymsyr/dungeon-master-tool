...
6. ✅ **`Species.traits`, `Feat.benefits`, `MagicItem.effects`, `Trait.description` markdown blob** — typed `granted_modifiers[]` DSL ile çözüldü (Tier-B). 24 modifier kind: ability_score_bonus, ac_bonus, save_bonus, skill_bonus, attack_bonus, damage_bonus, speed_bonus, hp_bonus_*, initiative_bonus, passive_score_bonus, resistance/immunity/vulnerability_grant, condition_save_advantage/immunity, attack_advantage_vs, proficiency/expertise_grant, language/sense/spell_known/spell_at_will/feat_grant, feature_text fallback. Markdown narrative kalır.
