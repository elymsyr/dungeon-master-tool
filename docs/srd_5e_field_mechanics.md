...
- [ ] [ ] [ ] **`size.space_ft` → creature space** — Tiny=2.5, S/M=5, Large=10, Huge=15, Gargantuan=20.
- [ ] [ ] [ ] **`size.hit_die_size` → monster default HD** — Class HD'si yoksa fallback (s.22).
- [ ] [ ] [ ] **`size.carrying_multiplier` → carry capacity** — `STR × 15 × multiplier` (s.16).
- [ ] [ ] [ ] **`sense.default_range_ft` → rangedSense fallback** — Darkvision 60, Truesight 120 vb. consumer override edebilir.
- [ ] [ ] [ ] **`damage-type.bypassable_by_magical` → resistance bypass** — Magical/silvered/adamantine nonmagical resistance'ı bypass eder (sadece B/P/S için true).
- [ ] [ ] [ ] **`rarity.value_gp` → fiyat formülü** — Common 100/Uncommon 400/Rare 4000/VR 40000/Legendary 200000; consumable ½.
- [ ] [ ] [ ] **`rarity.crafting_time_days` → downtime tracker** — Crafting süreci günleri.
- [ ] [ ] [ ] **`rarity.crafting_cost_gp` → gp drain** — Crafting maliyeti.
- [ ] [ ] [ ] **`coin.value_in_gp` → currency conversion** — CP=0.01, SP=0.1, EP=0.5, GP=1, PP=10; 50 coins = 1 lb.
- [ ] [ ] [ ] **`lifestyle.cost_per_day_gp` → günlük gp drain** — Wretched 0, Squalid 0.1, ..., Aristocratic ≥10.
- [ ] [ ] [ ] **`condition.stacks` → stack rule** — Sadece Exhaustion true; diğerleri non-stack.
- [ ] [ ] [ ] **`condition.grants_incapacitated` → incapacitated chain** — Stunned/Paralyzed/Petrified/Unconscious true → no actions + concentration drop.
- [ ] [ ] [ ] **`damage-type.is_physical` → B/P/S classification** — Magic weapon resistance bypass için kullanılır.
- [ ] [ ] [ ] **`duration-unit.is_concentration_compatible` → concentration check** — Rounds/Minutes/Hours true; Instantaneous false.
- [ ] [ ] [ ] **`weapon-property.mechanic_kind` → property dispatch** — 11 kind enum (Finesse/Heavy/Light vs.); resolver typed switch.
- [ ] [ ] [ ] **`weapon-mastery.effect_kind` → mastery effect dispatch** — 8 kind enum (Cleave/Graze/Nick/Push/Sap/Slow/Topple/Vex).
- [ ] [ ] [ ] **`weapon-mastery.effect_value` → mesafe/değer** — Push=10ft, Slow=10ft, Cleave/Graze=0.
- [ ] [ ] [ ] **`weapon-mastery.save_ability` → mastery save** — Topple=CON; diğerleri save'siz.

**§1.1 Identifier-only lookups (mekanik etkisi yok, sadece name/icon/description):**

- [ ] [ ] [ ] **36 identifier-only lookup şeması** — ability/skill/damage-type/condition/size/creature-type/alignment/language/weapon-category/property/mastery/armor-category/tool-category/spell-school/magic-item-category/rarity/speed-type/sense/action/area-shape/attitude/cover/illumination/hazard/feat-category/lifestyle/coin/tier-of-play/travel-pace/arcane-focus/druidic-focus/holy-symbol/plane/casting-component/casting-time-unit/duration-unit. Mekanik consumer field'larında uygulanır.
...
- [ ] [ ] [ ] **`primary_ability_ref` → multiclass eligibility** — Yeni sınıf eklenirken hem mevcut hem yeni sınıfın primary'si ≥13 olmalı; PC'a yazılmaz, gate kontrolünde okunur.
- [ ] [ ] [ ] **`hit_die` → `combat_stats.max_hp` & `hit_dice_remaining`** — İlk sınıfta L1 max+CON, sonraki seviyelerde avg+CON eklenir; her seviye HD havuzuna 1 die katar.
- [ ] [ ] [ ] **`saving_throw_refs` → `saving_throw_proficiencies[]`** — Sadece ilk sınıftan akar; multiclass'ta sonraki sınıfın save'leri gelmez.
- [ ] [ ] [ ] **`skill_proficiency_choice_count` + `skill_proficiency_options[]` → skill choice pool** — UI'de N adet kutu açar, kullanıcı seçer; sonuç `skill_proficiencies`'e union.
- [ ] [ ] [ ] **`weapon_proficiency_categories[]` → `weapon_proficiencies[]`** — Multiclass'ta SRD s.25 subset (Barb/Fight/Pal/Ranger martial alır, diğerleri kısıtlı).
- [ ] [ ] [ ] **`weapon_proficiency_specifics[]` → `weapon_proficiencies[]`** — Spesifik silah grant'leri; multiclass'ta tipik gelmez (sınıfa özel).
- [ ] [ ] [ ] **`tool_proficiency_count` + `tool_proficiency_options[]` → `tool_proficiencies[]`** — Skill ile aynı choice mantığı, first-class-only.
- [ ] [ ] [ ] **`armor_training_refs[]` → `armor_trainings[]`** — Multiclass'ta Light+Medium+Shield genelde gelir; Heavy & Martial sınıfa özel. Heavy worn without training → disadv Str/Dex tests, no spells.
- [ ] [ ] [ ] **`default_inventory_refs[]` → `PlayerCharacter.inventory[]`** — İlk sınıf seçildiğinde otomatik kopya, sonraki sınıflarda eklenmez.
- [ ] [ ] [ ] **`starting_gold_dice` → `PlayerCharacter.gp`** — İlk sınıfta zar atılır; background gold ile karşılıklı (biri seçilir).
- [ ] [ ] [ ] **`casting_ability_ref` → `spell_save_dc` / `spell_attack_bonus`** — Formül: DC = `8 + PB + ability_mod`, attack = `PB + ability_mod`. Caster olmayan sınıfta null. Multiclass'ta her caster kendi DC'sini hesaplar (şu an tek skaler — açık mekanik §5 #3).
- [ ] [ ] [ ] **`caster_kind` → spell slot havuzu girişi** — Full=lvl, Half=½lvl, Third=⅓lvl combined-table'a feed eder; Pact ayrı havuz; Ritual slot vermez.
- [ ] [ ] [ ] **`spellcasting_focus_ref` → V/S/M satisfaction flag** — Inventory'de focus item varsa Material component shortcut; UI rehberi.
- [ ] [ ] [ ] **`features` → `class_resources` + level-gated feature aktivasyonu** — Typed feature satırları (level/name/dice/uses/recharge); level-up'ta yeni satırlar aktive olur, Rage/Sneak Attack vb. bu havuzdan beslenir.
- [ ] [ ] [ ] **`cantrips_known_by_level` → `spells_known[type=cantrip]` cap** — Level başına bilinen cantrip sayısı sınırı; her caster sınıf kendi cap'i.
- [ ] [ ] [ ] **`prepared_spells_by_level` → `prepared_spells[]` cap** — Long-rest'te yeniden seçilebilen prepared spell sayısı; multiclass'ta her sınıf ayrı.
- [ ] [ ] [ ] **`spell_slots_by_level` → `spell_slots`** — Level-up + long-rest'te combined-table'a girer; multiclass'ta tüm caster'lar tek slot havuzunda toplanır (Pact ayrı).
- [ ] [ ] [ ] **`multiclass_requirements` → eligibility gate** — Karakter sınıfa girmeden önce kontrol; başarısızsa class atama bloklanır.
- [ ] [ ] [ ] **`primary_ability_ref` advisory + `secondary_ability_ref` advisory** — UI'de "stat dağıtımı için önerilen" rehber; mekanik etki yok.
- [ ] [ ] [ ] **`complexity` advisory** — UI'de zorluk rozeti; mekanik etki yok.
- [ ] [ ] [ ] **`starting_equipment_options` markdown choice prompt** — Narrative "A vs B" karar metni; typed import `default_inventory_refs` üstünden.
- [ ] [ ] [ ] **Class-aware multiclass starting profs subset (s.25)** — Şemada kodlu değil, narrative `multiclass_requirements`'tan parse edilmesi gerek (açık mekanik §5 #7).
...
- [ ] [ ] [ ] **`parent_class_ref` → eligibility gate** — Subclass yalnız parent class içinde geçerli; başka sınıfa atanamaz.
- [ ] [ ] [ ] **`granted_at_level` → aktivasyon seviyesi** — Parent class'ın level_in değeri bu eşiğe ulaştığında subclass features tetiklenir.
- [ ] [ ] [ ] **`features` → `class_resources`** — Typed feature satırları parent class feature'ları gibi level-up'ta sıralı aktive olur.
- [ ] [ ] [ ] **`flavor_description` → UI lore** — Mekanik etki yok, sadece subclass tanıtım metni.
...
- [ ] [ ] [ ] **`size_ref` → space + carry capacity** — Tiny/Small/Medium/Large; carrying = `STR × multiplier(size)`. Çoğu species Medium.
- [ ] [ ] [ ] **`speed_ft` → `combat_stats.speed`** — Walk speed override base; class feature/feat (Fast Movement, Mobile) üstüne ekler.
- [ ] [ ] [ ] **`creature_type_ref` → `PlayerCharacter.creature_type`** — Spell targeting (Hold Person sadece Humanoid), monster ability ref'leri için kullanılır.
- [ ] [ ] [ ] **`granted_modifiers[]` → PC derived stats** — Typed ability_score_bonus, sense_grant, resistance_grant vs. her modifier kind'ı kendi target'ına uygulanır.
- [ ] [ ] [ ] **`traits` → narrative UI** — Markdown; özel kurallar `granted_modifiers` ile kapsanmadıysa sadece açıklayıcı metin kalır.
- [ ] [ ] [ ] **`granted_languages[]` → `languages[]`** — Common otomatik default; bu liste ek diller union eder.
- [ ] [ ] [ ] **`granted_senses[]` → `senses[]`** — Darkvision 60 (most), 120 (Dwarf/Orc), Truesight (rare).
- [ ] [ ] [ ] **`granted_damage_resistances[]` → resistance set** — Damage roll'da tüketilir; Dwarf=Poison, Tiefling=Fire vb.
- [ ] [ ] [ ] **`granted_skill_proficiencies[]` → `skill_proficiencies[]`** — Elf Keen Senses gibi sabit skill grant'leri union edilir.
- [ ] [ ] [ ] **`age` → lore only** — Mekanik etki yok.
...
- [ ] [ ] [ ] **`granted_skill_refs[]` → `skill_proficiencies[]`** — 2 sabit skill, union edilir.
- [ ] [ ] [ ] **`granted_tool_refs[]` → `tool_proficiencies[]`** — Sabit veya choice tool prof grant'ı.
- [ ] [ ] [ ] **`granted_language_count` → language seçim sayısı** — UI choice; SRD 2024'te genelde 0.
- [ ] [ ] [ ] **`ability_score_options[]` + ASI dağılımı → `stat_block`** — +2/+1 veya +1/+1/+1 manuel dağıtım; max 20 cap. Açık mekanik §5 #12.
- [ ] [ ] [ ] **`origin_feat_ref` → `feats[]`** — Background L1 feat (Acolyte=Magic Initiate Cleric, Criminal=Alert vb.) otomatik append.
- [ ] [ ] [ ] **`default_inventory_refs[]` → `inventory[]`** — Equipment paketi auto-import.
- [ ] [ ] [ ] **`starting_equipment` markdown choice** — Narrative "A vs B" seçim metni; typed import default_inventory_refs.
- [ ] [ ] [ ] **`starting_gold_gp` → `gp`** — Default başlangıç altını; class.starting_gold_dice ile karşılıklı.
- [ ] [ ] [ ] **`gold_alternative_gp` → `gp` (replace inventory)** — Equipment paketi yerine altın seçeneği (~50 GP).
...
- [ ] [ ] [ ] **`category_ref` → eligibility gating** — Origin=L1, General=L4+ ASI, Fighting Style=class feature, Epic Boon=L19.
- [ ] [ ] [ ] **`prereq_ability_ref` + `prereq_min_score` → ability gate** — Heavy Armor Master STR≥13 gibi typed kontrol.
- [ ] [ ] [ ] **`prereq_class_refs[]` → class gate** — Sadece bu sınıflara açık (örn. War Caster caster sınıflarda).
- [ ] [ ] [ ] **`prereq_species_refs[]` → species gate** — Species-locked feat'lar (rare).
- [ ] [ ] [ ] **`prereq_min_character_level` → total level gate** — Epic Boons L19+ kontrolü.
- [ ] [ ] [ ] **`prereq_requires_spellcasting` → caster gate** — PC herhangi bir caster_kind ≠ None mı kontrolü (Magic Initiate, Ritual Caster).
- [ ] [ ] [ ] **`prerequisite` markdown** — Narrative açıklama; gating typed alanlardan.
- [ ] [ ] [ ] **`repeatable` + `repeatable_limit` → take counter** — Aynı feat'ı birden fazla alma izni; ASI/Skilled/Magic Initiate variant'lar.
- [ ] [ ] [ ] **`asi_ability_options[]` + `asi_amount` + `asi_max_score` → `stat_block` artışı** — Typed ASI uygulaması; cap'i aşmaz.
- [ ] [ ] [ ] **`ability_score_increase` markdown** — Narrative ASI metni; typed alanlar `asi_*`.
- [ ] [ ] [ ] **`granted_modifiers[]` → PC derived stats** — Alert init+PB, Tough +HP/level, Lucky reroll, Mobile +10 speed, Observant +5 passive vs.
- [ ] [ ] [ ] **`benefits` markdown** — Narrative; typed kapsama dışı kuralları açıklar.
...
- [ ] [ ] [ ] **`level` → required slot** — Cantrip (0) slot harcamaz; L1+ slot gerektirir, upcast slot ≥ level kontrolü.
- [ ] [ ] [ ] **`school_ref` → enum tag** — Counterspell/Detect Magic gibi school-bazlı interaction'lar için.
- [ ] [ ] [ ] **`casting_time_amount` + `casting_time_unit_ref` → action economy** — Action/BA/Reaction/dakika/saat.
- [ ] [ ] [ ] **`reaction_trigger` → reaction trigger string** — Reaction casting şart metni (Shield "when hit" vb.).
- [ ] [ ] [ ] **`is_ritual` → ritual cast option** — Slot harcamadan +10 dakika ile cast; Ritual Adept prepare etmeden mümkün kılar.
- [ ] [ ] [ ] **`range_type` + `range_ft` → targeting** — Self/Touch/Ranged/Sight/Unlimited; Ranged ise feet sayısı.
- [ ] [ ] [ ] **`area_shape_ref` + `area_size_ft` → AoE coverage** — Cone/Cube/Cylinder/Emanation/Line/Sphere; size feet.
- [ ] [ ] [ ] **`components[]` → V/S/M satisfy** — V=verbal, S=somatic, M=material; focus item M'i karşılar.
- [ ] [ ] [ ] **`material_description` + `material_cost_gp` + `material_consumed` → M cost** — Consumed=true ise gp düşer; örn Find Familiar 10gp consumed, Identify 100gp not.
- [ ] [ ] [ ] **`duration_unit_ref` + `duration_amount` → ends-at** — Instantaneous/Round/Minute/Hour/Day/Until Dispelled.
- [ ] [ ] [ ] **`requires_concentration` → `concentration` slot** — Tek concentration; yenisi eski'yi düşürür (s.179).
- [ ] [ ] [ ] **`description` markdown** — Narrative; mekanik etki `effects`'ten.
- [ ] [ ] [ ] **`effects[]` typed DSL → resolution** — Her satır kind (damage/heal/condition/buff/debuff) + dice + save_ability + save_effect (none/half/negate/partial); cast sırasında satır satır resolve.
- [ ] [ ] [ ] **`at_higher_levels_text` → upcast scaling** — Slot > base level'da ek dice; numeric scaling effects[].scaling_dice'da.
- [ ] [ ] [ ] **`class_refs[]` → spell list belonging** — PC.spells_known'a ekleme gating; multiclass'ta her sınıf kendi listesine bakar.
- [ ] [ ] [ ] **`damage_type_refs[]` → resistance check** — Damage roll'da hedefin resistance/vulnerability/immunity setiyle karşılaştırılır.
- [ ] [ ] [ ] **`save_ability_ref` → target save** — Save DC = caster'ın spell_save_dc'si; başarılı save → save_effect uygulanır.
- [ ] [ ] [ ] **`attack_type` → attack roll variant** — None/Melee/Ranged spell attack; vokabüler farkı creature-action ile (§5 #9).
- [ ] [ ] [ ] **`applied_condition_refs[]` → target conditions[]** — Spell-level condition apply (effects[] içindeki ile birlikte).
...
- [ ] [ ] [ ] **`category_ref` → prof check** — Simple/Martial; PC.weapon_proficiencies içeriyorsa attack roll'a PB eklenir.
- [ ] [ ] [ ] **`is_melee` → ability default** — STR (melee) / DEX (ranged); Finesse property override eder.
- [ ] [ ] [ ] **`damage_dice` → damage roll** — Roll + ability_mod; crit'te dice 2x.
- [ ] [ ] [ ] **`damage_type_ref` → resistance check** — Bludg/Pierc/Slash + magical type'lar.
- [ ] [ ] [ ] **`property_refs[]` → behavior modifiers** — Her property mechanic_kind enum'dan dispatch (Finesse/Heavy/Light/Two-Handed/Versatile/Reach/Thrown/Loading/Ammunition).
- [ ] [ ] [ ] **`mastery_ref` → mastery effect** — Cleave/Graze/Nick/Push/Sap/Slow/Topple/Vex; aktif mastery slotuna bağlı non-stack.
- [ ] [ ] [ ] **`normal_range_ft` + `long_range_ft` → range gating** — Beyond normal disadv, beyond long auto miss.
- [ ] [ ] [ ] **`versatile_damage_dice` → 2H grip damage** — Versatile property gerekli; iki elle tutulduğunda alternate dice.
- [ ] [ ] [ ] **`ammunition_type_ref` → ammo consumption** — Loading: 1 ammo per Action/BA/Reaction; recover ½ after fight.
- [ ] [ ] [ ] **`cost_gp` → buy/sell** — Sell ½ default.
- [ ] [ ] [ ] **`weight_lb` → carry total** — Encumbrance toplamına ekler.
...
- [ ] [ ] [ ] **`category_ref` → training check** — armor_trainings içermiyorsa disadv all Str/Dex tests + no spell.
- [ ] [ ] [ ] **`base_ac` + `adds_dex` + `dex_cap` → `combat_stats.ac`** — Formula: `base + (DEX cap'lı) + shield + misc`.
- [ ] [ ] [ ] **`strength_requirement` → speed penalty** — STR<req → speed −10.
- [ ] [ ] [ ] **`stealth_disadvantage` → stealth check** — DEX(Stealth) check'lerine disadv.
- [ ] [ ] [ ] **`don_time_minutes` / `doff_time_minutes` → time cost** — Light 1m, Medium 5m, Heavy 10m, Shield 1 action.
- [ ] [ ] [ ] **`cost_gp` → buy/sell** — Sell ½ default.
- [ ] [ ] [ ] **`weight_lb` → carry total** — Encumbrance ekleme.
...
- [ ] [ ] [ ] **`category_ref` → grouping** — Artisan/Gaming/Musical/Other.
- [ ] [ ] [ ] **`variant_of_ref` → variant link** — Sub-tool grupları için bağlantı.
- [ ] [ ] [ ] **`ability_ref` → check ability** — Tool kullanımında default ability rolü (örn Thieves' Tools = DEX).
- [ ] [ ] [ ] **`utilize_check_dc` → default DC** — Utilize action default zorluk.
- [ ] [ ] [ ] **`utilize_description` → usage hint** — UI rehber metni.
- [ ] [ ] [ ] **`craftable_items[]` → crafting input** — Bu tool ile yapılabilen eşya listesi (s.103).
- [ ] [ ] [ ] **`cost_gp` / `weight_lb` → ekonomi/carry** — Standart eşya alanları.
...
- [ ] [ ] [ ] **`cost_cp` → currency conversion** — CP precision; coin lookup üstünden cp/sp/ep/gp/pp arasına dönüştürülür.
- [ ] [ ] [ ] **`weight_lb` → carry total** — Encumbrance.
- [ ] [ ] [ ] **`utilize_check_dc` + `utilize_ability_ref` → ability check** — Utilize action'da DC + ability rolü.
- [ ] [ ] [ ] **`utilize_description` → narrative effect** — Markdown; mekanik gating typed alanlar üstünden.
- [ ] [ ] [ ] **`consumable` → inventory dec** — Use action'da envanterden 1 düşer (potion, holy water vb.).
- [ ] [ ] [ ] **`is_focus` + `focus_kind_ref` → V/S/M satisfy** — Spellcasting focus eşleşmesi sağlar.
...
- [ ] [ ] [ ] **`storage_container` → UI container hint** — Quiver/pouch/case ipucu.
- [ ] [ ] [ ] **`cost_gp` / `weight_lb` → ekonomi/carry** — Bundle başına.
- [ ] [ ] [ ] **`bundle_count` → attack consumption** — Attack başına 1 düşer; savaş sonu ½ recover.
...
- [ ] [ ] [ ] **`cost_gp` → buy** — Toplam pack fiyatı.
- [ ] [ ] [ ] **`weight_lb` → carry total** — Pack ağırlığı.
- [ ] [ ] [ ] **`content_refs[]` + `content_quantities` → `inventory[]`** — Map<entity_id, qty> ile auto-import (örn {torch_id: 10}).
- [ ] [ ] [ ] **`contents` markdown** — Narrative; typed alanlar zaten kapsıyor.
...
- [ ] [ ] [ ] **`carrying_capacity_lb` → cargo limit** — Mount üzerine yüklenebilen max ağırlık.
- [ ] [ ] [ ] **`speed_ft` → rider speed** — Mounted iken rider mount.speed_ft kullanır.
- [ ] [ ] [ ] **`cost_gp` → buy/sell** — Standart ekonomi.
- [ ] [ ] [ ] **`is_trained` → control mode** — Trained: rider tam kontrol; Untrained: mount independent (s.16).
...
- [ ] [ ] [ ] **`vehicle_kind` → grouping** — Land/Waterborne/Airborne tipi.
- [ ] [ ] [ ] **`speed_mph` → travel speed** — Saatlik kat edilen mesafe.
- [ ] [ ] [ ] **`crew` / `passengers` → kapasite** — Çalıştıracak ve binebilecek kişi sayısı.
- [ ] [ ] [ ] **`cargo_tons` → cargo limit** — Max yük.
- [ ] [ ] [ ] **`ac` + `hp` → combat target** — Saldırılırsa AC/HP üstünden çözülür.
- [ ] [ ] [ ] **`damage_threshold` → ignore-small-hits** — Bu değerin altında damage 0 sayılır.
- [ ] [ ] [ ] **`cost_gp` → buy/sell** — Ekonomi.
...
- [ ] [ ] [ ] **`roll_d100` → table key** — d100 sonucu trinket'i belirler (s.26-27).
- [ ] [ ] [ ] **`description` markdown** — Trinket'in açıklaması; flavor only.
...
- [ ] [ ] [ ] **`magic_category_ref` → grouping** — Armor/Potion/Ring/Rod/Scroll/Staff/Wand/Weapon/Wondrous.
- [ ] [ ] [ ] **`rarity_ref` → fiyat + filter** — Rarity.value_gp formülü ekonomi tarafına; UI filter.
- [ ] [ ] [ ] **`requires_attunement` → attuned_items[] gate** — Max 3 attune slot; untuned magic item çalışır ama sınırlı (sadece passive).
- [ ] [ ] [ ] **`attunement_class_refs[]` → class gate** — Holy Avenger=Paladin gibi class-locked.
- [ ] [ ] [ ] **`attunement_species_refs[]` → species gate** — Race-restricted.
- [ ] [ ] [ ] **`attunement_alignment_refs[]` → alignment gate** — Holy Avenger good-aligned.
- [ ] [ ] [ ] **`attunement_min_ability_ref` + `attunement_min_ability_score` → ability gate** — Belirli ability score eşiği.
- [ ] [ ] [ ] **`attunement_spellcaster_only` → caster gate** — Sadece caster sınıflar attune edebilir.
- [ ] [ ] [ ] **`attunement_prereq` markdown** — Narrative gating; typed gate alanlar baskın.
- [ ] [ ] [ ] **`is_cursed` → curse flag** — Identification gizler; remove curse spell ile çözülür.
- [ ] [ ] [ ] **`base_item_ref` → base inheritance** — Longsword +1 gibi item'lar base weapon stats devralır.
- [ ] [ ] [ ] **`charges_max` + `charge_regain` → charge pool** — Use action ile charge düşer; regain text "1d6+4 at dawn" parser yok (§5 #10).
- [ ] [ ] [ ] **`activation` → action economy** — None/Magic Action/BA/Reaction/Utilize/Command Word/Consumable.
- [ ] [ ] [ ] **`command_word` → use string** — Command Word activation için.
- [ ] [ ] [ ] **`granted_modifiers[]` → PC stats** — Cloak of Protection +1 AC/saves, Bracers of Archery +2 ranged vb.
- [ ] [ ] [ ] **`effects` markdown** — Narrative; typed kapsama dışı kuralları açıklar.
- [ ] [ ] [ ] **`cost_gp` / `weight_lb` → ekonomi/carry** — Standart eşya alanları.
- [ ] [ ] [ ] **`is_sentient` + sentient_int/wis/cha + sentient_alignment_ref → item NPC** — Sentient item kendi stat block'una sahip; alignment çatışması conflict resolution doğurur.
- [ ] [ ] [ ] **`sentient_communication` / `sentient_senses` / `sentient_special_purpose` → narrative** — Sentient item iletişim/algı/amaç metinleri; DM trigger için.
...
- [ ] [ ] [ ] **`source` → grouping** — Race/class/monster/feat name etiketi.
- [ ] [ ] [ ] **`trait_kind` → filter** — Passive/Sense/Defensive/Movement/Spellcasting/Other.
- [ ] [ ] [ ] **`granted_modifiers[]` → host stats** — Brave/Lucky/Darkvision-grant gibi typed bonusu host entity'e uygular.
- [ ] [ ] [ ] **`description` markdown** — Narrative; typed kapsama dışı.
...
- [ ] [ ] [ ] **`source` → grouping** — Origin (monster/NPC/class) etiketi.
- [ ] [ ] [ ] **`action_type` → economy slot** — Action/Bonus Action/Reaction/Legendary/Lair/Mythic/Free.
- [ ] [ ] [ ] **`recharge_kind` + `recharge_min_roll` → recharge** — None/Roll d6 ≥ X/Short Rest/Long Rest/Day/Dawn/Dusk.
- [ ] [ ] [ ] **`recharge` markdown** — Narrative fallback; typed pair var.
- [ ] [ ] [ ] **`uses_per_day` → daily charge** — Per-day budget.
- [ ] [ ] [ ] **`is_attack` → attack branch** — true → attack_kind/bonus/reach/range zorunlu.
- [ ] [ ] [ ] **`attack_kind` → flow** — Melee/Ranged × Weapon/Spell (4 değer; spell.attack_type ile §5 #9 vokabüler farkı).
- [ ] [ ] [ ] **`attack_bonus` → roll formül** — d20 + attack_bonus.
- [ ] [ ] [ ] **`reach_ft` → melee reach** — Default 5.
- [ ] [ ] [ ] **`range_normal_ft` + `range_long_ft` → ranged ranges** — Beyond normal disadv, beyond long auto miss.
- [ ] [ ] [ ] **`damage_dice` + `damage_type_ref` → damage roll** — Tipik formula + resistance check.
- [ ] [ ] [ ] **`save_dc` + `save_ability_ref` → target save** — Typed integer DC + ability rel.
- [ ] [ ] [ ] **`applied_condition_refs[]` → target conditions[]** — On hit / failed save uygulanan condition union.
- [ ] [ ] [ ] **`effects[]` typed DSL → resolution** — Spell effects ile aynı: damage/heal/condition/buff/debuff satırları.
- [ ] [ ] [ ] **`description` markdown** — Narrative; typed alanlar baskın.
...
- [ ] [ ] [ ] **`size_ref` → space + carry capacity** — Saldırı reach, hareket alanı.
- [ ] [ ] [ ] **`creature_type_ref` → targeting** — Hold Person (Humanoid), Turn Undead vb. spell gating.
- [ ] [ ] [ ] **`tags_line` → UI tags** — "(goblinoid)" gibi parantez tag'ler.
- [ ] [ ] [ ] **`alignment_ref` → lore** — Mekanik etki yok.
- [ ] [ ] [ ] **`ac` → hit/miss check** — Stat block AC final değer.
- [ ] [ ] [ ] **`ac_note` → AC source UI** — "natural armor", "plate" vb. açıklama.
- [ ] [ ] [ ] **`initiative_modifier` → init roll** — d20 + mod.
- [ ] [ ] [ ] **`initiative_score` → passive init** — 10 + mod (alternative).
- [ ] [ ] [ ] **`hp_average` / `hp_dice` → spawn HP** — Default avg; rolled mode dice atar.
- [ ] [ ] [ ] **`speed_walk_ft` + 4 alternatif speed → speed havuzu** — Walk/Burrow/Climb/Fly/Swim feet.
- [ ] [ ] [ ] **`can_hover` → fly hover flag** — Fly speed kontrolü.
- [ ] [ ] [ ] **`stat_block` → ability mods** — STR..CHA scores'tan modifier türevi.
- [ ] [ ] [ ] **`save_bonuses` → save override** — Stat block save satırı (ability_mod + PB if prof + misc).
- [ ] [ ] [ ] **`skill_bonuses` → skill override** — Stat block skill satırı (Stealth +6 vb.).
- [ ] [ ] [ ] **`resistance_refs[]` → ½ damage** — Damage roll'da uygulanır.
- [ ] [ ] [ ] **`vulnerability_refs[]` → ×2 damage** — Damage roll'da uygulanır.
- [ ] [ ] [ ] **`damage_immunity_refs[]` → 0 damage** — Damage roll'da uygulanır.
- [ ] [ ] [ ] **`condition_immunity_refs[]` → condition blocked** — Apply-condition akışında blok.
- [ ] [ ] [ ] **`senses[]` → typed sense+range** — Darkvision 60ft, Truesight 120ft (T7 kapandı).
- [ ] [ ] [ ] **`passive_perception` → passive Wis(Perception)** — Stealth karşıtı.
- [ ] [ ] [ ] **`language_refs[]` → comprehension** — Hangi dilleri anladığı.
- [ ] [ ] [ ] **`telepathy_ft` → telepathy range** — Mind link mesafesi.
- [ ] [ ] [ ] **`cr` → XP + difficulty** — Encounter difficulty input (s.202).
- [ ] [ ] [ ] **`xp` → death award** — Monster öldüğünde party'ye verilen XP.
- [ ] [ ] [ ] **`proficiency_bonus` → save/skill PB** — CR'den derive (mechanics §17.2).
- [ ] [ ] [ ] **`trait_refs[]` → passive abilities** — Typed trait listesi (T8 kapandı).
- [ ] [ ] [ ] **`action_refs[]` → action economy** — Action slot'undaki creature-action listesi.
- [ ] [ ] [ ] **`bonus_action_refs[]` → BA economy** — BA slot.
- [ ] [ ] [ ] **`reaction_refs[]` → reaction economy** — Reaction slot.
- [ ] [ ] [ ] **`legendary_action_uses` + `legendary_action_refs[]` → LA havuzu** — Round başına LA budget reset.
- [ ] [ ] [ ] **`lair_action_refs[]` → init 20 lair trigger** — Lair'da init 20'de tetiklenir.
- [ ] [ ] [ ] **`spell_refs[]` → spell list** — Typed spell relation (T4 kapandı); slot havuzu kendi class.spell_slots'ı veya manual.
- [ ] [ ] [ ] **`gear_refs[]` → loot inventory** — Ölünce drop edilebilen eşyalar.
...
- [ ] [ ] [ ] **Tüm field davranışı §2.19 Monster ile özdeş** — Animal-spesifik kural yok; ayrı slug sadece UI/filter.
- [ ] [ ] [ ] **Beast type filter eligibility** — Wild Shape / Conjure Animals / Ranger Companion gibi feature'lar Animal slug'ından çekim yapar.
...
- [ ] [ ] [ ] **`species_ref` → species derived'lar** — size, speed, granted_modifiers, creature_type kaynağı.
- [ ] [ ] [ ] **`class_refs[]` + `class_levels` → class derived'lar** — total_level, PB, all class field consumption.
- [ ] [ ] [ ] **`subclass_refs[]` → subclass features** — Parent class level eşeleşmeli; class_resources'a katkı.
- [ ] [ ] [ ] **`background_ref` → background grants** — Skill+tool, origin feat, gold, equipment kaynağı.
- [ ] [ ] [ ] **`alignment_ref` → lore + magic item gating** — Holy Avenger gibi alignment-locked attune'lerde gate.
- [ ] [ ] [ ] **`xp` → level türevi** — xpToLevel(xp) per s.23 → total_level.
- [ ] [ ] [ ] **`proficiency_bonus` → roll bonusları** — Şu an manuel; derive olmalı (§5 #2).
- [ ] [ ] [ ] **`feats[]` → feat effects** — ASI L4/8/12/16 + origin feat union.
- [ ] [ ] [ ] **`languages[]` → comprehension** — Common + species + background + class union.
- [ ] [ ] [ ] **`tool_proficiencies[]` → tool check bonus** — `ability_mod + (PB if in list)`.
- [ ] [ ] [ ] **`weapon_proficiencies[]` → attack bonus** — Attack roll'a PB ekleme.
- [ ] [ ] [ ] **`armor_trainings[]` → AC + spell gate** — Untrained Heavy → disadv Str/Dex tests + no spells.
- [ ] [ ] [ ] **`saving_throw_proficiencies[]` → save bonus** — `ability_mod + (PB if proficient)`.
- [ ] [ ] [ ] **`skill_proficiencies[]` + `skill_expertises[]` → skill bonus** — Expertise PB×2 (s.182).
- [ ] [ ] [ ] **`stat_block` → ability mods** — `floor((score-10)/2)`, cap 20 (30 max).
- [ ] [ ] [ ] **`combat_stats.hp` → live HP** — 0 → Unconscious + Death Saves.
- [ ] [ ] [ ] **`combat_stats.max_hp` → HP cap** — Class HD + CON formülü, Tough feat ekler.
- [ ] [ ] [ ] **`combat_stats.ac` → AC** — `armor.base_ac + DEX(cap'li) + shield + misc` (§5 #8).
- [ ] [ ] [ ] **`combat_stats.speed` → walk speed** — Species + class bonusları, Heavy STR penalty.
- [ ] [ ] [ ] **`combat_stats.initiative` → init roll** — `d20 + DEX_mod + PB-if-Alert/Feral`.
- [ ] [ ] [ ] **`combat_stats.level` → total level** — Σ class_levels.values.
- [ ] [ ] [ ] **`temp_hp` → buffer** — Damage'da önce düşer; LR ends; replace no-stack.
- [ ] [ ] [ ] **`death_saves_successes` / `death_saves_failures` → death save tracker** — 3 success → Stable; 3 fail → die; nat 20 → 1 HP; nat 1 → +2 fail.
- [ ] [ ] [ ] **`heroic_inspiration` → reroll** — Bool; spend → reroll d20 (max 1 active).
- [ ] [ ] [ ] **`hit_dice_remaining` → SR healing pool** — SR'da 1 spend → `d_size + CON` heal; LR'da +½ refresh.
- [ ] [ ] [ ] **`saving_throws` table → UI rendering** — Bonus formülü save-roll'da kullanılır.
- [ ] [ ] [ ] **`skills` table → UI rendering** — Skill bonus formülü check'te.
- [ ] [ ] [ ] **`senses[]` → special sight** — Darkvision/Truesight vb. union; species + magic item.
- [ ] [ ] [ ] **`passive_perception` → 10 + Wis(Perception) mod** — Stealth karşıtı; +5 adv, −5 disadv.
- [ ] [ ] [ ] **`passive_insight` / `passive_investigation` → 10 + ability(skill) mod** — Pasif check'ler.
- [ ] [ ] [ ] **`inventory[]` → eşya listesi** — weapon/armor/gear/magic-item.
- [ ] [ ] [ ] **`attuned_items[]` → attuned bonuses** — Max 3 attune; granted_modifiers aktif olur.
- [ ] [ ] [ ] **`cp/sp/ep/gp/pp` → currency** — Coin.value_in_gp ile convert.
- [ ] [ ] [ ] **`current_conditions[]` → active conditions** — Exhaustion stack; diğerleri non-stack.
- [ ] [ ] [ ] **`casting_ability_ref` → spell DC + atk** — Per-class; multiclass'ta birden fazla DC (§5 #3).
- [ ] [ ] [ ] **`spell_save_dc` → enemy save DC** — `8 + PB + ability_mod(casting_ability_ref)`.
- [ ] [ ] [ ] **`spell_attack_bonus` → spell attack roll** — `PB + ability_mod`.
- [ ] [ ] [ ] **`spells_known[]` → spell pool** — Class.cantrips_known + spellbook (Wizard).
- [ ] [ ] [ ] **`prepared_spells[]` → castable list** — LR'da yeniden seçim; cap class.prepared_spells_by_level.
- [ ] [ ] [ ] **`spell_slots` → castable resource** — Combined-multiclass-table (s.26).
- [ ] [ ] [ ] **`pact_magic_slots` → Warlock slot pool** — SR'da yenilenir; ayrı havuz (Pact ≠ regular).
- [ ] [ ] [ ] **`class_resources` → counter pool** — Rage/Sneak Attack/Bardic Insp/Action Surge vb. (§5 #1).
- [ ] [ ] [ ] **`trinket_ref` → flavor only** — Mekanik etki yok.
- [ ] [ ] [ ] **`personality_traits/ideals/bonds/flaws/age/height/weight/eyes/skin/hair/appearance/backstory/allies_organizations` → UI/roleplay** — Tüm narratif alanlar; mekanik etki yok.
...
- [ ] [ ] [ ] **`species_ref` / `class_refs[]` / `background_ref` → derived'lar** — PC ile aynı kaynak alanları.
- [ ] [ ] [ ] **`level` → PB (single-class)** — NPC multiclass desteklenmez; tek int.
- [ ] [ ] [ ] **`alignment_ref` → lore** — Mekanik etki yok.
- [ ] [ ] [ ] **`attitude_ref` → social DC** — Friendly/Indifferent/Hostile Influence action DC'sini etkiler (s.10, 184).
- [ ] [ ] [ ] **`location_ref` → world-map source** — UI/lore.
- [ ] [ ] [ ] **`faction` → free-form** — Lore tag.
- [ ] [ ] [ ] **`stat_block` + `combat_stats` + `proficiency_bonus` + `initiative_modifier` → combat numerics** — PC ile aynı semantik.
- [ ] [ ] [ ] **`saving_throws` / `skills` table → bonus rendering** — PC'deki gibi.
- [ ] [ ] [ ] **`resistance_refs[]` / `vulnerability_refs[]` / `damage_immunity_refs[]` / `condition_immunity_refs[]` → defenses** — Damage roll + apply-condition gating.
- [ ] [ ] [ ] **`senses[]` + `passive_perception` + `language_refs[]` + `telepathy_ft` → algı** — Vision/comprehension.
- [ ] [ ] [ ] **`trait_refs[]` → passive abilities** — NPC pariteyle Monster gibi.
- [ ] [ ] [ ] **`action_refs[]` / `special_action_refs[]` → action havuzu** — Action + LA/Lair/Mythic.
- [ ] [ ] [ ] **`equipment_refs[]` → inventory** — Stat block loot.
- [ ] [ ] [ ] **`spell_refs[]` → spellbook** — Typed spell relation.
- [ ] [ ] [ ] **`goals` / `appearance` / `mannerisms` / `secrets` markdown** — Narrative; mekanik etki yok.
...
- [ ] [ ] [ ] **`condition_ref` → host conditions[]** — Union (Exhaustion stack, diğerleri non-stack).
- [ ] [ ] [ ] **`source_entity_ref` → causality** — Hangi entity uyguladı (DM tracking).
- [ ] [ ] [ ] **`duration_rounds` → expiry tick** — null = indefinite; "1 minute / dawn" duration round'a sığmaz (§5 #11).
- [ ] [ ] [ ] **`save_dc` + `save_ability_ref` → save attempt** — End-of-turn veya trigger save.
- [ ] [ ] [ ] **`save_frequency` → save trigger window** — none / start-of-turn / end-of-turn / when-damaged.
- [ ] [ ] [ ] **`notes` → DM tracking** — Free-form not.
...
- [ ] [ ] [ ] **`danger_level` → encounter pacing** — Safe/Low/Medium/High/Deadly UI helper.
- [ ] [ ] [ ] **`environment` → tag** — "Forest", "Dungeon" gibi free-form.
- [ ] [ ] [ ] **`parent_location_ref` → hierarchy** — Nested location ağacı.
- [ ] [ ] [ ] **`plane_ref` → lore** — Material/Astral/Ethereal vb.
- [ ] [ ] [ ] **`illumination_ref` → vision modifier** — Bright/Dim/Darkness sight check etkisi (s.11).
- [ ] [ ] [ ] **`hazard_refs[]` → hazard exposure** — Burning/Falling/Suffocation vb. union (s.12).
- [ ] [ ] [ ] **`description_long` markdown → player-visible** — UI display.
- [ ] [ ] [ ] **`secrets` markdown → DM-only** — Online modda gizlenir.
...
- [ ] [ ] [ ] **`location_ref` → context** — Sahnenin geçtiği yer.
- [ ] [ ] [ ] **`status` → tracking** — Planned/Active/Completed/Skipped.
- [ ] [ ] [ ] **`illumination_ref` → vision modifier** — Bright/Dim/Darkness.
- [ ] [ ] [ ] **`travel_pace_ref` → speed multiplier** — Fast/Normal/Slow.
- [ ] [ ] [ ] **`beats` markdown → outline** — DM plan notları.
- [ ] [ ] [ ] **`npc_refs[]` → cast** — Sahnedeki NPC bağlantısı.
- [ ] [ ] [ ] **`quest_refs[]` → tied quest** — Sahne içinde aktif quest'ler.
...
- [ ] [ ] [ ] **`status` → tracking** — Not Started/Active/Completed/Failed.
- [ ] [ ] [ ] **`giver_ref` → quest NPC** — UI link.
- [ ] [ ] [ ] **`reward_item_refs[]` → `inventory[]`** — Typed loot auto-grant on complete.
- [ ] [ ] [ ] **`reward_xp` → `PlayerCharacter.xp`** — XP sum on complete.
- [ ] [ ] [ ] **`reward_gp` → `PlayerCharacter.gp`** — Gold sum on complete.
- [ ] [ ] [ ] **`reward` markdown → narrative** — Typed alanlar baskın.
- [ ] [ ] [ ] **`objective` markdown → player-visible** — Görev özeti.
- [ ] [ ] [ ] **`secrets` markdown → DM-only** — Twist/secret motivation.
...
- [ ] [ ] [ ] **`location_ref` → context** — Encounter geçtiği yer.
- [ ] [ ] [ ] **`difficulty` → XP budget validate** — Trivial/Low/Moderate/High/Deadly s.202 tablosu.
- [ ] [ ] [ ] **`monsters_refs[]` + `npcs_refs[]` → combat tracker** — Spawn listesi.
- [ ] [ ] [ ] **`environmental_effect_refs[]` → start-of-encounter trigger** — Per-encounter aktif environment.
- [ ] [ ] [ ] **`trap_refs[]` → trigger** — Encounter içindeki trap'lar.
- [ ] [ ] [ ] **`setup` markdown → DM notes** — Sahne hazırlık.
- [ ] [ ] [ ] **`tactics` markdown → DM-only** — Düşman taktikleri (gizli).
- [ ] [ ] [ ] **`xp_budget` → difficulty doğrulaması** — Σ monster.xp hesabı.
...
- [ ] [ ] [ ] **`trigger_kind` → trigger machinery** — Pressure/Tripwire/Proximity/Sound/Magical/Touch/Other.
- [ ] [ ] [ ] **`trigger` markdown** — Narrative; typed kind baskın.
- [ ] [ ] [ ] **`save_dc` + `save_ability_ref` → save attempt** — Tipik DEX (Reflex) save.
- [ ] [ ] [ ] **`damage_dice` + `damage_type_ref` → damage** — Typed dice (T1 kapandı).
- [ ] [ ] [ ] **`applied_condition_refs[]` → save-fail conditions** — Restrained, Poisoned vb.
- [ ] [ ] [ ] **`detection_dc` → Perception threshold** — Search action ile DC kontrolü.
- [ ] [ ] [ ] **`disable_dc` + `disable_ability_ref` → disarm threshold** — Genelde DEX (Thieves' Tools) veya INT (Investigation).
- [ ] [ ] [ ] **`countermeasures` markdown → DM helper** — Iron Spike vs gibi alternatif çözümler.
...
- [ ] [ ] [ ] **`poison_kind` → delivery flow** — Contact/Ingested/Inhaled/Injury (s.197).
- [ ] [ ] [ ] **`save_dc` + `save_ability_ref` → save attempt** — Tipik CON save.
- [ ] [ ] [ ] **`damage_dice` + `damage_type_ref` → damage** — Genelde Poison damage type.
- [ ] [ ] [ ] **`applied_condition_refs[]` → save-fail conditions** — Poisoned, Unconscious vb.
- [ ] [ ] [ ] **`duration_rounds` → effect süresi** — Save-fail'in geçerli kalma süresi.
- [ ] [ ] [ ] **`effect` markdown** — Narrative; typed alanlar baskın.
- [ ] [ ] [ ] **`cost_gp` → buy/sell** — Standart ekonomi.
...
- [ ] [ ] [ ] **`trigger` markdown → narrative** — Lanetin nasıl tetiklendiği.
- [ ] [ ] [ ] **`applied_condition_refs[]` → host conditions[]** — Typed condition grant.
- [ ] [ ] [ ] **`granted_modifiers[]` → ongoing PC stats** — Numeric ongoing penalties (yeni eklenmiş typed alan).
- [ ] [ ] [ ] **`removed_by_spell_refs[]` → cure gate** — Bu spell'lerden biri cast edilirse curse kalkar.
- [ ] [ ] [ ] **`effect` markdown → narrative** — Typed alanlar baskın.
- [ ] [ ] [ ] **`removed_by` markdown → narrative** — Cure açıklaması; typed list baskın.
...
- [ ] [ ] [ ] **`damage_dice` + `damage_type_ref` → damage** — Heat/Cold/Fall vb. typed dice.
- [ ] [ ] [ ] **`applied_condition_refs[]` → save-fail conditions** — Exhaustion, Frightened gibi.
- [ ] [ ] [ ] **`granted_modifiers[]` → ongoing PC stats** — Sürekli mekanik etki (typed yeni alan).
- [ ] [ ] [ ] **`save_dc` + `save_ability_ref` → save attempt** — Uygun ability save (genelde CON/STR/DEX).
- [ ] [ ] [ ] **`effect` markdown → narrative** — Typed alanlar baskın.
...
- [ ] [ ] [ ] **`skill_ref` → uzmanlık** — UI: hireling specialty.
- [ ] [ ] [ ] **`daily_cost_cp` → günlük gp drain** — Hire/day → currency düşer.
- [ ] [ ] [ ] **`skilled` → kategori flag** — Skilled (≥2 sp/day) / Untrained (~2 sp).
...
- [ ] [ ] [ ] **`kind` → grouping** — Spellcasting/Transport/Shelter/Other.
- [ ] [ ] [ ] **`cost_cp` → currency drain** — Use action'da PC.cp..pp düşer.
- [ ] [ ] [ ] **`availability` → UI hint** — "Cities only" gibi şart metni.
...
- [ ] [ ] [ ] **Adım 1: Class assignment kaynak akışı** — class_refs → hit_die/save/skill/weapon/armor/tool/equipment/gold/casting_ability/feature_table[1] tüm propagasyonları.
- [ ] [ ] [ ] **Adım 2: Origin (background + species) akışı** — Background skills/tools/ASI/feat/gold + Species size/speed/languages/senses/resistances/skills/traits.
- [ ] [ ] [ ] **Adım 3: Ability scores akışı** — Standard/Random/Point Buy + ASI'den stat_block + ability_mod ile saving_throws/skills/AC/init/HP/spell_DC propagasyonu.
- [ ] [ ] [ ] **Adım 4: Equipment + inventory akışı** — Class.starting_equipment + Background.default_inventory PC.inventory'ye union; Armor.equip → AC formula tetikleyicisi.
- [ ] [ ] [ ] **Adım 5: Final detaylar** — total_level=1 → PB=+2; combat_stats.hp = max_hp; passive_perception; spell_save_dc/atk (caster); spell_slots/cantrips_known.
...
- [ ] [ ] [ ] **xp → level threshold check** — xpToLevel(xp) > current_level olunca level-up tetikle.
- [ ] [ ] [ ] **class_levels[<chosen>] += 1 + total_level recompute** — Class seçimi, total_level Σ, PB güncelleme.
- [ ] [ ] [ ] **PB propagation** — saving_throws/skills/attack/spell DC rendering yenilenir.
- [ ] [ ] [ ] **HP artışı (rolled veya fixed)** — hit_dice_remaining +1; max_hp += avg(d_size)+CON.
- [ ] [ ] [ ] **Class.feature_table[N] aktivasyonu** — Yeni level satırındaki feature/resource'lar class_resources'a eklenir.
- [ ] [ ] [ ] **Subclass selection trigger** — Class.feature_table[N]'da subclass row varsa subclass_refs seçimi açılır.
- [ ] [ ] [ ] **ASI / Feat seçimi** — Level-up satırında ASI varsa stat_block veya feats[] güncellemesi.
- [ ] [ ] [ ] **Subclass.features aktivasyonu** — Subclass.granted_at_level ≤ N ise subclass features class_resources'a eklenir.
- [ ] [ ] [ ] **Long-rest spell_slots/pact/HD/exhaustion/prepared/temp_hp refresh** — Tüm post-rest tetikleyicileri.
...
- [ ] [ ] [ ] **Slot gate** — Spell.level vs PC.spell_slots[≥]; cantrip slot harcamaz; ritual path opsiyonel.
- [ ] [ ] [ ] **Action economy** — casting_time_amount + casting_time_unit_ref → Action/BA/Reaction/Min/Hour slot tüketimi; Reaction → reaction_trigger gerek.
- [ ] [ ] [ ] **V/S/M satisfy** — V (silenced değil), S (free hand), M (focus/pouch/specific item); material_consumed → gp düş.
- [ ] [ ] [ ] **Targeting** — range_type + range_ft + area_shape_ref + area_size_ft AoE coverage hesabı.
- [ ] [ ] [ ] **Attack roll branch** — attack_type Melee/Ranged → spell attack roll = `PB + spell_mod + bonuses`.
- [ ] [ ] [ ] **Save roll branch** — save_ability_ref → target.save_bonus vs PC.spell_save_dc; success → save_effect uygula.
- [ ] [ ] [ ] **Damage resistance check** — damage_type_refs vs target.resistance/vulnerability/immunity; ½ / ×2 / 0.
- [ ] [ ] [ ] **Concentration management** — requires_concentration=true → mevcut concentration drop; damage'da DC 10 ya da ½ dmg save.
- [ ] [ ] [ ] **Active effect tracker** — duration_unit_ref + duration_amount → AppliedCondition veya active-effect entry duration_rounds takibi.
...
- [ ] [ ] [ ] **#6 ✅ `Species.traits`, `Feat.benefits`, `MagicItem.effects`, `Trait.description` markdown blob** — typed `granted_modifiers[]` DSL ile çözüldü (Tier-B). 24 modifier kind tüm bonus tipi taşır; markdown narrative kalır.
- [ ] [ ] [ ] **#7 Multiclass starting proficiency tablosu (s.25)** — `Class.multiclass_requirements` markdown'da; data değil. "Barb multiclass'a girince sadece Shields + Martial weapon training" subset kuralı kodlanamaz.
- [ ] [ ] [ ] **#8 `PlayerCharacter.combat_stats.ac` serbest int** — armor formülü kodlu değil (`base_ac + DEX [+cap] + shield + misc`). Unarmored Defense (Barb=10+DEX+CON, Monk=10+DEX+WIS) seçim mekanizması yok; non-stack multiclass kuralı advisory.
- [ ] [ ] [ ] **#9 `Spell.attack_type` enum {None,Melee,Ranged}** vs **`creature-action.attack_kind` enum {Melee Weapon, Ranged Weapon, Melee Spell, Ranged Spell}** — iki vokabüler. Reconcile gerek veya mapping function.
...
- [ ] [ ] [ ] **#11 `AppliedCondition.duration_rounds int (null=indefinite)`** — Exhaustion stack ve "1 minute / until next dawn / until cured" duration'lar lossy. Round-based tek granularity; minute/hour/day → round (1 min = 10 rounds).
- [ ] [ ] [ ] **#12 `Background.ability_score_options` sadece ability listesi** — "+2/+1 vs +1/+1/+1" choice (s.83) kodlu değil; dağıtım PC.stat_block'ta serbest. UI seçim sunmalı + max-20 cap zorlamalı.
- [ ] [ ] [ ] **#13 `PlayerCharacter.class_levels` `levelTable` (Map<int,int>)** — implicit key class_id (UUID), value level. "level" değil. Type aliasing convention; FieldType naming yanıltıcı.
- [ ] [ ] [ ] **#14 `Animal` slug Monster duplicate** — şema seviyesinde `_animalCategory()` Monster field listesini `copyWith` ile rebuild eder. Field semantikleri özdeş; UI/import/codec ayrı slug filter için ayrı.
