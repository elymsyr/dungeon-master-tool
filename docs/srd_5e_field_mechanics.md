...
- [ ] [x] [ ] **`size.space_ft` → creature space** — Tiny=2.5, S/M=5, Large=10, Huge=15, Gargantuan=20.
- [ ] [x] [ ] **`size.hit_die_size` → monster default HD** — Class HD'si yoksa fallback (s.22).
- [ ] [x] [ ] **`size.carrying_multiplier` → carry capacity** — `STR × 15 × multiplier` (s.16).
- [ ] [x] [ ] **`sense.default_range_ft` → rangedSense fallback** — Darkvision 60, Truesight 120 vb. consumer override edebilir.
- [ ] [x] [ ] **`damage-type.bypassable_by_magical` → resistance bypass** — Magical/silvered/adamantine nonmagical resistance'ı bypass eder (sadece B/P/S için true).
- [ ] [x] [ ] **`rarity.value_gp` → fiyat formülü** — Common 100/Uncommon 400/Rare 4000/VR 40000/Legendary 200000; consumable ½.
- [ ] [x] [ ] **`rarity.crafting_time_days` → downtime tracker** — Crafting süreci günleri.
- [ ] [x] [ ] **`rarity.crafting_cost_gp` → gp drain** — Crafting maliyeti.
- [ ] [x] [ ] **`coin.value_in_gp` → currency conversion** — CP=0.01, SP=0.1, EP=0.5, GP=1, PP=10; 50 coins = 1 lb.
- [ ] [x] [ ] **`lifestyle.cost_per_day_gp` → günlük gp drain** — Wretched 0, Squalid 0.1, ..., Aristocratic ≥10.
- [ ] [x] [ ] **`condition.stacks` → stack rule** — Sadece Exhaustion true; diğerleri non-stack.
- [ ] [x] [ ] **`condition.grants_incapacitated` → incapacitated chain** — Stunned/Paralyzed/Petrified/Unconscious true → no actions + concentration drop.
- [ ] [x] [ ] **`damage-type.is_physical` → B/P/S classification** — Magic weapon resistance bypass için kullanılır.
- [ ] [x] [ ] **`duration-unit.is_concentration_compatible` → concentration check** — Rounds/Minutes/Hours true; Instantaneous false.
- [ ] [x] [ ] **`weapon-property.mechanic_kind` → property dispatch** — 11 kind enum (Finesse/Heavy/Light vs.); resolver typed switch.
- [ ] [x] [ ] **`weapon-mastery.effect_kind` → mastery effect dispatch** — 8 kind enum (Cleave/Graze/Nick/Push/Sap/Slow/Topple/Vex).
- [ ] [x] [ ] **`weapon-mastery.effect_value` → mesafe/değer** — Push=10ft, Slow=10ft, Cleave/Graze=0.
- [ ] [x] [ ] **`weapon-mastery.save_ability` → mastery save** — Topple=CON; diğerleri save'siz.

**§1.1 Identifier-only lookups (mekanik etkisi yok, sadece name/icon/description):**

- [ ] [x] [ ] **36 identifier-only lookup şeması** — ability/skill/damage-type/condition/size/creature-type/alignment/language/weapon-category/property/mastery/armor-category/tool-category/spell-school/magic-item-category/rarity/speed-type/sense/action/area-shape/attitude/cover/illumination/hazard/feat-category/lifestyle/coin/tier-of-play/travel-pace/arcane-focus/druidic-focus/holy-symbol/plane/casting-component/casting-time-unit/duration-unit. Mekanik consumer field'larında uygulanır.
...
- [ ] [x] [ ] **`primary_ability_ref` → multiclass eligibility** — Yeni sınıf eklenirken hem mevcut hem yeni sınıfın primary'si ≥13 olmalı; PC'a yazılmaz, gate kontrolünde okunur.
- [ ] [x] [ ] **`hit_die` → `combat_stats.max_hp` & `hit_dice_remaining`** — İlk sınıfta L1 max+CON, sonraki seviyelerde avg+CON eklenir; her seviye HD havuzuna 1 die katar.
- [ ] [x] [ ] **`saving_throw_refs` → `saving_throw_proficiencies[]`** — Sadece ilk sınıftan akar; multiclass'ta sonraki sınıfın save'leri gelmez.
- [ ] [x] [ ] **`skill_proficiency_choice_count` + `skill_proficiency_options[]` → skill choice pool** — UI'de N adet kutu açar, kullanıcı seçer; sonuç `skill_proficiencies`'e union.
- [ ] [x] [ ] **`weapon_proficiency_categories[]` → `weapon_proficiencies[]`** — Multiclass'ta SRD s.25 subset (Barb/Fight/Pal/Ranger martial alır, diğerleri kısıtlı).
- [ ] [x] [ ] **`weapon_proficiency_specifics[]` → `weapon_proficiencies[]`** — Spesifik silah grant'leri; multiclass'ta tipik gelmez (sınıfa özel).
- [ ] [x] [ ] **`tool_proficiency_count` + `tool_proficiency_options[]` → `tool_proficiencies[]`** — Skill ile aynı choice mantığı, first-class-only.
- [ ] [x] [ ] **`armor_training_refs[]` → `armor_trainings[]`** — Multiclass'ta Light+Medium+Shield genelde gelir; Heavy & Martial sınıfa özel. Heavy worn without training → disadv Str/Dex tests, no spells.
- [ ] [x] [ ] **`default_inventory_refs[]` → `PlayerCharacter.inventory[]`** — İlk sınıf seçildiğinde otomatik kopya, sonraki sınıflarda eklenmez.
- [ ] [x] [ ] **`starting_gold_dice` → `PlayerCharacter.gp`** — İlk sınıfta zar atılır; background gold ile karşılıklı (biri seçilir).
- [ ] [x] [ ] **`casting_ability_ref` → `spell_save_dc` / `spell_attack_bonus`** — Formül: DC = `8 + PB + ability_mod`, attack = `PB + ability_mod`. Caster olmayan sınıfta null. Multiclass'ta her caster kendi DC'sini hesaplar (şu an tek skaler — açık mekanik §5 #3).
- [ ] [x] [ ] **`caster_kind` → spell slot havuzu girişi** — Full=lvl, Half=½lvl, Third=⅓lvl combined-table'a feed eder; Pact ayrı havuz; Ritual slot vermez.
- [ ] [x] [ ] **`spellcasting_focus_ref` → V/S/M satisfaction flag** — Inventory'de focus item varsa Material component shortcut; UI rehberi.
- [ ] [x] [ ] **`features` → `class_resources` + level-gated feature aktivasyonu** — Typed feature satırları (level/name/dice/uses/recharge); level-up'ta yeni satırlar aktive olur, Rage/Sneak Attack vb. bu havuzdan beslenir.
- [ ] [x] [ ] **`cantrips_known_by_level` → `spells_known[type=cantrip]` cap** — Level başına bilinen cantrip sayısı sınırı; her caster sınıf kendi cap'i.
- [ ] [x] [ ] **`prepared_spells_by_level` → `prepared_spells[]` cap** — Long-rest'te yeniden seçilebilen prepared spell sayısı; multiclass'ta her sınıf ayrı.
- [ ] [x] [ ] **`spell_slots_by_level` → `spell_slots`** — Level-up + long-rest'te combined-table'a girer; multiclass'ta tüm caster'lar tek slot havuzunda toplanır (Pact ayrı).
- [ ] [x] [ ] **`multiclass_requirements` → eligibility gate** — Karakter sınıfa girmeden önce kontrol; başarısızsa class atama bloklanır.
- [ ] [x] [ ] **`primary_ability_ref` advisory + `secondary_ability_ref` advisory** — UI'de "stat dağıtımı için önerilen" rehber; mekanik etki yok.
- [ ] [x] [ ] **`complexity` advisory** — UI'de zorluk rozeti; mekanik etki yok.
- [ ] [x] [ ] **`starting_equipment_options` markdown choice prompt** — Narrative "A vs B" karar metni; typed import `default_inventory_refs` üstünden.
- [ ] [x] [ ] **Class-aware multiclass starting profs subset (s.25)** — Şemada kodlu değil, narrative `multiclass_requirements`'tan parse edilmesi gerek (açık mekanik §5 #7).
...
- [ ] [x] [ ] **`parent_class_ref` → eligibility gate** — Subclass yalnız parent class içinde geçerli; başka sınıfa atanamaz.
- [ ] [x] [ ] **`granted_at_level` → aktivasyon seviyesi** — Parent class'ın level_in değeri bu eşiğe ulaştığında subclass features tetiklenir.
- [ ] [x] [ ] **`features` → `class_resources`** — Typed feature satırları parent class feature'ları gibi level-up'ta sıralı aktive olur.
- [ ] [x] [ ] **`flavor_description` → UI lore** — Mekanik etki yok, sadece subclass tanıtım metni.
...
- [ ] [x] [ ] **`size_ref` → space + carry capacity** — Tiny/Small/Medium/Large; carrying = `STR × multiplier(size)`. Çoğu species Medium.
- [ ] [x] [ ] **`speed_ft` → `combat_stats.speed`** — Walk speed override base; class feature/feat (Fast Movement, Mobile) üstüne ekler.
- [ ] [x] [ ] **`creature_type_ref` → `PlayerCharacter.creature_type`** — Spell targeting (Hold Person sadece Humanoid), monster ability ref'leri için kullanılır.
- [ ] [x] [ ] **`granted_modifiers[]` → PC derived stats** — Typed ability_score_bonus, sense_grant, resistance_grant vs. her modifier kind'ı kendi target'ına uygulanır.
- [ ] [x] [ ] **`traits` → narrative UI** — Markdown; özel kurallar `granted_modifiers` ile kapsanmadıysa sadece açıklayıcı metin kalır.
- [ ] [x] [ ] **`granted_languages[]` → `languages[]`** — Common otomatik default; bu liste ek diller union eder.
- [ ] [x] [ ] **`granted_senses[]` → `senses[]`** — Darkvision 60 (most), 120 (Dwarf/Orc), Truesight (rare).
- [ ] [x] [ ] **`granted_damage_resistances[]` → resistance set** — Damage roll'da tüketilir; Dwarf=Poison, Tiefling=Fire vb.
- [ ] [x] [ ] **`granted_skill_proficiencies[]` → `skill_proficiencies[]`** — Elf Keen Senses gibi sabit skill grant'leri union edilir.
- [ ] [x] [ ] **`age` → lore only** — Mekanik etki yok.
...
- [ ] [x] [ ] **`granted_skill_refs[]` → `skill_proficiencies[]`** — 2 sabit skill, union edilir.
- [ ] [x] [ ] **`granted_tool_refs[]` → `tool_proficiencies[]`** — Sabit veya choice tool prof grant'ı.
- [ ] [x] [ ] **`granted_language_count` → language seçim sayısı** — UI choice; SRD 2024'te genelde 0.
- [ ] [x] [ ] **`ability_score_options[]` + ASI dağılımı → `stat_block`** — +2/+1 veya +1/+1/+1 manuel dağıtım; max 20 cap. Açık mekanik §5 #12.
- [ ] [x] [ ] **`origin_feat_ref` → `feats[]`** — Background L1 feat (Acolyte=Magic Initiate Cleric, Criminal=Alert vb.) otomatik append.
- [ ] [x] [ ] **`default_inventory_refs[]` → `inventory[]`** — Equipment paketi auto-import.
- [ ] [x] [ ] **`starting_equipment` markdown choice** — Narrative "A vs B" seçim metni; typed import default_inventory_refs.
- [ ] [x] [ ] **`starting_gold_gp` → `gp`** — Default başlangıç altını; class.starting_gold_dice ile karşılıklı.
- [ ] [x] [ ] **`gold_alternative_gp` → `gp` (replace inventory)** — Equipment paketi yerine altın seçeneği (~50 GP).
...
- [ ] [x] [ ] **`category_ref` → eligibility gating** — Origin=L1, General=L4+ ASI, Fighting Style=class feature, Epic Boon=L19.
- [ ] [x] [ ] **`prereq_ability_ref` + `prereq_min_score` → ability gate** — Heavy Armor Master STR≥13 gibi typed kontrol.
- [ ] [x] [ ] **`prereq_class_refs[]` → class gate** — Sadece bu sınıflara açık (örn. War Caster caster sınıflarda).
- [ ] [x] [ ] **`prereq_species_refs[]` → species gate** — Species-locked feat'lar (rare).
- [ ] [x] [ ] **`prereq_min_character_level` → total level gate** — Epic Boons L19+ kontrolü.
- [ ] [x] [ ] **`prereq_requires_spellcasting` → caster gate** — PC herhangi bir caster_kind ≠ None mı kontrolü (Magic Initiate, Ritual Caster).
- [ ] [x] [ ] **`prerequisite` markdown** — Narrative açıklama; gating typed alanlardan.
- [ ] [x] [ ] **`repeatable` + `repeatable_limit` → take counter** — Aynı feat'ı birden fazla alma izni; ASI/Skilled/Magic Initiate variant'lar.
- [ ] [x] [ ] **`asi_ability_options[]` + `asi_amount` + `asi_max_score` → `stat_block` artışı** — Typed ASI uygulaması; cap'i aşmaz.
- [ ] [x] [ ] **`ability_score_increase` markdown** — Narrative ASI metni; typed alanlar `asi_*`.
- [ ] [x] [ ] **`granted_modifiers[]` → PC derived stats** — Alert init+PB, Tough +HP/level, Lucky reroll, Mobile +10 speed, Observant +5 passive vs.
- [ ] [x] [ ] **`benefits` markdown** — Narrative; typed kapsama dışı kuralları açıklar.
...
- [ ] [x] [ ] **`level` → required slot** — Cantrip (0) slot harcamaz; L1+ slot gerektirir, upcast slot ≥ level kontrolü.
- [ ] [x] [ ] **`school_ref` → enum tag** — Counterspell/Detect Magic gibi school-bazlı interaction'lar için.
- [ ] [x] [ ] **`casting_time_amount` + `casting_time_unit_ref` → action economy** — Action/BA/Reaction/dakika/saat.
- [ ] [x] [ ] **`reaction_trigger` → reaction trigger string** — Reaction casting şart metni (Shield "when hit" vb.).
- [ ] [x] [ ] **`is_ritual` → ritual cast option** — Slot harcamadan +10 dakika ile cast; Ritual Adept prepare etmeden mümkün kılar.
- [ ] [x] [ ] **`range_type` + `range_ft` → targeting** — Self/Touch/Ranged/Sight/Unlimited; Ranged ise feet sayısı.
- [ ] [x] [ ] **`area_shape_ref` + `area_size_ft` → AoE coverage** — Cone/Cube/Cylinder/Emanation/Line/Sphere; size feet.
- [ ] [x] [ ] **`components[]` → V/S/M satisfy** — V=verbal, S=somatic, M=material; focus item M'i karşılar.
- [ ] [x] [ ] **`material_description` + `material_cost_gp` + `material_consumed` → M cost** — Consumed=true ise gp düşer; örn Find Familiar 10gp consumed, Identify 100gp not.
- [ ] [x] [ ] **`duration_unit_ref` + `duration_amount` → ends-at** — Instantaneous/Round/Minute/Hour/Day/Until Dispelled.
- [ ] [x] [ ] **`requires_concentration` → `concentration` slot** — Tek concentration; yenisi eski'yi düşürür (s.179).
- [ ] [x] [ ] **`description` markdown** — Narrative; mekanik etki `effects`'ten.
- [ ] [x] [ ] **`effects[]` typed DSL → resolution** — Her satır kind (damage/heal/condition/buff/debuff) + dice + save_ability + save_effect (none/half/negate/partial); cast sırasında satır satır resolve.
- [ ] [x] [ ] **`at_higher_levels_text` → upcast scaling** — Slot > base level'da ek dice; numeric scaling effects[].scaling_dice'da.
- [ ] [x] [ ] **`class_refs[]` → spell list belonging** — PC.spells_known'a ekleme gating; multiclass'ta her sınıf kendi listesine bakar.
- [ ] [x] [ ] **`damage_type_refs[]` → resistance check** — Damage roll'da hedefin resistance/vulnerability/immunity setiyle karşılaştırılır.
- [ ] [x] [ ] **`save_ability_ref` → target save** — Save DC = caster'ın spell_save_dc'si; başarılı save → save_effect uygulanır.
- [ ] [x] [ ] **`attack_type` → attack roll variant** — None/Melee/Ranged spell attack; vokabüler farkı creature-action ile (§5 #9).
- [ ] [x] [ ] **`applied_condition_refs[]` → target conditions[]** — Spell-level condition apply (effects[] içindeki ile birlikte).
...
- [ ] [x] [ ] **`category_ref` → prof check** — Simple/Martial; PC.weapon_proficiencies içeriyorsa attack roll'a PB eklenir.
- [ ] [x] [ ] **`is_melee` → ability default** — STR (melee) / DEX (ranged); Finesse property override eder.
- [ ] [x] [ ] **`damage_dice` → damage roll** — Roll + ability_mod; crit'te dice 2x.
- [ ] [x] [ ] **`damage_type_ref` → resistance check** — Bludg/Pierc/Slash + magical type'lar.
- [ ] [x] [ ] **`property_refs[]` → behavior modifiers** — Her property mechanic_kind enum'dan dispatch (Finesse/Heavy/Light/Two-Handed/Versatile/Reach/Thrown/Loading/Ammunition).
- [ ] [x] [ ] **`mastery_ref` → mastery effect** — Cleave/Graze/Nick/Push/Sap/Slow/Topple/Vex; aktif mastery slotuna bağlı non-stack.
- [ ] [x] [ ] **`normal_range_ft` + `long_range_ft` → range gating** — Beyond normal disadv, beyond long auto miss.
- [ ] [x] [ ] **`versatile_damage_dice` → 2H grip damage** — Versatile property gerekli; iki elle tutulduğunda alternate dice.
- [ ] [x] [ ] **`ammunition_type_ref` → ammo consumption** — Loading: 1 ammo per Action/BA/Reaction; recover ½ after fight.
- [ ] [x] [ ] **`cost_gp` → buy/sell** — Sell ½ default.
- [ ] [x] [ ] **`weight_lb` → carry total** — Encumbrance toplamına ekler.
...
- [ ] [x] [ ] **`category_ref` → training check** — armor_trainings içermiyorsa disadv all Str/Dex tests + no spell.
- [ ] [x] [ ] **`base_ac` + `adds_dex` + `dex_cap` → `combat_stats.ac`** — Formula: `base + (DEX cap'lı) + shield + misc`.
- [ ] [x] [ ] **`strength_requirement` → speed penalty** — STR<req → speed −10.
- [ ] [x] [ ] **`stealth_disadvantage` → stealth check** — DEX(Stealth) check'lerine disadv.
- [ ] [x] [ ] **`don_time_minutes` / `doff_time_minutes` → time cost** — Light 1m, Medium 5m, Heavy 10m, Shield 1 action.
- [ ] [x] [ ] **`cost_gp` → buy/sell** — Sell ½ default.
- [ ] [x] [ ] **`weight_lb` → carry total** — Encumbrance ekleme.
...
- [ ] [x] [ ] **`category_ref` → grouping** — Artisan/Gaming/Musical/Other.
- [ ] [x] [ ] **`variant_of_ref` → variant link** — Sub-tool grupları için bağlantı.
- [ ] [x] [ ] **`ability_ref` → check ability** — Tool kullanımında default ability rolü (örn Thieves' Tools = DEX).
- [ ] [x] [ ] **`utilize_check_dc` → default DC** — Utilize action default zorluk.
- [ ] [x] [ ] **`utilize_description` → usage hint** — UI rehber metni.
- [ ] [x] [ ] **`craftable_items[]` → crafting input** — Bu tool ile yapılabilen eşya listesi (s.103).
- [ ] [x] [ ] **`cost_gp` / `weight_lb` → ekonomi/carry** — Standart eşya alanları.
...
- [ ] [x] [ ] **`cost_cp` → currency conversion** — CP precision; coin lookup üstünden cp/sp/ep/gp/pp arasına dönüştürülür.
- [ ] [x] [ ] **`weight_lb` → carry total** — Encumbrance.
- [ ] [x] [ ] **`utilize_check_dc` + `utilize_ability_ref` → ability check** — Utilize action'da DC + ability rolü.
- [ ] [x] [ ] **`utilize_description` → narrative effect** — Markdown; mekanik gating typed alanlar üstünden.
- [ ] [x] [ ] **`consumable` → inventory dec** — Use action'da envanterden 1 düşer (potion, holy water vb.).
- [ ] [x] [ ] **`is_focus` + `focus_kind_ref` → V/S/M satisfy** — Spellcasting focus eşleşmesi sağlar.
...
- [ ] [x] [ ] **`storage_container` → UI container hint** — Quiver/pouch/case ipucu.
- [ ] [x] [ ] **`cost_gp` / `weight_lb` → ekonomi/carry** — Bundle başına.
- [ ] [x] [ ] **`bundle_count` → attack consumption** — Attack başına 1 düşer; savaş sonu ½ recover.
...
- [ ] [x] [ ] **`cost_gp` → buy** — Toplam pack fiyatı.
- [ ] [x] [ ] **`weight_lb` → carry total** — Pack ağırlığı.
- [ ] [x] [ ] **`content_refs[]` + `content_quantities` → `inventory[]`** — Map<entity_id, qty> ile auto-import (örn {torch_id: 10}).
- [ ] [x] [ ] **`contents` markdown** — Narrative; typed alanlar zaten kapsıyor.
...
- [ ] [x] [ ] **`carrying_capacity_lb` → cargo limit** — Mount üzerine yüklenebilen max ağırlık.
- [ ] [x] [ ] **`speed_ft` → rider speed** — Mounted iken rider mount.speed_ft kullanır.
- [ ] [x] [ ] **`cost_gp` → buy/sell** — Standart ekonomi.
- [ ] [x] [ ] **`is_trained` → control mode** — Trained: rider tam kontrol; Untrained: mount independent (s.16).
...
- [ ] [x] [ ] **`vehicle_kind` → grouping** — Land/Waterborne/Airborne tipi.
- [ ] [x] [ ] **`speed_mph` → travel speed** — Saatlik kat edilen mesafe.
- [ ] [x] [ ] **`crew` / `passengers` → kapasite** — Çalıştıracak ve binebilecek kişi sayısı.
- [ ] [x] [ ] **`cargo_tons` → cargo limit** — Max yük.
- [ ] [x] [ ] **`ac` + `hp` → combat target** — Saldırılırsa AC/HP üstünden çözülür.
- [ ] [x] [ ] **`damage_threshold` → ignore-small-hits** — Bu değerin altında damage 0 sayılır.
- [ ] [x] [ ] **`cost_gp` → buy/sell** — Ekonomi.
...
- [ ] [x] [ ] **`roll_d100` → table key** — d100 sonucu trinket'i belirler (s.26-27).
- [ ] [x] [ ] **`description` markdown** — Trinket'in açıklaması; flavor only.
...
- [ ] [x] [ ] **`magic_category_ref` → grouping** — Armor/Potion/Ring/Rod/Scroll/Staff/Wand/Weapon/Wondrous.
- [ ] [x] [ ] **`rarity_ref` → fiyat + filter** — Rarity.value_gp formülü ekonomi tarafına; UI filter.
- [ ] [x] [ ] **`requires_attunement` → attuned_items[] gate** — Max 3 attune slot; untuned magic item çalışır ama sınırlı (sadece passive).
- [ ] [x] [ ] **`attunement_class_refs[]` → class gate** — Holy Avenger=Paladin gibi class-locked.
- [ ] [x] [ ] **`attunement_species_refs[]` → species gate** — Race-restricted.
- [ ] [x] [ ] **`attunement_alignment_refs[]` → alignment gate** — Holy Avenger good-aligned.
- [ ] [x] [ ] **`attunement_min_ability_ref` + `attunement_min_ability_score` → ability gate** — Belirli ability score eşiği.
- [ ] [x] [ ] **`attunement_spellcaster_only` → caster gate** — Sadece caster sınıflar attune edebilir.
- [ ] [x] [ ] **`attunement_prereq` markdown** — Narrative gating; typed gate alanlar baskın.
- [ ] [x] [ ] **`is_cursed` → curse flag** — Identification gizler; remove curse spell ile çözülür.
- [ ] [x] [ ] **`base_item_ref` → base inheritance** — Longsword +1 gibi item'lar base weapon stats devralır.
- [ ] [x] [ ] **`charges_max` + `charge_regain` → charge pool** — Use action ile charge düşer; regain text "1d6+4 at dawn" parser yok (§5 #10).
- [ ] [x] [ ] **`activation` → action economy** — None/Magic Action/BA/Reaction/Utilize/Command Word/Consumable.
- [ ] [x] [ ] **`command_word` → use string** — Command Word activation için.
- [ ] [x] [ ] **`granted_modifiers[]` → PC stats** — Cloak of Protection +1 AC/saves, Bracers of Archery +2 ranged vb.
- [ ] [x] [ ] **`effects` markdown** — Narrative; typed kapsama dışı kuralları açıklar.
- [ ] [x] [ ] **`cost_gp` / `weight_lb` → ekonomi/carry** — Standart eşya alanları.
- [ ] [x] [ ] **`is_sentient` + sentient_int/wis/cha + sentient_alignment_ref → item NPC** — Sentient item kendi stat block'una sahip; alignment çatışması conflict resolution doğurur.
- [ ] [x] [ ] **`sentient_communication` / `sentient_senses` / `sentient_special_purpose` → narrative** — Sentient item iletişim/algı/amaç metinleri; DM trigger için.
...
- [ ] [x] [ ] **`source` → grouping** — Race/class/monster/feat name etiketi.
- [ ] [x] [ ] **`trait_kind` → filter** — Passive/Sense/Defensive/Movement/Spellcasting/Other.
- [ ] [x] [ ] **`granted_modifiers[]` → host stats** — Brave/Lucky/Darkvision-grant gibi typed bonusu host entity'e uygular.
- [ ] [x] [ ] **`description` markdown** — Narrative; typed kapsama dışı.
...
- [ ] [x] [ ] **`source` → grouping** — Origin (monster/NPC/class) etiketi.
- [ ] [x] [ ] **`action_type` → economy slot** — Action/Bonus Action/Reaction/Legendary/Lair/Mythic/Free.
- [ ] [x] [ ] **`recharge_kind` + `recharge_min_roll` → recharge** — None/Roll d6 ≥ X/Short Rest/Long Rest/Day/Dawn/Dusk.
- [ ] [x] [ ] **`recharge` markdown** — Narrative fallback; typed pair var.
- [ ] [x] [ ] **`uses_per_day` → daily charge** — Per-day budget.
- [ ] [x] [ ] **`is_attack` → attack branch** — true → attack_kind/bonus/reach/range zorunlu.
- [ ] [x] [ ] **`attack_kind` → flow** — Melee/Ranged × Weapon/Spell (4 değer; spell.attack_type ile §5 #9 vokabüler farkı).
- [ ] [x] [ ] **`attack_bonus` → roll formül** — d20 + attack_bonus.
- [ ] [x] [ ] **`reach_ft` → melee reach** — Default 5.
- [ ] [x] [ ] **`range_normal_ft` + `range_long_ft` → ranged ranges** — Beyond normal disadv, beyond long auto miss.
- [ ] [x] [ ] **`damage_dice` + `damage_type_ref` → damage roll** — Tipik formula + resistance check.
- [ ] [x] [ ] **`save_dc` + `save_ability_ref` → target save** — Typed integer DC + ability rel.
- [ ] [x] [ ] **`applied_condition_refs[]` → target conditions[]** — On hit / failed save uygulanan condition union.
- [ ] [x] [ ] **`effects[]` typed DSL → resolution** — Spell effects ile aynı: damage/heal/condition/buff/debuff satırları.
- [ ] [x] [ ] **`description` markdown** — Narrative; typed alanlar baskın.
...
- [ ] [x] [ ] **`size_ref` → space + carry capacity** — Saldırı reach, hareket alanı.
- [ ] [x] [ ] **`creature_type_ref` → targeting** — Hold Person (Humanoid), Turn Undead vb. spell gating.
- [ ] [x] [ ] **`tags_line` → UI tags** — "(goblinoid)" gibi parantez tag'ler.
- [ ] [x] [ ] **`alignment_ref` → lore** — Mekanik etki yok.
- [ ] [x] [ ] **`ac` → hit/miss check** — Stat block AC final değer.
- [ ] [x] [ ] **`ac_note` → AC source UI** — "natural armor", "plate" vb. açıklama.
- [ ] [x] [ ] **`initiative_modifier` → init roll** — d20 + mod.
- [ ] [x] [ ] **`initiative_score` → passive init** — 10 + mod (alternative).
- [ ] [x] [ ] **`hp_average` / `hp_dice` → spawn HP** — Default avg; rolled mode dice atar.
- [ ] [x] [ ] **`speed_walk_ft` + 4 alternatif speed → speed havuzu** — Walk/Burrow/Climb/Fly/Swim feet.
- [ ] [x] [ ] **`can_hover` → fly hover flag** — Fly speed kontrolü.
- [ ] [x] [ ] **`stat_block` → ability mods** — STR..CHA scores'tan modifier türevi.
- [ ] [x] [ ] **`save_bonuses` → save override** — Stat block save satırı (ability_mod + PB if prof + misc).
- [ ] [x] [ ] **`skill_bonuses` → skill override** — Stat block skill satırı (Stealth +6 vb.).
- [ ] [x] [ ] **`resistance_refs[]` → ½ damage** — Damage roll'da uygulanır.
- [ ] [x] [ ] **`vulnerability_refs[]` → ×2 damage** — Damage roll'da uygulanır.
- [ ] [x] [ ] **`damage_immunity_refs[]` → 0 damage** — Damage roll'da uygulanır.
- [ ] [x] [ ] **`condition_immunity_refs[]` → condition blocked** — Apply-condition akışında blok.
- [ ] [x] [ ] **`senses[]` → typed sense+range** — Darkvision 60ft, Truesight 120ft (T7 kapandı).
- [ ] [x] [ ] **`passive_perception` → passive Wis(Perception)** — Stealth karşıtı.
- [ ] [x] [ ] **`language_refs[]` → comprehension** — Hangi dilleri anladığı.
- [ ] [x] [ ] **`telepathy_ft` → telepathy range** — Mind link mesafesi.
- [ ] [x] [ ] **`cr` → XP + difficulty** — Encounter difficulty input (s.202).
- [ ] [x] [ ] **`xp` → death award** — Monster öldüğünde party'ye verilen XP.
- [ ] [x] [ ] **`proficiency_bonus` → save/skill PB** — CR'den derive (mechanics §17.2).
- [ ] [x] [ ] **`trait_refs[]` → passive abilities** — Typed trait listesi (T8 kapandı).
- [ ] [x] [ ] **`action_refs[]` → action economy** — Action slot'undaki creature-action listesi.
- [ ] [x] [ ] **`bonus_action_refs[]` → BA economy** — BA slot.
- [ ] [x] [ ] **`reaction_refs[]` → reaction economy** — Reaction slot.
- [ ] [x] [ ] **`legendary_action_uses` + `legendary_action_refs[]` → LA havuzu** — Round başına LA budget reset.
- [ ] [x] [ ] **`lair_action_refs[]` → init 20 lair trigger** — Lair'da init 20'de tetiklenir.
- [ ] [x] [ ] **`spell_refs[]` → spell list** — Typed spell relation (T4 kapandı); slot havuzu kendi class.spell_slots'ı veya manual.
- [ ] [x] [ ] **`gear_refs[]` → loot inventory** — Ölünce drop edilebilen eşyalar.
...
- [ ] [x] [ ] **Tüm field davranışı §2.19 Monster ile özdeş** — Animal-spesifik kural yok; ayrı slug sadece UI/filter.
- [ ] [x] [ ] **Beast type filter eligibility** — Wild Shape / Conjure Animals / Ranger Companion gibi feature'lar Animal slug'ından çekim yapar.
...
- [ ] [x] [ ] **`species_ref` → species derived'lar** — size, speed, granted_modifiers, creature_type kaynağı.
- [ ] [x] [ ] **`class_refs[]` + `class_levels` → class derived'lar** — total_level, PB, all class field consumption.
- [ ] [x] [ ] **`subclass_refs[]` → subclass features** — Parent class level eşeleşmeli; class_resources'a katkı.
- [ ] [x] [ ] **`background_ref` → background grants** — Skill+tool, origin feat, gold, equipment kaynağı.
- [ ] [x] [ ] **`alignment_ref` → lore + magic item gating** — Holy Avenger gibi alignment-locked attune'lerde gate.
- [ ] [x] [ ] **`xp` → level türevi** — xpToLevel(xp) per s.23 → total_level.
- [ ] [x] [ ] **`proficiency_bonus` → roll bonusları** — Şu an manuel; derive olmalı (§5 #2).
- [ ] [x] [ ] **`feats[]` → feat effects** — ASI L4/8/12/16 + origin feat union.
- [ ] [x] [ ] **`languages[]` → comprehension** — Common + species + background + class union.
- [ ] [x] [ ] **`tool_proficiencies[]` → tool check bonus** — `ability_mod + (PB if in list)`.
- [ ] [x] [ ] **`weapon_proficiencies[]` → attack bonus** — Attack roll'a PB ekleme.
- [ ] [x] [ ] **`armor_trainings[]` → AC + spell gate** — Untrained Heavy → disadv Str/Dex tests + no spells.
- [ ] [x] [ ] **`saving_throw_proficiencies[]` → save bonus** — `ability_mod + (PB if proficient)`.
- [ ] [x] [ ] **`skill_proficiencies[]` + `skill_expertises[]` → skill bonus** — Expertise PB×2 (s.182).
- [ ] [x] [ ] **`stat_block` → ability mods** — `floor((score-10)/2)`, cap 20 (30 max).
- [ ] [x] [ ] **`combat_stats.hp` → live HP** — 0 → Unconscious + Death Saves.
- [ ] [x] [ ] **`combat_stats.max_hp` → HP cap** — Class HD + CON formülü, Tough feat ekler.
- [ ] [x] [ ] **`combat_stats.ac` → AC** — `armor.base_ac + DEX(cap'li) + shield + misc` (§5 #8).
- [ ] [x] [ ] **`combat_stats.speed` → walk speed** — Species + class bonusları, Heavy STR penalty.
- [ ] [x] [ ] **`combat_stats.initiative` → init roll** — `d20 + DEX_mod + PB-if-Alert/Feral`.
- [ ] [x] [ ] **`combat_stats.level` → total level** — Σ class_levels.values.
- [ ] [x] [ ] **`temp_hp` → buffer** — Damage'da önce düşer; LR ends; replace no-stack.
- [ ] [x] [ ] **`death_saves_successes` / `death_saves_failures` → death save tracker** — 3 success → Stable; 3 fail → die; nat 20 → 1 HP; nat 1 → +2 fail.
- [ ] [x] [ ] **`heroic_inspiration` → reroll** — Bool; spend → reroll d20 (max 1 active).
- [ ] [x] [ ] **`hit_dice_remaining` → SR healing pool** — SR'da 1 spend → `d_size + CON` heal; LR'da +½ refresh.
- [ ] [x] [ ] **`saving_throws` table → UI rendering** — Bonus formülü save-roll'da kullanılır.
- [ ] [x] [ ] **`skills` table → UI rendering** — Skill bonus formülü check'te.
- [ ] [x] [ ] **`senses[]` → special sight** — Darkvision/Truesight vb. union; species + magic item.
- [ ] [x] [ ] **`passive_perception` → 10 + Wis(Perception) mod** — Stealth karşıtı; +5 adv, −5 disadv.
- [ ] [x] [ ] **`passive_insight` / `passive_investigation` → 10 + ability(skill) mod** — Pasif check'ler.
- [ ] [x] [ ] **`inventory[]` → eşya listesi** — weapon/armor/gear/magic-item.
- [ ] [x] [ ] **`attuned_items[]` → attuned bonuses** — Max 3 attune; granted_modifiers aktif olur.
- [ ] [x] [ ] **`cp/sp/ep/gp/pp` → currency** — Coin.value_in_gp ile convert.
- [ ] [x] [ ] **`current_conditions[]` → active conditions** — Exhaustion stack; diğerleri non-stack.
- [ ] [x] [ ] **`casting_ability_ref` → spell DC + atk** — Per-class; multiclass'ta birden fazla DC (§5 #3).
- [ ] [x] [ ] **`spell_save_dc` → enemy save DC** — `8 + PB + ability_mod(casting_ability_ref)`.
- [ ] [x] [ ] **`spell_attack_bonus` → spell attack roll** — `PB + ability_mod`.
- [ ] [x] [ ] **`spells_known[]` → spell pool** — Class.cantrips_known + spellbook (Wizard).
- [ ] [x] [ ] **`prepared_spells[]` → castable list** — LR'da yeniden seçim; cap class.prepared_spells_by_level.
- [ ] [x] [ ] **`spell_slots` → castable resource** — Combined-multiclass-table (s.26).
- [ ] [x] [ ] **`pact_magic_slots` → Warlock slot pool** — SR'da yenilenir; ayrı havuz (Pact ≠ regular).
- [ ] [x] [ ] **`class_resources` → counter pool** — Rage/Sneak Attack/Bardic Insp/Action Surge vb. (§5 #1).
- [ ] [x] [ ] **`trinket_ref` → flavor only** — Mekanik etki yok.
- [ ] [x] [ ] **`personality_traits/ideals/bonds/flaws/age/height/weight/eyes/skin/hair/appearance/backstory/allies_organizations` → UI/roleplay** — Tüm narratif alanlar; mekanik etki yok.
...
- [ ] [x] [ ] **`species_ref` / `class_refs[]` / `background_ref` → derived'lar** — PC ile aynı kaynak alanları.
- [ ] [x] [ ] **`level` → PB (single-class)** — NPC multiclass desteklenmez; tek int.
- [ ] [x] [ ] **`alignment_ref` → lore** — Mekanik etki yok.
- [ ] [x] [ ] **`attitude_ref` → social DC** — Friendly/Indifferent/Hostile Influence action DC'sini etkiler (s.10, 184).
- [ ] [x] [ ] **`location_ref` → world-map source** — UI/lore.
- [ ] [x] [ ] **`faction` → free-form** — Lore tag.
- [ ] [x] [ ] **`stat_block` + `combat_stats` + `proficiency_bonus` + `initiative_modifier` → combat numerics** — PC ile aynı semantik.
- [ ] [x] [ ] **`saving_throws` / `skills` table → bonus rendering** — PC'deki gibi.
- [ ] [x] [ ] **`resistance_refs[]` / `vulnerability_refs[]` / `damage_immunity_refs[]` / `condition_immunity_refs[]` → defenses** — Damage roll + apply-condition gating.
- [ ] [x] [ ] **`senses[]` + `passive_perception` + `language_refs[]` + `telepathy_ft` → algı** — Vision/comprehension.
- [ ] [x] [ ] **`trait_refs[]` → passive abilities** — NPC pariteyle Monster gibi.
- [ ] [x] [ ] **`action_refs[]` / `special_action_refs[]` → action havuzu** — Action + LA/Lair/Mythic.
- [ ] [x] [ ] **`equipment_refs[]` → inventory** — Stat block loot.
- [ ] [x] [ ] **`spell_refs[]` → spellbook** — Typed spell relation.
- [ ] [x] [ ] **`goals` / `appearance` / `mannerisms` / `secrets` markdown** — Narrative; mekanik etki yok.
...
- [ ] [x] [ ] **`condition_ref` → host conditions[]** — Union (Exhaustion stack, diğerleri non-stack).
- [ ] [x] [ ] **`source_entity_ref` → causality** — Hangi entity uyguladı (DM tracking).
- [ ] [x] [ ] **`duration_rounds` → expiry tick** — null = indefinite; "1 minute / dawn" duration round'a sığmaz (§5 #11).
- [ ] [x] [ ] **`save_dc` + `save_ability_ref` → save attempt** — End-of-turn veya trigger save.
- [ ] [x] [ ] **`save_frequency` → save trigger window** — none / start-of-turn / end-of-turn / when-damaged.
- [ ] [x] [ ] **`notes` → DM tracking** — Free-form not.
...
- [ ] [x] [ ] **`danger_level` → encounter pacing** — Safe/Low/Medium/High/Deadly UI helper.
- [ ] [x] [ ] **`environment` → tag** — "Forest", "Dungeon" gibi free-form.
- [ ] [x] [ ] **`parent_location_ref` → hierarchy** — Nested location ağacı.
- [ ] [x] [ ] **`plane_ref` → lore** — Material/Astral/Ethereal vb.
- [ ] [x] [ ] **`illumination_ref` → vision modifier** — Bright/Dim/Darkness sight check etkisi (s.11).
- [ ] [x] [ ] **`hazard_refs[]` → hazard exposure** — Burning/Falling/Suffocation vb. union (s.12).
- [ ] [x] [ ] **`description_long` markdown → player-visible** — UI display.
- [ ] [x] [ ] **`secrets` markdown → DM-only** — Online modda gizlenir.
...
- [ ] [x] [ ] **`location_ref` → context** — Sahnenin geçtiği yer.
- [ ] [x] [ ] **`status` → tracking** — Planned/Active/Completed/Skipped.
- [ ] [x] [ ] **`illumination_ref` → vision modifier** — Bright/Dim/Darkness.
- [ ] [x] [ ] **`travel_pace_ref` → speed multiplier** — Fast/Normal/Slow.
- [ ] [x] [ ] **`beats` markdown → outline** — DM plan notları.
- [ ] [x] [ ] **`npc_refs[]` → cast** — Sahnedeki NPC bağlantısı.
- [ ] [x] [ ] **`quest_refs[]` → tied quest** — Sahne içinde aktif quest'ler.
...
- [ ] [x] [ ] **`status` → tracking** — Not Started/Active/Completed/Failed.
- [ ] [x] [ ] **`giver_ref` → quest NPC** — UI link.
- [ ] [x] [ ] **`reward_item_refs[]` → `inventory[]`** — Typed loot auto-grant on complete.
- [ ] [x] [ ] **`reward_xp` → `PlayerCharacter.xp`** — XP sum on complete.
- [ ] [x] [ ] **`reward_gp` → `PlayerCharacter.gp`** — Gold sum on complete.
- [ ] [x] [ ] **`reward` markdown → narrative** — Typed alanlar baskın.
- [ ] [x] [ ] **`objective` markdown → player-visible** — Görev özeti.
- [ ] [x] [ ] **`secrets` markdown → DM-only** — Twist/secret motivation.
...
- [ ] [x] [ ] **`location_ref` → context** — Encounter geçtiği yer.
- [ ] [x] [ ] **`difficulty` → XP budget validate** — Trivial/Low/Moderate/High/Deadly s.202 tablosu.
- [ ] [x] [ ] **`monsters_refs[]` + `npcs_refs[]` → combat tracker** — Spawn listesi.
- [ ] [x] [ ] **`environmental_effect_refs[]` → start-of-encounter trigger** — Per-encounter aktif environment.
- [ ] [x] [ ] **`trap_refs[]` → trigger** — Encounter içindeki trap'lar.
- [ ] [x] [ ] **`setup` markdown → DM notes** — Sahne hazırlık.
- [ ] [x] [ ] **`tactics` markdown → DM-only** — Düşman taktikleri (gizli).
- [ ] [x] [ ] **`xp_budget` → difficulty doğrulaması** — Σ monster.xp hesabı.
...
- [ ] [x] [ ] **`trigger_kind` → trigger machinery** — Pressure/Tripwire/Proximity/Sound/Magical/Touch/Other.
- [ ] [x] [ ] **`trigger` markdown** — Narrative; typed kind baskın.
- [ ] [x] [ ] **`save_dc` + `save_ability_ref` → save attempt** — Tipik DEX (Reflex) save.
- [ ] [x] [ ] **`damage_dice` + `damage_type_ref` → damage** — Typed dice (T1 kapandı).
- [ ] [x] [ ] **`applied_condition_refs[]` → save-fail conditions** — Restrained, Poisoned vb.
- [ ] [x] [ ] **`detection_dc` → Perception threshold** — Search action ile DC kontrolü.
- [ ] [x] [ ] **`disable_dc` + `disable_ability_ref` → disarm threshold** — Genelde DEX (Thieves' Tools) veya INT (Investigation).
- [ ] [x] [ ] **`countermeasures` markdown → DM helper** — Iron Spike vs gibi alternatif çözümler.
...
- [ ] [x] [ ] **`poison_kind` → delivery flow** — Contact/Ingested/Inhaled/Injury (s.197).
- [ ] [x] [ ] **`save_dc` + `save_ability_ref` → save attempt** — Tipik CON save.
- [ ] [x] [ ] **`damage_dice` + `damage_type_ref` → damage** — Genelde Poison damage type.
- [ ] [x] [ ] **`applied_condition_refs[]` → save-fail conditions** — Poisoned, Unconscious vb.
- [ ] [x] [ ] **`duration_rounds` → effect süresi** — Save-fail'in geçerli kalma süresi.
- [ ] [x] [ ] **`effect` markdown** — Narrative; typed alanlar baskın.
- [ ] [x] [ ] **`cost_gp` → buy/sell** — Standart ekonomi.
...
- [ ] [x] [ ] **`trigger` markdown → narrative** — Lanetin nasıl tetiklendiği.
- [ ] [x] [ ] **`applied_condition_refs[]` → host conditions[]** — Typed condition grant.
- [ ] [x] [ ] **`granted_modifiers[]` → ongoing PC stats** — Numeric ongoing penalties (yeni eklenmiş typed alan).
- [ ] [x] [ ] **`removed_by_spell_refs[]` → cure gate** — Bu spell'lerden biri cast edilirse curse kalkar.
- [ ] [x] [ ] **`effect` markdown → narrative** — Typed alanlar baskın.
- [ ] [x] [ ] **`removed_by` markdown → narrative** — Cure açıklaması; typed list baskın.
...
- [ ] [x] [ ] **`damage_dice` + `damage_type_ref` → damage** — Heat/Cold/Fall vb. typed dice.
- [ ] [x] [ ] **`applied_condition_refs[]` → save-fail conditions** — Exhaustion, Frightened gibi.
- [ ] [x] [ ] **`granted_modifiers[]` → ongoing PC stats** — Sürekli mekanik etki (typed yeni alan).
- [ ] [x] [ ] **`save_dc` + `save_ability_ref` → save attempt** — Uygun ability save (genelde CON/STR/DEX).
- [ ] [x] [ ] **`effect` markdown → narrative** — Typed alanlar baskın.
...
- [ ] [x] [ ] **`skill_ref` → uzmanlık** — UI: hireling specialty.
- [ ] [x] [ ] **`daily_cost_cp` → günlük gp drain** — Hire/day → currency düşer.
- [ ] [x] [ ] **`skilled` → kategori flag** — Skilled (≥2 sp/day) / Untrained (~2 sp).
...
- [ ] [x] [ ] **`kind` → grouping** — Spellcasting/Transport/Shelter/Other.
- [ ] [x] [ ] **`cost_cp` → currency drain** — Use action'da PC.cp..pp düşer.
- [ ] [x] [ ] **`availability` → UI hint** — "Cities only" gibi şart metni.
...
- [ ] [x] [ ] **Adım 1: Class assignment kaynak akışı** — class_refs → hit_die/save/skill/weapon/armor/tool/equipment/gold/casting_ability/feature_table[1] tüm propagasyonları.
- [ ] [x] [ ] **Adım 2: Origin (background + species) akışı** — Background skills/tools/ASI/feat/gold + Species size/speed/languages/senses/resistances/skills/traits.
- [ ] [x] [ ] **Adım 3: Ability scores akışı** — Standard/Random/Point Buy + ASI'den stat_block + ability_mod ile saving_throws/skills/AC/init/HP/spell_DC propagasyonu.
- [ ] [x] [ ] **Adım 4: Equipment + inventory akışı** — Class.starting_equipment + Background.default_inventory PC.inventory'ye union; Armor.equip → AC formula tetikleyicisi.
- [ ] [x] [ ] **Adım 5: Final detaylar** — total_level=1 → PB=+2; combat_stats.hp = max_hp; passive_perception; spell_save_dc/atk (caster); spell_slots/cantrips_known.
...
- [ ] [x] [ ] **xp → level threshold check** — xpToLevel(xp) > current_level olunca level-up tetikle.
- [ ] [x] [ ] **class_levels[<chosen>] += 1 + total_level recompute** — Class seçimi, total_level Σ, PB güncelleme.
- [ ] [x] [ ] **PB propagation** — saving_throws/skills/attack/spell DC rendering yenilenir.
- [ ] [x] [ ] **HP artışı (rolled veya fixed)** — hit_dice_remaining +1; max_hp += avg(d_size)+CON.
- [ ] [x] [ ] **Class.feature_table[N] aktivasyonu** — Yeni level satırındaki feature/resource'lar class_resources'a eklenir.
- [ ] [x] [ ] **Subclass selection trigger** — Class.feature_table[N]'da subclass row varsa subclass_refs seçimi açılır.
- [ ] [x] [ ] **ASI / Feat seçimi** — Level-up satırında ASI varsa stat_block veya feats[] güncellemesi.
- [ ] [x] [ ] **Subclass.features aktivasyonu** — Subclass.granted_at_level ≤ N ise subclass features class_resources'a eklenir.
- [ ] [x] [ ] **Long-rest spell_slots/pact/HD/exhaustion/prepared/temp_hp refresh** — Tüm post-rest tetikleyicileri.
...
- [ ] [x] [ ] **Slot gate** — Spell.level vs PC.spell_slots[≥]; cantrip slot harcamaz; ritual path opsiyonel.
- [ ] [x] [ ] **Action economy** — casting_time_amount + casting_time_unit_ref → Action/BA/Reaction/Min/Hour slot tüketimi; Reaction → reaction_trigger gerek.
- [ ] [x] [ ] **V/S/M satisfy** — V (silenced değil), S (free hand), M (focus/pouch/specific item); material_consumed → gp düş.
- [ ] [x] [ ] **Targeting** — range_type + range_ft + area_shape_ref + area_size_ft AoE coverage hesabı.
- [ ] [x] [ ] **Attack roll branch** — attack_type Melee/Ranged → spell attack roll = `PB + spell_mod + bonuses`.
- [ ] [x] [ ] **Save roll branch** — save_ability_ref → target.save_bonus vs PC.spell_save_dc; success → save_effect uygula.
- [ ] [x] [ ] **Damage resistance check** — damage_type_refs vs target.resistance/vulnerability/immunity; ½ / ×2 / 0.
- [ ] [x] [ ] **Concentration management** — requires_concentration=true → mevcut concentration drop; damage'da DC 10 ya da ½ dmg save.
- [ ] [x] [ ] **Active effect tracker** — duration_unit_ref + duration_amount → AppliedCondition veya active-effect entry duration_rounds takibi.
...
- [ ] [x] [ ] **#1 ✅ `Class.features` / `Subclass.features` `classFeatures` typed list** — Rage uses, Sneak Attack dice, Bardic Inspiration die, Extra Attack count typed encode. (Önceden levelTable Map<int,int>.)
- [ ] [x] [ ] **#2 `PlayerCharacter.proficiency_bonus` manuel int** — `total_level`'dan derive edilmeli. Multiclass §1.8 zorunluluğu (PB total character level'a göre). Şu an cache; level-up'ta güncellenmesi convention.
- [ ] [x] [ ] **#3 `PlayerCharacter.spell_save_dc` / `spell_attack_bonus` tek skaler** — multiclass'ta iki caster için iki ayrı DC olur (Cleric WIS, Wizard INT). Schema tek tutuyor. Deferred: per-class derived map.
- [ ] [x] [ ] **#4 `Class.spell_slots_by_level` per-class ama multiclass slot havuzu combined table gerektirir** (s.26). Cached field yok; resolver runtime'da Σ over caster classes hesaplamalı. Pact slots `pact_magic_slots`'ta ayrı.
- [ ] [x] [ ] **#5 `Class.saving_throw_refs` first-class-only kuralı** — SRD-mandated (s.25) ama şemada kodlu değil. "Yalnız PC'nin starting class'ı save prof verir" kuralı convention; resolver bunu zorlamalı.
- [ ] [x] [ ] **#6 ✅ `Species.traits`, `Feat.benefits`, `MagicItem.effects`, `Trait.description` markdown blob** — typed `granted_modifiers[]` DSL ile çözüldü (Tier-B). 24 modifier kind tüm bonus tipi taşır; markdown narrative kalır.
- [ ] [x] [ ] **#7 Multiclass starting proficiency tablosu (s.25)** — `Class.multiclass_requirements` markdown'da; data değil. "Barb multiclass'a girince sadece Shields + Martial weapon training" subset kuralı kodlanamaz.
- [ ] [x] [ ] **#8 `PlayerCharacter.combat_stats.ac` serbest int** — armor formülü kodlu değil (`base_ac + DEX [+cap] + shield + misc`). Unarmored Defense (Barb=10+DEX+CON, Monk=10+DEX+WIS) seçim mekanizması yok; non-stack multiclass kuralı advisory.
- [ ] [x] [ ] **#9 `Spell.attack_type` enum {None,Melee,Ranged}** vs **`creature-action.attack_kind` enum {Melee Weapon, Ranged Weapon, Melee Spell, Ranged Spell}** — iki vokabüler. Reconcile gerek veya mapping function.
- [ ] [x] [ ] **#10 `MagicItem.charges_max + charge_regain (text)`** — regain free-form ("1d6+4 at dawn"). Long-rest engine auto-restore edemez; parser yazılana kadar manuel.
- [ ] [x] [ ] **#11 `AppliedCondition.duration_rounds int (null=indefinite)`** — Exhaustion stack ve "1 minute / until next dawn / until cured" duration'lar lossy. Round-based tek granularity; minute/hour/day → round (1 min = 10 rounds).
- [ ] [x] [ ] **#12 `Background.ability_score_options` sadece ability listesi** — "+2/+1 vs +1/+1/+1" choice (s.83) kodlu değil; dağıtım PC.stat_block'ta serbest. UI seçim sunmalı + max-20 cap zorlamalı.
- [ ] [x] [ ] **#13 `PlayerCharacter.class_levels` `levelTable` (Map<int,int>)** — implicit key class_id (UUID), value level. "level" değil. Type aliasing convention; FieldType naming yanıltıcı.
- [ ] [x] [ ] **#14 `Animal` slug Monster duplicate** — şema seviyesinde `_animalCategory()` Monster field listesini `copyWith` ile rebuild eder. Field semantikleri özdeş; UI/import/codec ayrı slug filter için ayrı.
