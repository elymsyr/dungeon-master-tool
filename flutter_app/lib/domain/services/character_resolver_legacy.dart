...
class LegacyCharacterResolver {
  ...
  static EffectiveCharacter resolve(
    ...
    // ── 10. Prerequisite validation (warn-keep; roadmap 1.1) ────────────
    // Every grant the character carries is re-checked against the FINAL
    // accumulators (all proficiency/ability passes done), so a retroactive
    // ability edit, an imported character, or a sync from another client
    // surfaces "no longer qualifies" warnings. Policy is WARN-KEEP: the
    // mechanics above already applied; nothing is rolled back. The same
    // clause interpreter (`rules/prereq_evaluator.dart`) drives the picker
    // dialogs' candidate filtering, so the two surfaces can't drift.
    final unmetPrerequisites = <UnmetPrerequisite>[];
    {
      String lowerNameOf(String idOrRaw) =>
          (entitiesById[idOrRaw]?.name ?? idOrRaw).toLowerCase();
      // Proficient skill NAMES: resolver-granted skill ids + the PC's own
      // `skills` proficiency-table rows (the dialog historically compares
      // names from that table).
      final skillNames = <String>{
        for (final id in skills)
          if (entitiesById[id] != null) entitiesById[id]!.name,
      };
      ...
      if (rawSkills is Map) {
        ...
        if (rows is List) {
          for (final r in rows) {
            if (r is Map && r['proficient'] == true && r['name'] is String) {
              skillNames.add(r['name'] as String);
            }
          }
        }
      }
      final hasSpellcasting = grantedSpellIds.isNotEmpty ||
          grantedCantripIds.isNotEmpty ||
          _readStringList(fields['spells_known']).isNotEmpty ||
          _readStringList(fields['prepared_spells']).isNotEmpty ||
          classLevels.keys.any((cid) {
            final ck = entitiesById[cid]?.fields['caster_kind'];
            return ck is String && ck.isNotEmpty && ck.toLowerCase() != 'none';
          });
      final ctx = PrereqContext(
        characterLevel: classLevels.values.fold<int>(0, (a, b) => a + b),
        abilityScores: abilities,
        hasSpellcasting: hasSpellcasting,
        proficientArmorCategoriesLower: {
          for (final v in armorCats) lowerNameOf(v),
        },
        proficientWeaponClassesLower: {
          for (final v in weaponCats) lowerNameOf(v),
        },
        proficientSkillNames: skillNames,
        classLevelsById: classLevels,
        speciesId: raceId,
        alignmentId: _resolveRef(fields['alignment_ref'], entitiesById),
      );
      // Chosen feats only — auto-granted feats are class/species features the
      // content itself bestowed; they carry no player-side gate.
      final checkedFeatIds = <String>{};
      for (final fid in featIds) {
