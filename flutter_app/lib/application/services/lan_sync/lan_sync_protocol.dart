...
import '../content_transfer/content_item.dart';
export '../content_transfer/content_item.dart';
...
LanSyncPlan diffManifests({
  required List<ContentItemRef> local,
  required List<ContentItemRef> peer,
}) {
  final localByKey = {for (final r in local) r.key: r};
  final peerByKey = {for (final r in peer) r.key: r};

  final pull = <ContentItemRef>[];
  final push = <ContentItemRef>[];
  var skipped = 0;
