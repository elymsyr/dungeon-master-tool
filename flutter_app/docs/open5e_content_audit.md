...
first name wins. **457** child rows across 7 packs now render under another
...
*(Corrected 2026-08-18 by the `tob3` unit: the original recipe reads v2 rows
only, and `tob3`'s 1,370 actions are v1-derived. The same parent-scoped test on
that side finds **105** more swallowed names — mostly the usage counter,
`Cast a Spell (2)` → `Cast a Spell` — so `tob3`'s share is **128**, not 23, and
the corpus total **487**, not 382. `_cleanChildName` does not strip those
suffixes; the loss is `_ensureChild`'s, i.e. the same defect.)*
*(Corrected again 2026-08-19 by the `a5e-mm` unit: 30 of that pack's 106 hits
are an artefact of matching an **empty** `desc` against the corpus's one
empty-description entity, so its share is **96**, not 126, and the corpus total
**457**.)*
...
`tier0Slugs`). `tob3` re-read the first constant a fourth time
(`CreatureTrait.type` null on 1,230/1,230) and could not test the second at all
— v1 `legendary_desc` is empty on 397/397 there, so the constant 3 on 21 rows is
neither confirmed nor refuted, which is exactly what D2's declared rule says.
Its own K7 items: `tags_line` (v1 `subtype` filled on exactly 1/397, the pack's
rate to the row), the six 0% fields (v2 `hover` and `telepathy_range` empty, no
`LAIR_ACTION` row, v1 `spells_json` the literal string `"null"` on all 397,
`group` empty), `alignment_ref` 378 + `alignment_note` 19 = 397/397 with **0**
`tob3` rows in `unmapped_report`, the 13 `Npc: …` names (the recovery index
joins on the **raw** name, so all 13 keep their actions), and Berberoka — the
one actionless creature, actionless in v1 and v2 alike, which `gate_packs`
passes because it has a bonus action. The blank attack columns are B8's written
gap (`S`) and stayed out of the ledger; `is_attack` did not — see F-pass0-25.
...
| `creature-action.is_attack` (the false negative) | ⚠️ | **New row, F3/`tob3` 2026-08-18.** The field is derived from the *existence* of a `CreatureActionAttack` row, so "no attack fixture" is written as the positive claim "not an attack". `tob3` ships no attack fixture at all → `false` on 1,577/1,577 (`audit_packs`: `⚠ const`), **634** of which begin *"Melee Weapon Attack: +N to hit"*; corpus-wide **681** rows in 5 packs. Distinct from the four attack columns being empty — those are honest blanks with a written reason. See **F-pass0-25** |
| `monster.action_refs` (the partial v2 bucket) | ⚠️ | **New row, F3/`tob3` 2026-08-18.** B8's v1 recovery only fills a bucket v2 left **entirely** empty, and its write-up puts the cost at "one monster: Abaasy". Measured by text across the corpus the cost is **11 actions on 10 creatures** (`tob-2023` 5, `tob` 3, `tob3` 3) — the Red Hag has no `Multiattack`, the Tosculi Hive-Queen's *Glitter Dust* survives only as its legendary shortcut. B8's case against the loose name-based rule is unaffected. See **F-pass0-24** |
...
- [x] **F3 — Run it.** 20 scan units in four waves. **done 2026-08-19.**
      ...
      **(2026-08-17): 7 / 20** — Pass 0, Dalga 0 (built-in), Dalga 1's
      ...
      held **17** findings then). `open5e` (22 entities) produced the first pack-owned
      ...
      **`spells-that-dont-suck` (180 spells, 2026-08-18)** — 1,678 ok /
      0 disagree / 0 absent / 0 unsourced / 253 unverifiable at 180/180, gate
      green, census 0, no catalog drift; 31 items at
      **25 ✅ · 6 ➖ · 0 ⛔ · 3 ⚠️**, the wave's cleanest unit (all 11 required
      fields 180/180, `class_refs` complete, 22 of 25 schema fields filled).
      **F-spells-that-dont-suck-01** (A3, `M`): 8 cards write
      `Self (60-foot radius)` / `Self (10-foot dome)` / `Self (1-mile radius)`
      and the card keeps neither number nor shape — `_range` stops at the `self`
      keyword and the source's numeric `range` column is `0` there; no other
      document writes `Self (N …)` at all. **F-spells-that-dont-suck-02**
      (A3, `S`): 5 cards ship `material_cost_gp: 0` while their own
      `material_description` names a price (`…worth at least 665 gp`) — the
      source column is `'0'` on those rows and a real number on 18 others, so
      `0` means "not entered". This is the first unit where `material_*` was
      measurable (81/180); `deepm` shows the harmless shape (column `null`, the
      field never written, **0** zero-cost cards of 288). **F-pass0-15** (C8,
      `A`, spreading): §5.8's ⚪ rationale for `spell.effects` rests on two
      reasons and the second — "no structured damage rows upstream" — is
      measurably wrong: `damage_roll` is filled on **303 of 1,297** shipping
      spells (285 plain dice). The ⚪ verdict survives on the reader half; the
      written reason is what needs correcting, the same shape as
      `monster.lair_action_refs`. All four inherited expectations held
      (F-pass0-11 → 1, F-pass0-12/13/14 → 0), the rule-applied `desc` check came
      out 180/180 with 87 `higher_level` rows in play, and
      `SpellCastingOption.json` survived a third reading on a **changed**
      ...
      **326** monster names but **254** of them differ numerically — two
      ...
      `tob3` (2026-08-18, unit 18/20) read 2,787 entities — **7,757 ok /
      0 disagree / 0 absent / 0 unsourced / 1,985 unverifiable**, 397/397
      coverage, gate green, census 0, no catalog drift, **19 ✅ · 7 ➖ · 1 ⛔ ·
      4 ⚠️**, and its own pack counter stays **0**. It is the one pack whose
      actions come from v1: v2 ships 309 `CreatureAction` rows for 397 creatures
      and only **2** are `ACTION`, so B8's recovery supplies the rest. Every
      child count was re-derived by re-running the recovery rule and matched the
      pack exactly (1,370 / 136 / 96 / 75 actions, 1,230 traits — 0 lost rows).
      Inherited records: F-pass0-19 → 12, -20 → 123, -21 → 1 held, and -18, -22,
      -23 measured **0** (all three checks, not assumptions). **F-pass0-17 was
      corrected**: its recipe reads v2 only, so `tob3` looked like 23; applied
      parent-scoped to the v1-derived rows it is **128**, corpus total **487**.
      Two new records, both `pass0`: **F-pass0-24** (C4, `M`) — B8's recovery
      skips a bucket that is merely *partial*, and its "entire cost is one
      monster" line is short by 8: measured by text, **11 actions on 10
      creatures** (`tob-2023` 5, `tob` 3, `tob3` 3) ship nowhere, including the
      Red Hag's `Multiattack`; **F-pass0-25** (A3, `M`) — `is_attack` is derived
      from an attack row's *existence*, so `tob3` (no attack fixture at all)
      writes `false` on 1,577/1,577, **634** of them on rows whose text is an
      attack; **681** corpus-wide in 5 packs. Measured and not filed: the four
      empty attack columns (B8's written gap, `S`), `tags_line` 1 = v1 `subtype`
      1, five more 0% fields empty at source, `alignment_ref` 378 + note 19 =
      397/397, `trait_kind` null on 1,230/1,230, `unmapped_report` 0 rows,
      B2 clean (`monster` name overlap with every other pack is **0**), and
      Berberoka, the single actionless creature, actionless upstream too.

      `a5e-mm` **plus `bfrd`'s monster rows (2026-08-19, unit 19/20)** was the
      wave's largest and its only two-pack unit — 946 monsters, 5,541 entities.
      **16,760 ok / 0 disagree / 0 absent / 0 unsourced / 4,730 unverifiable**
      at 586/586 + 360/360, gate green, census 0, no catalog drift, both packs
      absent from `--list-builtin-same` and from `unmapped_report.json`;
      31 items at **19 ✅ · 7 ➖ · 1 ⛔ · 6 ⚠️**, the scan's most-warned unit,
      four of the six inherited. Its two new records are **F-a5e-mm-01** (C4,
      `M`) — upstream's statblock segmenter put the rule in `name` and its
      continuation in `desc`, so **57 rows on 41 monsters ship nowhere**
      (19 of them mechanical: Gelatinous Cube's escape DC, Medusa's DC 14
      petrification, Dread Knight's wall), and the 5 rows whose `desc` is
      empty collapse into a single **text-less** `Luck` card that 3 monsters
      reference — which is exactly why `creature-action.description` reads
      2,992/2,993 — and **F-pass0-26** (A5, `S`) — `alignment_ref` is
      `Chaotic Evil` on **946/946** monsters because the source column has
      collapsed to one value in these two documents alone (the other eight
      carry 10–21 distinct values), so Pixie, Unicorn and Elk all ship as
      chaotic evil.
      **Three older records were corrected by measurement.** F-pass0-17's
      `a5e-mm` share is **126 → 96** and the corpus **487 → 457**: the
      loose ⟷ strict gap (106 ⟷ 85) is not `bonus_action_refs` as the handover
      guessed but the **empty `desc`** — the recipe matches on text, and an
      empty text matches the one empty-description entity in the pack; on
      non-empty rows both methods give **76 ⟷ 76**, like every other monster
      pack. **F-tob-01 is not `tob`-specific**: `a5e-mm` has no v1 document and
      states the count in a v2 `CreatureAction.name` (*"The aboleth can take 2
      legendary actions"*), where **16 of 66** declarations contradict the
      card's constant 3 — corpus **3 → 19**. And `bfrd`'s child side was
      measured **lossless** (2,519/2,519 source rows on the card, 6 of them via
      L1 soft refs). All four handover warnings held on independent
      re-measurement: F-pass0-23 → 90 `keensense` rows and exactly **41**
      senseless cards, F-pass0-20 → 189 + 77, F-pass0-25 → 32 + 0,
      F-pass0-22 → 0 + 58, F-pass0-24 → 0; F-pass0-18 (828 + 512), -19
      (87 + 27 / 24 + 6) and -21 (60 + 37) too.
      **Seven candidates died**, six of them to question 3 or 7: `hp_dice`
      586/946 (`bfrd`'s `hit_dice` column is empty on 360/360 — B11 working),
      `tags_line` 74/946 (= `blackflag`'s v1 `subtype`, exactly; `a5e-mm` has
      no v1), `alignment_note` 0/946 (nothing to note — `alignment_ref` is
      full), the 3 action-less monsters, `bfrd`'s `game_system: 5e-2014`
      (the source's own `Document.gamesystem`), and the seven 0% fields that
      all carry a written §5.8 cause. The seventh gave the unit's own lesson,
      now question **8** on the board: `Nimble Escape` looked lost until the
      soft-ref path was counted — L1 leaves the built-in's copy in place and
      writes a `{slug,name}` ref, so any loss measurement that reads only hard
      refs over-counts (6 rows in `bfrd`, 9 in `a5e-mm`).
      **B2's real exam pair came out the same way `tob` ⟷ `tob-2023` did**:
