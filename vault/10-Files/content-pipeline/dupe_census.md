...
- **`creature-action` / `trait` rows are monster-owned children, not library entities.** `_collectOwnedIds` walks the five `monster` child-ref lists (`action_refs`, `bonus_action_refs`, `reaction_refs`, `legendary_action_refs`, `trait_refs`) and marks every id they reach; collapsing those by name reassigns another creature's text to this creature. **15,038 of 20,712 entities (72.6%) are owned rows, and 3,153 of the 4,331 redundant copies are** — which is why the census's headline is now **actionable redundancy 1,178 (5.7%)** and both categories report 0. `--list` marks them `[owned]`. See [[Package-Links]] and audit doc §2.5.
...
## Notes
- **Run 2026-07-30 (post-L0):** 19 packs / 20,712 entities vs 2,717 built-in rows (2,719 before folding case — two built-in rows differ only in case) → 4,331 redundant (20.9%) of which 3,153 monster-owned → **actionable 1,178 (5.7%)**. A = 1,660 (7 same text / 1,630 name-only / 23 no-text), B = 2,540 names / 3,393 copies. Section C = 135 softRefs (105 → built-in, 29 → own pack, **0 → another bundled pack**, **1 dangling**).
