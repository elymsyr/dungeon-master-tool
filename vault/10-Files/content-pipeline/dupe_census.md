...
- `--list-shared` prints **section B's link candidates**, in the three buckets the decision actually needs (audit **L2**, 2026-08-13): identical text with no statblock owner (the only real candidates), identical text but owned, and *no text on any copy yet an identical card body*.
...
- **What the census decided for L2 (2026-08-13).** Section B's 189 identical-text names are **188 statblock-owned + 1**: `language` "Void Speech" in six packs. Collapsing that one means five packs declaring `metadata.links → open5e-tob`, and `FirstPartyInstallNotifier.install` walks `requires` transitively *before* installing — 2.9 MB of Tome of Beasts pulled in to avoid a one-row card, and a failed dependency fails the whole install. **Kept duplicated on purpose**; `emit.assemblePack` therefore still writes no `links` key and every catalog entry's `requires` stays `[]`. The link machinery is complete and has no producer.
