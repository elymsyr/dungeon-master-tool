...
- **`spells.dart` derives three fields from each spell's own description (audit T2-3, 2026-08-14) rather than from a parameter.** `_areas` is a `name → (area-shape, size ft)` table feeding `area_shape_ref`/`area_size_ft` (**72/341** — 64 from prose, 8 from the pinned snapshot's `shape_type` where the prose is silent; Cylinder stores the radius, Line the length; `Wall of Force`, `Private Sanctum`, `Antipathy/Sympathy` are recorded non-areas). `_reactionTriggers` covers all **4** Reaction-cast spells. `_upcastHeading` lifts the upcast clause verbatim into `at_higher_levels_text` as a single entry keyed at `level + 1`, with `_upcasts` harvested from the pinned snapshot's `higher_level` covering the rows this file's abridged prose dropped and appended to their description so field and card agree (**109/341**; prose wins where both exist) — the SRD states upcast as a formula, not a table, and Cantrip Upgrade is excluded because it scales on character level. **Measuring a fill against this file's own prose only proves the prose is self-consistent** — the abridging drops SRD clauses (`Chromatic Orb`, `Blight`), so coverage is measured against the pinned snapshot (`d4276c58`, `srd-2024/Spell.json`), the same source T2-2 used. `test/domain/srd_core_spell_fields_test.dart` re-reads every description to prove nothing was invented, and sweeps the other way so a new spell with an area but no shape fails.
...
- **`monsters.dart` / `animals.dart` carry the SRD save + skill lines (R7, 2026-08-20).** The MOD/SAVE columns and the "Skills" line of **336** SRD 5.2.1 statblocks were transcribed onto the 345 creature cards (**334** matched by name) through `saves([...])` / `skills({...})` ([[srd_helpers]]): **123** cards write `save_bonuses`, **220** write `skill_bonuses`. The remaining matched cards write neither **on purpose** — their statblock prints no such line, so the schema's all-unproficient default is the correct value. Proficiency flags are derived from the *source's* own MOD↔SAVE difference, so they stay faithful on the 22 cards whose scores still disagree with SRD 5.2.1 (and Pirate Captain's PB); `misc` carries the four remainders PB cannot explain. Guarded by `test/domain/srd_core_save_skill_test.dart`; 11 cards (Beholder, Mind Flayer, Drow, …) have no SRD counterpart at all.
