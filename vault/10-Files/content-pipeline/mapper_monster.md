...
- **`_ensureChild`'s content-hash dedup is the model to copy, not the name-based dedup the audit once proposed for whole packs.** Within a pack it shares a child only when the text is byte-identical and otherwise mints `Name (Creature)`; measured over the shipped assets that yields 905 legitimately-shared rows and **zero orphaned** `creature-action`/`trait` rows. Across packs the same names are 90% *different* text, which is why [[dupe_census]] section B is not a deletion list for these categories. The qualified names it mints are also why a cross-pack softRef must never rely on [[entity_ref]]'s qualifier-stripping fallback.
