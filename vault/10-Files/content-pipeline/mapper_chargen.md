...
- `_addUnique` disambiguates same-slug name collisions (3rd-party docs reuse generic subclass/feat names) by suffixing the parent tag or a counter. **It returns the name it registered** — the caller needs that to aim a `_ref`, which the original name would miss whenever the suffix fired.
- **`_mintFeatureFeats` — a feature becomes a card (audit B4, 2026-09-12).** Every grant path is ref-based (`CharacterResolver` Pass 4b, the editor's `absorbFeatureRowsInRange`), so a `features` row carrying only `{level, name, description}` put **nothing** on the sheet: 82 of the 117 bundled subclasses, and every third-party base class, were inert while their rules sat in prose nobody rendered. Each named feature now mints a class-feature `feat` (`Subclass Feature` / `Class Feature` category, prose in `benefits`) and the row that *first* grants it carries `granted_feat_refs`. Only the lowest-level row gets the ref — a feature improving at 6 and 14 is one card, or the resolver applies it twice. Skipped for a spell-table row (`_isSpellTableRow`) and for a feature with no prose (an empty card helps nobody). The prose still names no *mechanic* to type; what changed is that the feature is reachable. 745 refs, 0 dangling. Tests: `test/tool/class_feature_levels_test.dart` (mapper side), `bundled_pack_resolve_test` → *a third-party subclass reaches the sheet* (asset side).
- **`siblingGrantLevel` — the featureless subclass (F-toh-02, 2026-09-12).** A document can ship a subclass with zero `ClassFeatureItem` rows, so `granted_at_level` stayed absent and both readers defaulted it to **1** — a Barbarian path openable at level 1. The level is now taken from the subclass's *siblings* under the same parent (in 5e every path of a class shares it): derived from the data, not invented, and a parent whose every subclass is featureless still gets nothing.
...
- **One subclass has no features of its own**: `Path of Hellfire` (`open5e-toh`) ships a `CharacterClass` row and zero `ClassFeature` rows upstream, so it is the single bundled subclass that grants nothing — a source hole, not a conversion failure, and named in `bundled_pack_resolve_test`'s `knownFeatureless` so a *different* subclass going bare still fails. Since 2026-09-12 it does at least open at the right level (`siblingGrantLevel`, above).
