...
- `metadata` keys: `title`, `publisher`, `license`, `attribution`, `game_system`, `source`, `source_doc_slug`, `pack_version` (the `packVersion` const, **`2.1.0`** since 2026-09-12 — B4's class-feature cards), `source_data_rev`, `is_srd_overlap`, `counts`.
...
- **`writeManifest` keeps the entries it did not produce (2026-09-12).** `assets/open5e_packs/` can be written by more than one pipeline (the Cairn packs, removed 2026-09-14, came from `tool/content/cairn/build_cairn.dart`), and the plain overwrite silently evicted those entries from the manifest on **every** `build_packs` run. The files stayed on disk, so nothing failed; they just stopped being discovered. A prior entry is carried over only while its `.pkg.json` is still present, so a pack that is genuinely gone still leaves the manifest. The file also gets a trailing newline now, so a rebuild stops flip-flopping the last line.
