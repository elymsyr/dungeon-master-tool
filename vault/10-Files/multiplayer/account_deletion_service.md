...
- Bucket taraması **recursive + sayfalı**: ilk hâli tek seviye tarıyordu ve `shared-payloads` içindeki marketplace payload'larını (`{uid}/listings/{id}.json.gz`) hiç görmüyordu — `list` klasörü `id == null` bir girdi olarak döndürüyor, `remove` ona dokunmuyor, objeler sahipsiz kalıyordu. `_purgeStorageFolder` klasörleri ayırt edip içine iniyor.
- **Sayfalama (güvenlik düzeltmesi):** `SearchOptions.limit` varsayılanı **100** (`storage_client/types.dart`) ve `list()` onu gövdeye koyuyor — belirtilmediği sürece 100'den fazlası hiç listelenmiyor, dolayısıyla hiç silinmiyor ve **hata da alınmıyordu**. Hesap satırı düştükten sonra o objelerin owner-scoped RLS'i artık hiçbir JWT ile eşleşmediği için kalıcı olarak erişilemez hâle geliyorlardı. Artık `limit: 1000` ve "her turda baştan oku, dosya kalmayana kadar sil" döngüsü (sildikçe offset kaydığı için offset'le sayfalanamaz; klasörler sanal, altları boşalınca listeden düşer). `_storageMaxPasses` aşılırsa sessiz döngü değil `StateError`.
- **Eksik silme artık sesli:** `remove()` yalnız gerçekten sildiklerini döner — RLS bir objeyi engellerse 200 + eksik liste gelir, exception gelmez. Dönen sayı istenenden azsa `StateError`.
- **Boş `DMT_WORKER_URL` artık hata:** eskiden sessiz `return`'dü ("offline build, R2 kullanılmıyor"). Buraya gelindiğinde `SupabaseConfig.isConfigured` zaten true, yani bu online bir build ve tek eksik define; aynı hesap daha önce worker URL'li bir build'den (CI release) obje yüklemiş olabilir. Sessizce atlanırsa `delete_my_account()` çalıştıktan sonra o objeler **kalıcı olarak** yetim kalır — `/admin/purge-user`'ın kullanıcı yolu `sub == user_id` istiyor ve o `sub` için artık token üretilemez.
