...
Hesap silme ([[account_deletion_service]]) yerel ağacı silmiyor, misafir köküne **geri veriyor** — terfinin simetriği. Üç adım: (1) `retireClaimedGuestTree()` kökteki bayat kopyayı arşive alır **ve talebi düşürür** — bu sıra zorunlu, aksi halde `deactivate()`'in ikinci emekliliği az önce taşınan taze veriyi arşivler; (2) `db/` (WAL çifti dahil) + medya alt ağaçları köke taşınır, üzerine hiçbir zaman yazılmaz — kökte talep edilmemiş bir DB duruyorsa o da arşive park edilir; `.v12_cut_applied` işaretçisi taşınan DB'nin yanına yazılır, yoksa bir sonraki açılış onu pre-v12 sanıp `.legacy`'ye alır (O4'ün bulduğu hatanın aynısı); (3) kalan hesap kökü (`cache/`, sentinel'ler) silinir. Talep ve nesil düştüğü için alan **yeniden devredilebilir** hale gelir. Testler: `the account goes away but the device keeps the work` grubu (3 vaka).

Bir dördüncü adım taşımanın **öncesinde**, DB hâlâ açıkken koşar: `releaseAccountCharacters(db, uid)` — `claimGuestCharacters`'ın simetriği, `world_characters.owner_id`'yi NULL'lar. Karakter sekmesi own-only (`owner_id == uid`, ya da oturum kapalıyken `owner_id == null`), dolayısıyla silinmiş bir uid'e ait satırlar diskte durup hiçbir ekranda görünmüyordu — rapor: "world ve package kaldı ama karakterler gitti". Serbest bırakma anlamıyla karışma riski burada yok: sahip hesap artık mevcut değil. Çağrı [[account_deletion_service]] `finishAccountDeletion` içinde.
