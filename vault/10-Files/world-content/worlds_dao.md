...
- Used by: [[world_repository_impl]], [[campaign_provider]], [[world_mirror_service]], `world_reconciler.dart` (kaldırıldı), [[content_codec]] (`.dmtz` import)
- Domain map: [[World-and-Content]]
- System flow:
- Spec / reference: [[Data-Layer]]

## Key Logic / Variables
- `setUpdatedAt(id, ts)` — `.dmtz` import ([[content_codec]]) için: uygulanan item'ın `worlds.updated_at`'ini kaynağınkine sabitler. (`setRenamedAt` Faz 6'da silindi — çağıranı yoktu.) Repository `save()` her yazımda `now()` bastığı için restamp olmadan çekilen içerik anında geri push edilirdi.
