...
- Used by: `main_screen.dart` ve `package_screen.dart` (açıkken keep-alive `ref.watch`), [[save_sync_indicator]] (dünya/paket ilk yayınında `full: true`), `world_mirror_provider.dart` (DM kanalının `onSubscribed` → `catchUp`, `onRevision` → `onSignal`; bkz. [[world_sync_service]]).
- Domain map: [[Sync-and-Realtime]]
- Spec / reference: `docs/online-sync-redesign.md` §4.6 (Faz 4a), §4.7 (Faz 4b), §4.8 (Faz 5a), §4.8.1 (Faz 5b).
