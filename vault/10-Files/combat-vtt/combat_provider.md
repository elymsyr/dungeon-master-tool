...
- Constructor deps (lazy closures): `entityProvider` (entity map), `worldSchemaProvider` (`EncounterConfig`), **`combatCharactersProvider`** (hub chars ∪ `world_characters` mirror), `activeCampaignProvider.notifier.data` (campaign map incl. `combat_state`), `eventBusProvider`, a `saveSettingsPatch` closure, `characterListProvider.notifier.updateInWorld` (HP write-back), and an `_isLoadWithoutDataSafe` gate closure.
...
- **Write-back reaches other players' PCs (fixed 2026-09-13).** Two closures were scoped to the DM's own hub list, so in multiplayer the whole path was a silent no-op: `_characterByEntityId` searched `characterListProvider` (which never holds another player's char — that is why `addCombatantForCharacter` exists at all), and the save closure was `characterListProvider.notifier.update`, which would have forked a hub copy of someone else's PC into the DM's Drift. The source is now `combatCharactersProvider` (the same union the projection snapshot builder uses) and the sink is `updateInWorld`: hub char → `update`, foreign char → `_mirrorPush` only. RLS allows it (`Chars: update` → `owner_id = auth.uid() OR is_world_dm(world_id)`), and the owning player picks the row up through the `world_characters` CDC branch in `world_mirror_applier` (`newOwnerId == selfUid` → `characterListProvider.applyMirror`). Side benefit: `addCombatantFromEntity`'s `isCharacter` detection is now correct for foreign PCs too.
