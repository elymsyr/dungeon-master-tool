...
- **Failure policy:** per-ref failures land in `PublishPinResult.failures` and the old ref survives (publish is not blocked). `PinnedQuotaExceededException` (pool 5 GB / publisher 500 MB) is **rethrown** — half-publishing silently would be worse than an explicit "pool full".
