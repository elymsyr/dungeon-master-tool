...
## Inputs / Outputs
**Inputs**
- Constructor deps: `AssetService` (R2 pinned upload), `AssetRefResolver` (turns any ref — local, cached, public, content, counted cloud — into a `File`).
...
## Key Logic / Variables
- **`isMediaRef`** is the single branch point (public so the test can hit it): pins local image paths (needs a separator + known image ext), `dmt-public://`, `dmt-content://` (dünya medyası — dünya silinince gider, yayına bağlı kalamaz), and counted `dmt-asset://`. **Skips** `dmt-art://` (first-party catalog, lives in R2 `catalog/`, no refcount) and refs already under `dmt-asset://pub/`.
