...
**Outputs**
- Public API: `schedule`, `dispose`; top-level `collectContentRefs`.
...
## Key Logic / Variables
- `collectContentRefs` gövdeyi **şemadan bağımsız** gezer: her string'e bakar, `dmt-transient://` ise `sha → ref` haritasına yazar. Yeni bir medya alanı eklendiğinde burada değişiklik gerekmez.
...
- Faz 3.5: tarayıcı `dmt-content://` **ve** eski `dmt-transient://` gövdelerini birlikte toplar (`collectContentRefs`) — ikisi de aynı sha'yı verir, `dmt-asset://` / `dmt-public://` kasten dışarıda. Bkz. [[content_ref_index]].
