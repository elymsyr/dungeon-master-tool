...
- **resource-pool fold** — max = `count_by_level` table, else `count_formula` (`cha_mod_min_1` and friends via `evalCountFormula`), else the flat `count`. `pool_ref` is **resolved to an entity id** before it is emitted (2026-08-31): the built-in map ships ids, but pack- and blueprint-sourced cards can still carry a `{_lookup|slug, name}` envelope, and the sheet's tracker only draws rows whose ref is a resolvable id — so the same class showed a Bardic Inspiration counter or none depending on where the card came from. An unresolvable value is passed through untouched.
...
1. **Raw choice reads** — typed-read the PC fields (`_readStringList`/`_readIntMap`/…). `feat_asi_choices` map `{featId:{ABBR:amt}}` records user ASI picks.
   **Two vocabularies, and the second one is a fallback.** The wizard writes id-keyed choice fields that live *outside* the player-category schema (`class_levels` keyed by class **entity id**, `race_id`, `background_id`, `subclass_id`, `feat_ids`, `base_abilities`). A PC that did not come from the wizard — a blueprint, an imported package, a LAN-synced card — can only carry the schema's relation fields, so each read falls back to its schema twin: `feats` → `feat_ids`, `species_ref` → `race_id`, `background_ref` → `background_id`, `subclass_refs[0]` → `subclass_id`, `stat_block` → `base_abilities`, and `_resolveClassLevels` maps a `class_levels` key that is a class **name** to its id via `findEntityIdByName`. Without these the card resolved with no class (`Missing class entity Fighter`), no species, no background and all-10 abilities — which also made `armorClass`, spell save DC, initiative and every skill modifier wrong. The card's own id-keyed value always wins; the fallback only fires when it is absent. One known edge: a background carrying `asi_fixed_ability_ref` (no SRD background does) adds its +1 on top of a `stat_block` that already includes it.
...
4. **Pass 4b auto-grant walker** — reads the *granting* card, not the granted one: each Class `features` row at or below that class's level, each Subclass row gated by the parent class's level, plus `species`/`subspecies` `granted_feat_refs`. Row `granted_feat_refs` → `autoGrantedFeatIds` (grants applied); `granted_trait_refs` → `autoGrantedTraitIds`; **`always_prepared_spell_refs` → `alwaysPreparedSpells` (R5 / F-pass0-08)** — a 2014-shaped domain / circle spell list arrives in tiers ("1st: false life · 3rd: …"), so it is gated by the row's own level exactly like the feats; built-in 2024 content hands the same mechanic out through a feat card and both land in the same accumulator. A Background's `origin_feat_ref` is deliberately NOT read — the wizard writes it into `feat_ids` so the player can swap it. This replaced an inverse `auto_granted_by` edge on the feat, which stored the level twice and had to scan every entity in the world.
5. **Pass 3** — for each feat: scalar ASI (honoring recorded `feat_asi_choices`, else heuristic first-uncapped option, capped by `asi_max_score`), then `applyGrantsFrom(feat.fields, 'feat:<name>')`. The heuristic reads `asi_ability_options` through [[entity_ref]]'s `abilityAbbrevFromRef` — until audit **M1** (2026-08-13) it read only the unresolved `{_lookup, name}` shape, so **no installed pack's feat ever applied its ASI**; same reader now covers `unarmored_ac_abilities`.
6. **Pass 5** — species, subspecies and background. Subspecies resolved as a first-class `subspecies` entity (by id / name / `legacy_subspecies_key`) or a legacy nested `subspecies_options` row; all three go through `applyGrantsFrom`. Background adds `granted_skill_refs`, `granted_tool_refs`, `granted_languages` (**R5 / F-pass0-09** — the named half of a language line; `granted_language_count` stays the free-pick counter), and `background_asi` (SRD 2024: +2/+1 or +1/+1/+1, total 3, capped 20, gated by `ability_score_options` with a warning on out-of-list). **R5 / F-pass0-03**: `asi_fixed_ability_ref` is A5E's mandatory "+1 Charisma" — applied even when the player recorded nothing, applied **once** when `background_asi` names the same ability (that entry *is* the record of this bump), never counted as out-of-list, and `asi_free_bonus_count` is read as the ceiling on the free half (exceeded → warning).
