...
type: file-note
domain: data-layer
path: flutter_app/lib/data/database/tables/trash_items_table.dart + raw side tables in app_database.dart (asset_refs, content_paths, migration_progress, sync_tombstones)
layer: data
language: dart
status: stable
updated: 2026-09-24
...
> **`trash_items`** is the soft-delete store (replaces the legacy `/trash/` directory). Four **raw side tables** (created with `CREATE TABLE IF NOT EXISTS` in [[drift_database]]'s `beforeOpen`, no Drift codegen, no schema bump) sit alongside it: `asset_refs`, `content_paths`, `migration_progress`, `sync_tombstones`. `lan_paired_devices` was retired in Faz 6 (2026-09-24) — now in `_retiredTablesDDL`.
...
- Used by: `TrashDao`, [[pending_write_buffer]], the eviction sweeper (via `asset_refs`), [[cloud_push_service]] (via `sync_tombstones`).
