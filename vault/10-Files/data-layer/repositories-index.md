...
- **`character_repository.dart`** → `CharacterRepository` (provider `characterRepositoryProvider`). PR-D4 rewrite: characters ride `world_characters` Drift table via `WorldCharactersDao`. `loadAll()`/`loadAllWithLegacy()` (corrupt payloads skipped, sorted by `updatedAt` desc), `save()` upserts. Trash via `trash_items.kind='character'`. `CharacterLoadResult.legacyWorldNames` always empty under v12 fresh-cut. Treats `payloadJson` as opaque ([[character_resolver]] derives mechanics). **`_rowToCharacterJson` anahtarları camelCase olmak zorunda:** `Character.fromJson` json_serializable'ın varsayılan adlandırmasını okuyor (`ownerId`, `worldId`, `templateId`, `createdAt`/`updatedAt`). Snake_case yazıldıkları sürece "kolonlar kanoniktir" satırlarının hiçbiri okunmuyordu ve `...payload` içindeki bayat kopya kazanıyordu — hesap silinip sahiplik kolonda düşürülünce ([[guest_promotion_service]] `releaseAccountCharacters`) karakterler hâlâ silinmiş uid'e ait görünüp own-only karakter sekmesinden kayboluyordu. Test: `test/data/repositories/character_row_columns_test.dart`.
